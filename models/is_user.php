...
if (isSecureCookie('user_login') != true) {
    ...
    if ($SMARTSTUDY->site('status_only_device_client') == 1) {
    ...
    if (function_exists('apcu_exists') && function_exists('apcu_store')) {
        ...
        if (!apcu_exists($cache_key)) {
            ...
            $count_before = $SMARTSTUDY->get_row_safe("SELECT COUNT(id) as total FROM dongtien WHERE user_id = ?", [$getUser['id']])['total'];

            // Thực hiện xóa log
            $SMARTSTUDY->query("DELETE
                FROM dongtien
            ...
            $count_after = $SMARTSTUDY->get_row_safe("SELECT COUNT(id) as total FROM dongtien WHERE user_id = ?", [$getUser['id']])['total'];
