...
if (isSecureCookie('admin_login') != true) {
    ...
    $getUser = $SMARTSTUDY->get_row_safe(" SELECT * FROM `users` WHERE `token` = ? AND `admin` != 0", [$token]);
    ...
    if($SMARTSTUDY->site('status_only_device_admin') == 1){
    ...
    if($SMARTSTUDY->site('status_only_ip_login_admin') == 1){
        if($getUser['ip'] != myip()){
            logout_admin();
        }
    }
