...
$home   = $module == 'client' ? $SMARTSTUDY->site('home_page') : 'home';
...
if (isset($_GET['aff'])) {
    ...
    if ($aff !== false) {
        ...
        if ($user_ref = $SMARTSTUDY->get_row_safe("SELECT id FROM `users` WHERE `id` = ?", [$aff])) {
            // CỘNG LƯỢT CLICK (Prepared)
            $SMARTSTUDY->cong('users', 'ref_click', 1, " `id` = ? ", [$user_ref['id']]);
        }
