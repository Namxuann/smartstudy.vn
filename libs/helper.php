...
$SMARTSTUDY = new DB;
$timezone = $SMARTSTUDY->site('timezone');
if (empty($timezone)) {
    $timezone = 'Asia/Ho_Chi_Minh'; // Default timezone
}
date_default_timezone_set($timezone);
$session_login = $SMARTSTUDY->site('session_login');
...
if ($SMARTSTUDY->get_row_safe("SELECT * FROM `block_ip` WHERE `ip` = ? AND `banned` = 1", [myip()])) {
...
{
    global $CACHE_DATA, $SMARTSTUDY;

    if (!isset($CACHE_DATA['categories_all'])) {
        $CACHE_DATA['categories_all'] = $SMARTSTUDY->get_list_safe(
...
function get_languages_cached()
{
    ...
    if (!isset($CACHE_DATA['languages'])) {
        $CACHE_DATA['languages'] = $SMARTSTUDY->get_list_safe(
...
function get_currencies_cached()
{
    ...
    if (!isset($CACHE_DATA['currencies'])) {
        $CACHE_DATA['currencies'] = $SMARTSTUDY->get_list_safe(
...
function get_payment_manual_cached()
{
    ...
    if (!isset($CACHE_DATA['payment_manual'])) {
        $CACHE_DATA['payment_manual'] = $SMARTSTUDY->get_list_safe(
...
function column_exists($table, $column)
{
    ...
    $result = $SMARTSTUDY->get_row("SHOW COLUMNS FROM `$table` LIKE '$column'");
    return ($result != false);
}
...
function checkBlockIP($type, $time = 15)
{
    ...
    if ($type == 'API') {
        ...
        $max_attempts = $SMARTSTUDY->site('limit_block_ip_api') ? $SMARTSTUDY->site('limit_block_ip_api') : 10;  // Số lần thử tối đa
    } elseif ($type == 'LOGIN') {
        $reason = 'Đăng nhập thất bại quá nhiều lần';
        $max_attempts = $SMARTSTUDY->site('limit_block_ip_login') ? $SMARTSTUDY->site('limit_block_ip_login') : 10;  // Số lần thử tối đa
    } elseif ($type == 'ADMIN') {
        $reason = 'Đăng nhập Admin thất bại quá nhiều lần';
        $max_attempts = $SMARTSTUDY->site('limit_block_ip_admin_access') ? $SMARTSTUDY->site('limit_block_ip_admin_access') : 10;  // Số lần thử tối đa
    } elseif ($type == 'CTV') {
        $reason = 'Đăng nhập CTV Panel thất bại quá nhiều lần';
        $max_attempts = $SMARTSTUDY->site('limit_block_ip_admin_access') ? $SMARTSTUDY->site('limit_block_ip_admin_access') : 10;  // Số lần thử tối đa
    } elseif ($type == 'RESET_PASSWORD') {
        $reason = 'Spam khôi phục mật khẩu';
        $max_attempts = $SMARTSTUDY->site('limit_block_ip_reset_password') ? $SMARTSTUDY->site('limit_block_ip_reset_password') : 10;  // Số lần thử tối đa
    } elseif ($type == 'OTP') {
        $reason = 'Spam OTP';
        $max_attempts = $SMARTSTUDY->site('limit_block_ip_otp') ? $SMARTSTUDY->site('limit_block_ip_otp') : 10;  // Số lần thử tối đa
    } elseif ($type == '2FA') {
        $reason = 'Spam 2FA';
        $max_attempts = $SMARTSTUDY->site('limit_block_ip_2fa') ? $SMARTSTUDY->site('limit_block_ip_2fa') : 10;  // Số lần thử tối đa
    } elseif ($type == 'PAYMENT') {
        $reason = 'Spam Tạo hóa đơn nạp tiền quá nhiều lần';
        $max_attempts = $SMARTSTUDY->site('limit_block_ip_payment') ? $SMARTSTUDY->site('limit_block_ip_payment') : 10;  // Số lần thử tối đa
    } elseif ($type == 'SCAN') {
        ...
        $max_attempts = $SMARTSTUDY->site('limit_block_ip_load_products') ? $SMARTSTUDY->site('limit_block_ip_load_products') : 30;  // Số lần thử tối đa
    } else {
        $reason = 'Spam Request quá nhiều lần';
        $max_attempts = $SMARTSTUDY->site('limit_block_ip_spam') ? $SMARTSTUDY->site('limit_block_ip_spam') : 10;  // Số lần thử tối đa
    }
    // Thêm log thất bại vào bảng failed_attempts
    $SMARTSTUDY->insert("failed_attempts", [
    ...
    $attempts = $SMARTSTUDY->get_row_safe("SELECT COUNT(*) as total FROM `failed_attempts` 
        WHERE `ip_address` = ? 
        AND `type` = ?
    ...
    if ($attempts['total'] >= $max_attempts) {
        ...
        $SMARTSTUDY->remove('failed_attempts', " `ip_address` = '$ip_address' AND `type` = '$type'");
...
function checkDomainAPI($domain, $proxy = '')
{
    ...
    if ($domain === '') {
        return [
            'msg' => 'Vui lòng nhập tên miền nhà cung cấp API.',
            'status' => false
        ];
    }
    ...
    if ($host === null || $host === false || !filter_var($url, FILTER_VALIDATE_URL)) {
        return [
            'msg' => 'Tên miền nhà cung cấp API không hợp lệ.',
            'status' => false
        ];
    }
...
function setSecureCookie($name, $value)
{
    ...
    $options = [
        'expires' => time() + $SMARTSTUDY->site('session_login'),
        'path' => '/',
...
{
    global $SMARTSTUDY;
    if (!$SMARTSTUDY->get_row_safe("SELECT * FROM `settings` WHERE `name` = ?", [$name])) {
        $SMARTSTUDY->insert("settings", [
            'name'  => $name,
            'value' => $value
        ]);
    }
...
{
    global $SMARTSTUDY;
    if (!$SMARTSTUDY->get_row_safe("SELECT * FROM `block_ip` WHERE `ip` = ?", [$ip])) {
        $SMARTSTUDY->insert('block_ip', [
            'ip'        => check_string($ip),
            'attempts'  => 5,
            'banned'    => 1,
...
function checkAccessAttempts($max_attempts = 5)
{
    global $SMARTSTUDY;
    $ip_address = myip();
    $attempt = $SMARTSTUDY->get_row_safe("SELECT * FROM `failed_attempts` WHERE `ip_address` = ? AND `type` = 'Spam Request' ", [$ip_address]);
    ...
    if ($attempt && $attempt['attempts'] >= $max_attempts) {
        ...
        $SMARTSTUDY->remove('failed_attempts', " `ip_address` = ? ", [$ip_address]);
        return true;
    }
    // Nếu chưa đến mức lockout, tăng số lần thử
    if ($attempt) {
        // Cập nhật số lần thất bại
        $SMARTSTUDY->cong('failed_attempts', 'attempts', 1, " `ip_address` = ? ", [$ip_address]);
    } else {
...
{
    $SMARTSTUDY = new DB;
    foreach ($SMARTSTUDY->get_list_safe("SELECT * FROM `product_discount` WHERE `min` <= ? AND `product_id` = ? ORDER BY `min` DESC", [$amount, $product_id]) as $discount) {
...
{
    global $SMARTSTUDY;
    foreach ($SMARTSTUDY->get_list_safe("SELECT * FROM `promotions` WHERE `min` <= ? ORDER by `min` DESC", [$amount]) as $promotion) {
...
function debit_processing($user_id)
{
    ...
    $getUser = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `id` = ?", [$user_id]);
    if ($getUser && $getUser['debit'] > 0) {
        if ($getUser['money'] >= $getUser['debit']) {
            // ĐỦ TIỀN TRẢ NỢ
            $isTru = $SMARTSTUDY->tru('users', 'debit', $getUser['debit'], " `id` = ? ", [$user_id]);
            if ($isTru) {
            ...
            // KHÔNG ĐỦ TIỀN
            $isTru = $SMARTSTUDY->tru('users', 'debit', $getUser['money'], " `id` = ? ", [$user_id]);
            if ($isTru) {
...
function checkPermission($admin_id, $role)
{
    ...
    $row = $SMARTSTUDY->get_row("SELECT * FROM `admin_role` WHERE `id` = '" . $admin_id . "' ");
...
function getStock($code)
{
    ...
    $result = $SMARTSTUDY->get_row("SELECT COUNT(id) as count FROM `product_stock` WHERE `product_code` = '" . $code . "'");
    return $result ? $result['count'] : 0;
}
function currencyDefault()
{
    $SMARTSTUDY = new DB;
    return $SMARTSTUDY->get_row_safe(" SELECT `code` FROM `currencies` WHERE `display` = 1 AND `default_currency` = 1")['code'];
}
...
function setCurrency($id)
{
    ...
    if ($row = $SMARTSTUDY->get_row_safe("SELECT * FROM `currencies` WHERE `id` = ? AND `display` = 1", [$id])) {
...
function getCurrency()
{
    ...
    if (isset($_COOKIE['currency'])) {
        ...
        if ($currency !== false) {
            $rowcurrency = $SMARTSTUDY->get_row_safe("SELECT * FROM `currencies` WHERE `id` = ? AND `display` = 1", [$currency]);
            if ($rowcurrency) {
                return $rowcurrency['id'];
            }
        }
    }
    $rowcurrency = $SMARTSTUDY->get_row_safe("SELECT * FROM `currencies` WHERE `default_currency` = 1", []);
    if ($rowcurrency) {
        return $rowcurrency['id'];
    }
    return false;
...
{
    $SMARTSTUDY = new DB;
    if ($SMARTSTUDY->site('status_ref') != 1) {
        return false;
    }
    $getUser = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `id` = ?", [$user_id]);
    if ($getUser && $getUser['ref_id'] != 0) {
        $price = $price * $SMARTSTUDY->site('ck_ref') / 100;
        $SMARTSTUDY->cong('users', 'ref_money', $price, " `id` = ? ", [$getUser['ref_id']]);
        $SMARTSTUDY->cong('users', 'ref_total_money', $price, " `id` = ? ", [$getUser['ref_id']]);
        $SMARTSTUDY->cong('users', 'ref_amount', $price, " `id` = ? ", [$getUser['id']]);
        $SMARTSTUDY->insert('log_ref', [
...
function sendMessTelegram($my_text, $token = '', $chat_id = '', $proxy = '', $proxy_type = '')
{
    ...
    if ($SMARTSTUDY->site('telegram_status') != 1) {
        return false;
    }
...
function claimSpin($user_id, $trans_id, $total_money)
{
    $SMARTSTUDY = new DB();
    $USER = new users();
    if ($SMARTSTUDY->site('status_spin') == 1) {
        if ($total_money >= $SMARTSTUDY->site('condition_spin')) {
...
{
    global $SMARTSTUDY;
    if ($data = $SMARTSTUDY->get_row_safe("SELECT `" . $row . "` FROM `$table` WHERE `id` = ?", [$id])) {
        return $data[$row];
    }
    return false;
}
...
function url()
{
    ...
    $allowed_domains = array_map('trim', explode(',', $SMARTSTUDY->site('domains'))); // Làm sạch danh sách domains
...
function base_url($url = '')
{
    ...
    if ($host === 'localhost') {
        $base = 'http://localhost/SMARTSTUDY';
    } else {
        $base = $protocol . '://' . $host;
    }
...
{
    $SMARTSTUDY = new DB();
    if ($SMARTSTUDY->site('type_password') == 'md5') {
        return md5($password);
    }
    if ($SMARTSTUDY->site('type_password') == 'bcrypt') {
        return password_hash($password, PASSWORD_BCRYPT);
    }
    if ($SMARTSTUDY->site('type_password') == 'sha1') {
...
{
    $SMARTSTUDY = new DB();
    $result = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `id` = ?", [$id]);
...
{
    $SMARTSTUDY = new DB();
    $currency = $SMARTSTUDY->site('currency');
    if ($currency == 'USD') {
        return '$' . number_format($amount / $SMARTSTUDY->site('usd_rate'), 2, '.', '');
    } elseif ($currency == 'VND') {
...
{
    global $SMARTSTUDY;
    $partner_id = $SMARTSTUDY->site('card_partner_id');
    $partner_key = $SMARTSTUDY->site('card_partner_key');
...
{
    global $SMARTSTUDY;
    return $SMARTSTUDY->site('captcha_status') == 1;
}
...
function isCaptchaEnabledForModule($module)
{
    ...
    $captchaModules = $SMARTSTUDY->site('captcha_modules') ?? '';
...
{
    global $SMARTSTUDY;
    return $SMARTSTUDY->site('captcha_type') ?: 'reCAPTCHA';
}
...
function getCaptchaSiteKey()
{
    ...
    if ($type === 'Cloudflare') {
        return $SMARTSTUDY->site('captcha_site_key');
    } else {
        // Fallback cho reCAPTCHA cũ
        return $SMARTSTUDY->site('captcha_site_key') ?: $SMARTSTUDY->site('reCAPTCHA_site_key');
    }
}
...
function getCaptchaSecretKey()
{
    ...
    if ($type === 'Cloudflare') {
        return $SMARTSTUDY->site('captcha_secret_key');
    } else {
        // Fallback cho reCAPTCHA cũ
        return $SMARTSTUDY->site('captcha_secret_key') ?: $SMARTSTUDY->site('reCAPTCHA_secret_key');
    }
}
...
{
    global $SMARTSTUDY, $getUser;
    if ($SMARTSTUDY->site('status') != 1 && (!isset($getUser) || empty($getUser['admin']) || $getUser['admin'] <= 0)) {
...
function findUserByDescription($description, $bankRechargeType)
{
    ...
    if ($bankRechargeType == 'fullname_transfer') {
        ...
        $prefix = str_replace(' ', '', mb_strtoupper($SMARTSTUDY->site('prefix_autobank'), 'UTF-8'));
        // Ghép prefix_fullname + prefix (xóa space), check trùng LIỀN NHAU trong description (đã xóa space)
        return $SMARTSTUDY->get_row_safe(
        ...
        $prefix = mb_strtoupper($SMARTSTUDY->site('prefix_autobank'), 'UTF-8');
        ...
        if (!empty($prefix) && preg_match('/' . preg_quote($prefix, '/') . '\s*(\d+)/i', $descClean, $m)) {
            $user_id = intval($m[1]);
            if ($user_id > 0) {
                return $SMARTSTUDY->get_row_safe(" SELECT * FROM `users` WHERE `id` = ? ", [$user_id]);
            }
        }
        return null;
    }
