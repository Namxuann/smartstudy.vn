...
if (!$getUser = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `api_key` = ? AND `banned` = 0 AND `admin` != 0", [$api_key])) {
    // Rate limit
    checkBlockIP('API', 15);
    die(json_encode(['status' => 'error', 'msg' => __('API Key không hợp lệ')]));
}
...
foreach ($accountLines as $accountLine) {
    ...
    if ($filter == 1) {
        ...
        if ($SMARTSTUDY->get_row_safe("SELECT * FROM `product_stock` WHERE `uid` = ?", [$uid_validated])) {
            ...
            $sold_exists = $SMARTSTUDY->get_row_safe("SELECT `id` FROM `product_sold` WHERE `uid` = ? LIMIT 1", [$uid_validated]);
            if (!$sold_exists) {
