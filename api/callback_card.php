...
if(isset($_GET['request_id']) && isset($_GET['callback_sign'])){
    ...
    if($callback_sign != md5($SMARTSTUDY->site('card_partner_key').$code.$serial)){
        die('callback_sign_error');
    }
    if (!$row = $SMARTSTUDY->get_row_safe(" SELECT * FROM `cards` WHERE `trans_id` = ? AND `status` = 'pending' ", [$request_id])) {
        die('request_id_error');
    }
    if (!$getUser = $SMARTSTUDY->get_row_safe(" SELECT * FROM `users` WHERE `id` = ? AND `banned` = 0 ", [$row['user_id']])) {
        die('user không hợp lệ');
    }
    if($status == 1){
        if($SMARTSTUDY->site('card_ck') == 0){
            $price = $amount;
        }else{
            $price = $value - $value * $SMARTSTUDY->site('card_ck') / 100;
        }
        $SMARTSTUDY->update("cards", array(
            'status'        => 'completed',
            'price'         => $price,
            'update_date'    => gettime()
        ), " `id` = ? ", [$row['id']]);
