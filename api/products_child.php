...
foreach($SMARTSTUDY->get_list_safe(" SELECT * FROM `categories` WHERE `parent_id` = 0 AND `status` = 1 ") as $parent_category){
    $data_category_child = [];
    
    // Lấy danh sách chuyên mục con của chuyên mục cha này
    foreach($SMARTSTUDY->get_list_safe(" SELECT * FROM `categories` WHERE `parent_id` = '".$parent_category['id']."' AND `status` = 1 ") as $child_category){
        $data_product = [];
        
        // Lấy danh sách sản phẩm của chuyên mục con
        foreach($SMARTSTUDY->get_list_safe(" SELECT * FROM `products` WHERE `category_id` = '".$child_category['id']."' AND `status` = 1 AND `allow_api` = 1 AND `hide_in_shop` = 0 ".(column_exists('products', 'pending') ? " AND `pending` = 0 " : "")." ") as $product){
