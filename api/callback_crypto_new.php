...
$expected_merchant_id = trim($SMARTSTUDY->site('crypto_merchant_id')); // Thay bằng Merchant ID của bạn
$expected_api_key = trim($SMARTSTUDY->site('crypto_api_key')); // Thay bằng API Key của bạn
...
if(!$row = $SMARTSTUDY->get_row_safe(" SELECT * FROM `payment_crypto` WHERE `request_id` = ? ", [$request_id])){
    echo json_encode([
        'status' => 'error',
...
switch ($status) {
    ...
    case 'expired':
        ...
        $SMARTSTUDY->update('payment_crypto', [
            'status'            => 'expired',
    ...
    case 'completed':
        ...
        $isUpdate = $SMARTSTUDY->update('payment_crypto', [
            'status'            => 'completed',
            'update_gettime'    => gettime()
        ], " `id` = ? ", [$row['id']]);
        if($isUpdate){
