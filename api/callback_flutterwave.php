...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($data)) {
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            "Authorization: Bearer ".$SMARTSTUDY->site('flutterwave_secretKey')
        ]);
        ...
        $price = $amount * $SMARTSTUDY->site('flutterwave_rate');
        if (($response['data']['status'] ?? '') == 'successful') {
            if($row = $SMARTSTUDY->get_row_safe(" SELECT * FROM `payment_flutterwave` WHERE `tx_ref` = ? AND `currency` = ? AND `status` = 'pending'  ", [$txRef, $currency])){
                $user = new users;
                $isCong = $user->AddCredits($row['user_id'], $price, __('Recharge Flutterwave').' #'.$id, 'TOPUP_Flutterwave_'.$txRef);
                if($isCong){
                    $SMARTSTUDY->update('payment_flutterwave', [
                        'status'   => 'success',
                        'price'     => $price,
