...
$data_category = [];
foreach($SMARTSTUDY->get_list_safe(" SELECT * FROM `categories` WHERE `status` = 1 ORDER BY `parent_id` ASC, `stt` ASC ") as $category){
    $data_product = [];
    foreach($SMARTSTUDY->get_list_safe(" SELECT * FROM `products` WHERE `category_id` = '".$category['id']."' AND `status` = 1 AND `allow_api` = 1 AND `hide_in_shop` = 0 ".(column_exists('products', 'pending') ? " AND `pending` = 0 " : "")." ") as $product){
