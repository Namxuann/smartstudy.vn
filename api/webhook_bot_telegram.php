...
if ($SMARTSTUDY->site('telegram_assistant_status') != 1) {
...
function sendMessage($chat_id, $text) {
    ...
    $url = "https://api.telegram.org/bot" . $SMARTSTUDY->site('telegram_assistant_token') . "/sendMessage";
...
function sendMessageWithButtons($chat_id, $text, $replyMarkup, $message_id = null) {
    ...
    $url = "https://api.telegram.org/bot" . $SMARTSTUDY->site('telegram_assistant_token') . "/";
    $data = [
        'chat_id'     => $chat_id,
        'text'        => $text,
        'parse_mode'  => 'Markdown',
...
function saveTelegramLog($username, $command, $params, $type = 'message') {
    global $SMARTSTUDY;
    $SMARTSTUDY->insert('telegram_logs', [
        'username' => $username,
        'command'  => $command,
...
$secret_token = $SMARTSTUDY->site('telegram_assistant_secret_token') ?? '';
...
$allowed_usernames = array_map('trim', explode(',', $SMARTSTUDY->site('telegram_assistant_list_username')));
...
// -----------------------------------------------------
$commands = [
    'addfund' => function($params) use ($chat_id, $SMARTSTUDY) {
        // Tách tham số
        ...
        if (!$user = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `username` = ?", [$username])) {
            return sendMessage($chat_id,
    ...
    'removefund' => function($params) use ($chat_id, $SMARTSTUDY) {
        // Tách tham số
        [$usernameParam, $amountParam, $reasonParam] = $params;
    ...
    'balance' => function($params) use ($chat_id, $SMARTSTUDY) {
        // Tách tham số
    ...
    'userinfo' => function($params) use ($chat_id, $SMARTSTUDY) {
        [$usernameParam] = $params;
        if (empty($usernameParam)) {
        ...
        if (!$getUser = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `username` = ?", [$username])) {
    ...
    'listusers' => function($params) use ($chat_id, $SMARTSTUDY) {
        ...
        $totalUsers = $SMARTSTUDY->num_rows_safe("SELECT `id` FROM `users`", []);
        $totalPages = ceil($totalUsers / $perPage);
        // Lấy danh sách user theo trang
        $users = $SMARTSTUDY->get_list_safe("SELECT * FROM `users` ORDER BY `id` DESC LIMIT ? OFFSET ?", [$perPage, $offset]);
    ...
    'blockuser' => function($params) use ($chat_id, $SMARTSTUDY) {
        ...
        if (!$SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `username` = ?", [$username])) {
            return sendMessage($chat_id, "⚠️ *Không tìm thấy thông tin tài khoản: $username*");
        }
        $SMARTSTUDY->update('users', ['banned' => 1, 'reason_banned' => $reason], " `username` = ? ", [$username]);
        sendMessage($chat_id, "✅ Tài khoản `$username` đã bị khóa.");
    },
    'unblockuser' => function($params) use ($chat_id, $SMARTSTUDY) {
        [$usernameParam] = $params;
        if (empty($usernameParam)) {
        ...
        $SMARTSTUDY->update('users', ['banned' => 0], " `username` = ? ", [$username]);
        sendMessage($chat_id, "✅ Tài khoản `$username` đã được mở khóa.");
    },
    'topusers' => function($params) use ($chat_id, $SMARTSTUDY) {
        // Số user hiển thị mỗi trang
        $perPage = 10;
        ...
        $users = $SMARTSTUDY->get_list_safe("
            SELECT * 
            FROM `users` 
    ...
    'siteinfo' => function() use ($chat_id, $SMARTSTUDY) {
        $site_status = $SMARTSTUDY->site('status') == 1 ? 'Hoạt động' : 'Bảo trì';
        sendMessage($chat_id, "🌐 *Thông tin website:*\n".
            "Tên site: *" . $SMARTSTUDY->site('title') . "*\n".
            "Trạng thái: *$site_status*\n".
            "Người dùng: *" . $SMARTSTUDY->num_rows_safe("SELECT `id` FROM `users`", []) . "*");
    },
    'changepassword' => function($params) use ($chat_id, $SMARTSTUDY) {
        ...
        $SMARTSTUDY->update('users', ['password' => TypePassword($new_password)], " `username` = ? ", [$username]);
        sendMessage($chat_id, "✅ Mật khẩu của `$username` đã được thay đổi.");
    },
    'setstatus' => function($params) use ($chat_id, $SMARTSTUDY) {
        [$statusParam] = $params;
        ...
        $SMARTSTUDY->update('settings', ['value' => $status], " `name` = 'status' ");
        sendMessage($chat_id, "✅ " . ($status == 1 ? "Bật" : "Tắt") . " bảo trì thành công.");
    },
    'revenuetoday' => function() use ($chat_id, $SMARTSTUDY) {
        // -----------------------------
        ...
        ";
        $resultToday = $SMARTSTUDY->get_row_safe($queryToday, [$currentDate.'%']);
        ...
        $new_users_today = $SMARTSTUDY->get_row_safe("
        ...
        $queryYesterday = "
            ...
            AND `create_gettime` LIKE ?
        ";
        $resultPrev = $SMARTSTUDY->get_row_safe($queryYesterday, [$yesterdayDate.'%']);
    ...
    'revenueweek' => function() use ($chat_id, $SMARTSTUDY) {
        // -----------------------------
        ...
        ";
        $resultThis = $SMARTSTUDY->get_row_safe($queryThisWeek, [$startOfWeek, $endOfWeek]);
        ...
        $new_users_week = $SMARTSTUDY->get_row_safe("
        ...
        $queryPrevWeek = "
            ...
            AND `create_gettime` BETWEEN '? 00:00:00' AND '? 23:59:59'
        ";
        $resultPrev = $SMARTSTUDY->get_row_safe($queryPrevWeek, [$previousWeekStart, $previousWeekEnd]);
    ...
    'revenuemonth' => function() use ($chat_id, $SMARTSTUDY) {
        // -----------------------------
        ...
        ";
        $resultThis = $SMARTSTUDY->get_row_safe($queryThisMonth, [$startOfMonth.' 00:00:00', $endOfMonth.' 23:59:59']);
        ...
        $new_users_month = $SMARTSTUDY->get_row_safe("
        ...
        $queryPrevMonth = "
            ...
            AND `create_gettime` BETWEEN ? AND ?
        ";
        $resultPrev = $SMARTSTUDY->get_row_safe($queryPrevMonth, [$previousMonthStart.' 00:00:00', $previousMonthEnd.' 23:59:59']);
    ...
    'orders' => function($params) use ($SMARTSTUDY, $chat_id) {
        $perPage = 5;
        ...
        $offset      = ($page - 1) * $perPage;
        $totalOrders = $SMARTSTUDY->num_rows_safe("SELECT `id` FROM `product_order`", []);
        $totalPages  = ceil($totalOrders / $perPage);

        // Lấy danh sách đơn hàng
        $orders  = $SMARTSTUDY->get_list_safe("SELECT * FROM `product_order` 
                                     ORDER BY `id` DESC 
    ...
    'checkorder' => function($params) use ($chat_id, $SMARTSTUDY) {
        ...
        $product = $SMARTSTUDY->get_row_safe("SELECT * FROM `products` WHERE `id` = ?", [$order['product_id']]);
    ...
    'bestsellers' => function($params) use ($chat_id, $SMARTSTUDY) {
        ...
        $rowCount = $SMARTSTUDY->get_row_safe("
    ...
    'logs' => function($params) use ($chat_id, $SMARTSTUDY) {
    ...
    'log' => function($params) use ($chat_id, $SMARTSTUDY) {
        ...
        $user = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `username` = ?", [$usernameValid]);
        if (!$user) {
        ...
        $user_id   = $user['id'];
        $totalLogs = $SMARTSTUDY->num_rows_safe("SELECT `id` FROM `logs` WHERE `user_id` = ?", [$user_id]);
        $totalPages = $totalLogs > 0 ? ceil($totalLogs / $perPage) : 1;
    ...
    'topmoney' => function($params) use ($chat_id, $SMARTSTUDY) {
        // Số user hiển thị mỗi trang
        $perPage = 10;
...
if ($callback_query) {
    ...
    if (isset($commands[$command])) {
        ...
        file_get_contents("https://api.telegram.org/bot".$SMARTSTUDY->site('telegram_assistant_token')."/answerCallbackQuery?".
            http_build_query([
...
$botToken = $SMARTSTUDY->site('telegram_assistant_token');
