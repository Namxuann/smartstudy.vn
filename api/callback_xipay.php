...
if($SMARTSTUDY->site('gateway_xipay_status') != 1){
...
$epay_config['pid'] = $SMARTSTUDY->site('gateway_xipay_pid');
$epay_config['key'] = $SMARTSTUDY->site('gateway_xipay_md5key');
...
if ($verify_result) { // Nếu xác thực thành công
    ...
    if ($trade_status == 'TRADE_SUCCESS') {
        if($row = $SMARTSTUDY->get_row_safe(" SELECT * FROM `payment_xipay` WHERE `out_trade_no` = ? AND `transaction_id` IS NULL AND `status` =  0 ", [$out_trade_no])){
            $user = new users;
        ...
        if($row = $SMARTSTUDY->get_row_safe(" SELECT * FROM `payment_xipay` WHERE `out_trade_no` = ? AND `status` = 0 ", [$out_trade_no])){
            $SMARTSTUDY->update('payment_xipay', [
                'status'    => 2
