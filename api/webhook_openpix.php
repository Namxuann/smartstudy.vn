...
$secretKeyOnOpenpixPlatform = $SMARTSTUDY->site('openpix_HMAC_key_completed');
if (isset($data['event'])) {
    if ($data['event'] == 'OPENPIX:CHARGE_COMPLETED') {
        ...
        $secretKeyOnOpenpixPlatform = $SMARTSTUDY->site('openpix_HMAC_key');
    } else {
...
if(hash_equals($hmac, (string)$signature)) {
    ...
    if ($data['event'] == 'OPENPIX:CHARGE_COMPLETED') {
        ...
        if ($status == 'COMPLETED') {
            if ($row = $SMARTSTUDY->get_row_safe("SELECT * FROM `payment_openpix` WHERE `trans_id` = ? AND `status` = 0", [$userCorrelationID])) {
                $user = new users;
