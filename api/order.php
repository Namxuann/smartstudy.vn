...
$order = $SMARTSTUDY->get_row_safe("SELECT * FROM `product_order` WHERE `trans_id` = ? AND `buyer` = ?", [$order_id, $user_id]);
if(!$order){
...
$accounts_data = $SMARTSTUDY->get_list("SELECT * FROM `product_sold` WHERE `trans_id` = '".$order['trans_id']."' ");
