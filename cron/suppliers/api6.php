<?php
    ...
    /* START CHỐNG SPAM */
    $elapsed = time() - (int)$SMARTSTUDY->site('time_cron_suppliers_api6');
    ...
    foreach($SMARTSTUDY->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_6']) as $supplier){
        // CẬP NHẬT SỐ DƯ API
