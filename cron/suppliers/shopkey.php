...
/* START CHỐNG SPAM */
$elapsed = time() - (int)$SMARTSTUDY->site('time_cron_suppliers_shopkey');
...
foreach ($SMARTSTUDY->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'SHOPKEY']) as $supplier) {
    // CẬP NHẬT SỐ DƯ API
