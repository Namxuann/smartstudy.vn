...
$elapsed = time() - (int)$SMARTSTUDY->site('time_cron_suppliers_api45');
if ($elapsed >= 0 && $elapsed < 5) {
    die('Thao tác quá nhanh, vui lòng thử lại sau!');
}
$SMARTSTUDY->update("settings", [
    'value' => time()
], " `name` = 'time_cron_suppliers_api45' ");

// Lặp qua tất cả nhà cung cấp API_45 đang hoạt động
foreach ($SMARTSTUDY->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_45']) as $supplier) {

    // =============================================
    // HEALTH CHECK - Lấy số dư tài khoản
    // Endpoint: GET /api/telegram-buyer/balance?key=xxx (domain được mã hóa trong hàm)
    // =============================================
    if (!empty($supplier['token'])) {
        $result_raw = balance_API_45('', $supplier['token'], $supplier['proxy']);
    ...
    // =============================================
    // ĐỒNG BỘ SẢN PHẨM
    // Endpoint: GET /api/telegram-buyer/products?key=xxx (domain được mã hóa trong hàm)
    ...
    $result_raw = listProduct_API_45('', $supplier['token'], $supplier['proxy']);
        ...
        foreach ($data['products'] as $product_api) {
            ...
            if (!$product = $SMARTSTUDY->get_row_safe(" SELECT * FROM `products` WHERE `api_id` = ? AND `supplier_id` = ? ", [$api_id, $supplier['id']])) {
                ...
                if ($SMARTSTUDY->site('debug_api_suppliers') == 1) {
                    echo '<b style="color:red;">CREATE</b> - [API_45] Tạo sản phẩm ' . $api_name . ' (ID: ' . $api_id . ') thành công!<br>';
                }
            } else {
                ...
                if ($SMARTSTUDY->site('debug_api_suppliers') == 1) {
                    echo '<b style="color:green;">UPDATE</b> - [API_45] Cập nhật sản phẩm ' . $api_name . ' (ID: ' . $api_id . ') thành công!<br>';
                }
            }
    ...
    // =============================================
    if ($hasData) {
        $SMARTSTUDY->remove('products', " `supplier_id` = ? AND " . time() . " - `api_time_update` >= 3600 ", [$supplier['id']]);
    }
}
