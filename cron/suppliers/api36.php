...
/* START CHỐNG SPAM */
$elapsed = time() - (int)$SMARTSTUDY->site('time_cron_suppliers_api36');
...
foreach ($SMARTSTUDY->get_list_safe(" SELECT * FROM `suppliers` WHERE `status` = ? AND `type` = ? ", [1, 'API_36']) as $supplier) {
    // CẬP NHẬT SỐ DƯ API
    ...
    if ($current_page > $last_page) {
        ...
        $SMARTSTUDY->remove('products', " `supplier_id` = '" . $supplier['id'] . "' AND " . time() . " - `api_time_update` >= 7200 ");
