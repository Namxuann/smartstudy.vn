<?php
    ...
    /* START CHỐNG SPAM */
    $elapsed = time() - (int)$SMARTSTUDY->site('time_cron_checklive_hotmail');
    ...
    if($SMARTSTUDY->site('api_check_live_hotmail') == ''){
...
$products_list = $SMARTSTUDY->get_list_safe("SELECT * FROM `products` WHERE `check_live` = ?", ['Hotmail']);
...
$limit = intval($SMARTSTUDY->site('limit_check_live_clone')) ?? 500;
$thirty_minutes_ago = time() - $SMARTSTUDY->site('time_limit_check_live_hotmail'); 
$products = $SMARTSTUDY->get_list_safe("SELECT * FROM `product_stock` WHERE `time_check_live` < ? $where_is_checklive ORDER BY `time_check_live` ASC LIMIT ?", [$thirty_minutes_ago, $limit]);
...
foreach ($uids as $uid) {
    $ch = curl_init();
    $url = $SMARTSTUDY->site('api_check_live_hotmail')."?account=".$uid."&api_key=".$SMARTSTUDY->site('api_key_check_live_hotmail');
...
foreach ($curl_handles as $uid => $ch) {
    ...
    if ($info['http_code'] == 200) {
        ...
        if (isset($result_array['data']) && $result_array['data']['status'] == 'live') {
            ...
            $SMARTSTUDY->update("product_stock", ['time_check_live' => time()], " `id` = '".$products_info[$uid]['id']."' ");
            echo "HOTMAIL: ".substr($uid, 0, 8)."*******, Result: LIVE <br>";
            ...
            try {
                ...
                $isInsert = $SMARTSTUDY->insert('product_die', [
                ...
                if($isInsert){
                    ...
                    $SMARTSTUDY->remove('product_stock', " `id` = '".$products_info[$uid]['id']."' ");
                    echo "HOTMAIL: ".substr($uid, 0, 8)."*******, Result: DIE ".$result."<br>";
                    $SMARTSTUDY->query("COMMIT");
                } else {
                    $SMARTSTUDY->query("ROLLBACK");
                    echo "HOTMAIL: ".substr($uid, 0, 8)."*******, Result: DIE | Lỗi: Không thể chèn dữ liệu <br>";
                }
            } catch (Exception $e) {
                $SMARTSTUDY->query("ROLLBACK");
                echo "HOTMAIL: ".substr($uid, 0, 8)."*******, Result: DIE | Lỗi: ".$e->getMessage()." <br>";
            }
        }else{
            $error_message = "HOTMAIL: ".substr($uid, 0, 8)."*******, Result: ERROR, Error: " . $result;
