<?php
    ...
    /* START CHỐNG SPAM */
    $elapsed = time() - (int)$SMARTSTUDY->site('time_cron_checklive_clone');
    ...
    $products_list = $SMARTSTUDY->get_list_safe("SELECT * FROM `products` WHERE `check_live` = ?", ['Clone']);
    ...
    $products = $SMARTSTUDY->get_list_safe("SELECT * FROM `product_stock` WHERE `id` > ? $where_is_checklive ORDER BY `time_check_live` ASC LIMIT ? ", [0, $limit]);
    ...
    foreach ($uid_chunks as $chunk_index => $chunk) {
        ...
        if (!empty($live_ids)) {
            ...
            $SMARTSTUDY->query("UPDATE `product_stock` SET `time_check_live` = ".time()." WHERE `id` IN ($live_ids_str)");
        ...
        if (!empty($error_ids)) {
            ...
            $SMARTSTUDY->query("UPDATE `product_stock` SET `time_check_live` = ".time()." WHERE `id` IN ($error_ids_str)");
            ...
            try {
                ...
                foreach ($die_records as $record) {
                    ...
                    if($isInsert){
                        ...
                        $SMARTSTUDY->remove('product_stock', " `id` = '".$info['id']."' ");
                        $success_count++;
                    } else {
                        $failed_count++;
                    }
                ...
                $SMARTSTUDY->query("UPDATE `product_stock` SET `time_check_live` = ".time()." WHERE `id` IN ($die_ids_str)");
            }
        }
