<?php
    ...
    /* START CHỐNG SPAM */
    $elapsed = time() - (int)$SMARTSTUDY->site('time_cron_checklive_gmail');
    ...
    if($SMARTSTUDY->site('api_check_live_gmail') == ''){
...
$products_list = $SMARTSTUDY->get_list_safe("SELECT * FROM `products` WHERE `check_live` = ?", ['Gmail']);
...
$thirty_minutes_ago = time() - $SMARTSTUDY->site('time_limit_check_live_gmail'); // 1800 giây = 30 phút mới check tiếp gmail đó
...
// Khởi tạo cURL
$ch = curl_init();
$url = $SMARTSTUDY->site('api_check_live_gmail');
$api_key = $SMARTSTUDY->site('api_key_check_live_gmail');
...
if ($info['http_code'] == 200) {
    ...
    foreach ($response_data['data'] as $email_data) {
        ...
        if ($status == 'live') {
            ...
            $SMARTSTUDY->update("product_stock", ['time_check_live' => time()], " `id` = '".$products_info[$email]['id']."' ");
            ...
            try {
                ...
                if($isInsert){
                    ...
                    $SMARTSTUDY->remove('product_stock', " `id` = '".$products_info[$email]['id']."' ");
