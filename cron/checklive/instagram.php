<?php
    ...
    /* START CHỐNG SPAM */
    $elapsed = time() - (int)$SMARTSTUDY->site('time_cron_checklive_instagram');
    ...
    if($SMARTSTUDY->site('api_check_live_instagram') == ''){
...
$products_list = $SMARTSTUDY->get_list_safe("SELECT * FROM `products` WHERE `check_live` = ?", ['Instagram']);
...
$thirty_minutes_ago = time() - $SMARTSTUDY->site('time_limit_check_live_instagram'); // 1800 giây = 30 phút mới check tiếp gmail đó
...
foreach ($uids as $uid) {
    $ch = curl_init();
    $url = $SMARTSTUDY->site('api_check_live_instagram')."?account=".$uid."&api_key=".$SMARTSTUDY->site('api_key_check_live_instagram');
