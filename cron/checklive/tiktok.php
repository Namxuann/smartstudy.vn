<?php
    ...
    /* START CHỐNG SPAM */
    $elapsed = time() - (int)$SMARTSTUDY->site('time_cron_checklive_tiktok');
    ...
    if($SMARTSTUDY->site('api_check_live_tiktok') == ''){
...
$products_list = $SMARTSTUDY->get_list_safe("SELECT * FROM `products` WHERE `check_live` = ?", ['Tiktok']);
...
$thirty_minutes_ago = time() - $SMARTSTUDY->site('time_limit_check_live_tiktok'); 
...
foreach ($uids as $uid) {
    $ch = curl_init();
    $url = $SMARTSTUDY->site('api_check_live_tiktok')."?account=".$uid."&api_key=".$SMARTSTUDY->site('api_key_check_live_tiktok');
