...
if ($SMARTSTUDY->site('order_by_product_home') == 1) {
    $order_by = ' ORDER BY `stt` DESC ';
} else if ($SMARTSTUDY->site('order_by_product_home') == 2) {
    $order_by = ' ORDER BY `price` ASC ';
} else if ($SMARTSTUDY->site('order_by_product_home') == 3) {
...
if ($action == 'search' && !empty($keyword)) {
    ...
    $sql_list = "SELECT `id`, `name`, `slug`, `price`, `discount`, `short_desc`, `sold`, `code`, `supplier_id`, `api_stock`, `preview_uid`, `product_type` FROM `products` WHERE $where_clause $order_by LIMIT ?, ?";
    $params_with_limit = array_merge($where_params, [$from, $limit]);
    $listDatatable = $SMARTSTUDY->get_list_safe($sql_list, $params_with_limit);

    $sql_count = "SELECT COUNT(*) as total FROM `products` WHERE $where_clause";
    $totalDatatable = $SMARTSTUDY->get_row_safe($sql_count, $where_params)['total'];
...
?>
    ...
    <div class="row row-cols-1 row-cols-md-1 row-cols-lg-1 row-cols-xl-1">
        <?php foreach ($listDatatable as $product): ?>
            <?php $stock = ($product['product_type'] === 'course') ? 1 : ($product['supplier_id'] != 0 ? $product['api_stock'] : getStock($product['code'])); ?>
            <?php
            if ($SMARTSTUDY->site('product_hide_outstock') == 1 && $stock == 0) {
                continue;
            }
            ?>
            <div>
                    ...
                    <div class="feature-content">
                        <div class="row">
                            <div class="col-8 col-md-9">
                                ...
                                <div class="row">
                                    <?php if ($SMARTSTUDY->site('product_rating_display') == 1): ?>
                                    ...
                                    <?php endif ?>
                                    <div class="col-12 col-md-12">
                                        <label class="label-text feat"><?= $product['product_type'] === 'course' ? __('Hình thức:') : __('Kho hàng:'); ?>
                                            <b><?= $product['product_type'] === 'course' ? __('Học trực tuyến') : format_cash($stock); ?></b></label>
                                        <?php if ($SMARTSTUDY->site('product_sold_display') == 1): ?>
...
?>
    <div class="col-lg-12 mb-5" id="category<?= $category['id']; ?>">
        <?php if ($SMARTSTUDY->site('type_show_product') == 'BOX'): ?>
            <div class="home-heading mb-3">
                <h3>
            ...
            <div
                class="row row-cols-1 row-cols-md-1 row-cols-lg-2 <?= $SMARTSTUDY->site('cot_so_du_ben_phai') == 1 ? 'row-cols-xl-2' : 'row-cols-xl-3'; ?>">
                <?php
                // ⚡ Select chỉ các fields cần thiết thay vì SELECT *
                $i = -1;
                foreach ($SMARTSTUDY->get_list_safe(" SELECT `id`, `name`, `slug`, `price`, `discount`, `short_desc`, `sold`, `code`, `supplier_id`, `api_stock`, `images`, `flag`, `preview_uid`, `product_type` FROM `products` WHERE `status` = ? AND `category_id` = ? AND `hide_in_shop` = 0 " . (column_exists('products', 'pending') ? " AND `pending` = 0 " : "") . " $order_by ", [1, $category['id']]) as $product): ?>
                    <?php $stock = ($product['product_type'] === 'course') ? 1 : ($product['supplier_id'] != 0 ? $product['api_stock'] : getStock($product['code'])); ?>
                    <?php
                    if ($SMARTSTUDY->site('product_hide_outstock') == 1 && $stock == 0) {
                        continue;
                    ...
                    if ($category_id == '' && $i >= $SMARTSTUDY->site('max_show_product_home')) {
                        continue;
                    }
                    ?>
                    <div>
        ...
        <?php elseif ($SMARTSTUDY->site('type_show_product') == 'BOX_5'): ?>
            <div class="home-heading mb-3">
                <h3>
                    <img src="<?= base_url($category['icon']); ?>">
                    <?= $category['name']; ?>
        ...
        <?php elseif ($SMARTSTUDY->site('type_show_product') == 'BOX_6'): ?>
            <div class="home-heading mb-3">
                <h3>
                    <img src="<?= base_url($category['icon']); ?>">
                    <?= $category['name']; ?>
        ...
        <?php elseif ($SMARTSTUDY->site('type_show_product') == 'LIST'): ?>
            <div class="home-heading mb-3">
                <h3>
                    <img src="<?= base_url($category['icon']); ?>">
                    <?= $category['name']; ?>
        ...
        <?php elseif ($SMARTSTUDY->site('type_show_product') == 'BOX_4'): ?>
            <div class="home-heading mb-3">
                <h3>
                    <img src="<?= base_url($category['icon']); ?>">
                    <?= $category['name']; ?>
            ...
            <div class="row">
                ...
                foreach ($SMARTSTUDY->get_list_safe(" SELECT `id`, `name`, `slug`, `price`, `discount`, `short_desc`, `sold`, `code`, `supplier_id`, `api_stock`, `images`, `flag`, `preview_uid`, `product_type` FROM `products` WHERE `status` = ? AND `category_id` = ? AND `hide_in_shop` = 0 " . (column_exists('products', 'pending') ? " AND `pending` = 0 " : "") . " $order_by ", [1, $category['id']]) as $product): ?>
                    ...
                    ?>
                    <div class="prod-item col-sm-6 col-md-4 <?= $SMARTSTUDY->site('cot_so_du_ben_phai') == 1 ? 'col-xl-4' : 'col-xl-3'; ?> mb-3"
                        ...
                        <div class="product-box4 ">
                            ...
                            <div class="product-footer-box4">
                                <div class="row">
                                    ...
                                    <div class="col-4 text-center border-end-box4">
                                        <strong><?= $product['product_type'] === 'course' ? __('Hình thức') : __('Hiện có'); ?></strong>
                                        <span
                                            class="badge bg-primary rounded-pill"><?= $product['product_type'] === 'course' ? __('Học online') : format_cash($stock); ?></span>
                                    </div>
                                    <div class="col-4">
        ...
        <?php if ($category_id == '' && $i >= $SMARTSTUDY->site('max_show_product_home')): ?>
