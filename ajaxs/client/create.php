...
if ($_POST['action'] == 'WithdrawCommission') {
    ...
    if ($SMARTSTUDY->site('affiliate_status') != 1) {
    ...
    if (!$getUser = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `token` = ? AND `banned` = 0", [$token])) {
        die(json_encode(['status' => 'error', 'msg' => __('Vui lòng đăng nhập')]));
    }
    ...
    if ($amount < $SMARTSTUDY->site('affiliate_min')) {
        die(json_encode(['status' => 'error', 'msg' => __('Số tiền rút tối thiểu phải là') . ' ' . format_currency($SMARTSTUDY->site('affiliate_min'))]));
    }
    ...
    if ($isTru) {
        ...
        $isInsert = $SMARTSTUDY->insert('aff_withdraw', [
            'trans_id'  => $trans_id,
            'user_id'   => $getUser['id'],
            'bank'      => $bank,
            'stk'       => $stk,
        ...
        if ($isInsert) {
            /** NOTE ACTION */
            $my_text = $SMARTSTUDY->site('noti_affiliate_withdraw');
            ...
            sendMessTelegram($my_text, '', $SMARTSTUDY->site('affiliate_chat_id_telegram'));
...
if ($_POST['action'] == 'nap_the') {
    if ($SMARTSTUDY->site('card_status') != 1) {
    ...
    $list_network_topup_card = $SMARTSTUDY->site('list_network_topup_card');
    ...
    if ($SMARTSTUDY->num_rows_safe("SELECT * FROM `cards` WHERE `user_id` = ? AND `status` = 'pending'", [$getUser['id']]) > 5) {
        die(json_encode(['status' => 'error', 'msg' => __('Vui lòng không spam!')]));
    }
    if (
        $SMARTSTUDY->num_rows_safe("SELECT * FROM `cards` WHERE `status` = 'error' AND `user_id` = ? AND `create_date` >= DATE(NOW()) AND `create_date` < DATE(NOW()) + INTERVAL 1 DAY", [$getUser['id']]) -
        $SMARTSTUDY->num_rows_safe("SELECT * FROM `cards` WHERE `status` = 'complted' AND `user_id` = ? AND `create_date` >= DATE(NOW()) AND `create_date` < DATE(NOW()) + INTERVAL 1 DAY", [$getUser['id']]) >= 5
    ) {
    ...
    if ($data['status'] == 99) {
        $isInsert = $SMARTSTUDY->insert("cards", array(
            'trans_id'  => $trans_id,
            'telco'     => $telco,
            'amount'    => $amount,
            'serial'    => $serial,
...
if ($_POST['action'] == 'RechargeCryptoNew') {
    ...
    if ($amount < $SMARTSTUDY->site('crypto_min')) {
        die(json_encode(['status' => 'error', 'msg' => __('The minimum deposit amount is:') . ' $' . $SMARTSTUDY->site('crypto_min')]));
    }
    if ($amount > $SMARTSTUDY->site('crypto_max')) {
        die(json_encode(['status' => 'error', 'msg' => __('The maximum deposit amount is:') . ' $' . format_cash($SMARTSTUDY->site('crypto_max'))]));
    }
    if ($SMARTSTUDY->site('crypto_status') != 1) {
        die(json_encode(['status' => 'error', 'msg' => __('This function is under maintenance')]));
    }
    if ($SMARTSTUDY->site('crypto_merchant_id') == '' || $SMARTSTUDY->site('crypto_api_key') == '') {
        die(json_encode(['status' => 'error', 'msg' => __('Chức năng này chưa được cấu hình, vui lòng liên hệ Admin')]));
    }
    if ($SMARTSTUDY->num_rows_safe("SELECT * FROM `payment_crypto` WHERE `user_id` = ? AND `status` = 'waiting' AND ROUND(`amount`) = ?", [$getUser['id'], $amount]) >= 3) {
    ...
    curl_setopt_array($curl, array(
        ...
        CURLOPT_POSTFIELDS => array(
            'merchant_id'     => $SMARTSTUDY->site('crypto_merchant_id'),                          // Không được để lộ thông tin này
            'api_key'         => $SMARTSTUDY->site('crypto_api_key'),                 // Không được để lộ thông tin này
    ...
    if ($result['status'] == 'success') {
        ...
        $received = calculateCryptoReceivedAmount($amount, $SMARTSTUDY->site('crypto_promotions'));
        $received = $received * $SMARTSTUDY->site('crypto_rate');
        $isInsert = $SMARTSTUDY->insert('payment_crypto', [
            'trans_id'          => $trans_id,
            'user_id'           => $getUser['id'],
            'request_id'        => $request_id,
            'amount'            => $amount,
            'received'          => $received,
            'exchange_rate'     => $SMARTSTUDY->site('crypto_rate'), // Lưu tỷ giá USDT tại thời điểm tạo hóa đơn
            'create_gettime'    => gettime(),
            'update_gettime'    => gettime(),
            'status'            => $status,
...
if ($_POST['action'] == 'CreateToyyibpay') {
    ...
    if ($SMARTSTUDY->site('status') != 1 && isSecureCookie('admin_login') != true) {
        die(json_encode(['status' => 'error', 'msg' => __('The system is maintenance')]));
    }
    if ($SMARTSTUDY->site('toyyibpay_status') != 1) {
        die(json_encode(['status' => 'error', 'msg' => __('This function is under maintenance')]));
    }
    if ($SMARTSTUDY->site('toyyibpay_userSecretKey') == '') {
    ...
    if ($amount < $SMARTSTUDY->site('toyyibpay_min')) {
        die(json_encode(['status' => 'error', 'msg' => __('Minimum deposit amount is RM' . $SMARTSTUDY->site('toyyibpay_min') . '')]));
    }
    $trans_id = random('QWERTYUIOPASDFGHJKLZXCVBNM', 3) . time();

    $toyyibpay = new toyyibpay($SMARTSTUDY->site('toyyibpay_userSecretKey'));
    $result = $toyyibpay->createBill([
        'categoryCode' => $SMARTSTUDY->site('toyyibpay_categoryCode'),
        ...
        'billChargeToCustomer'  => $SMARTSTUDY->site('toyyibpay_billChargeToCustomer'),
        'billExpiryDate'    => '',
        'billExpiryDays'    => 3
    ]);
    ...
    $isInsert = $SMARTSTUDY->insert('payment_toyyibpay', array(
        'user_id'           => $getUser['id'],
        'trans_id'          => $trans_id,
        'billName'          => 'Invoice - RM ' . $amount,
        'amount'            => $amount,
...
if ($_POST['action'] == 'RechargeKorapay') {
    ...
    if ($SMARTSTUDY->site('korapay_status') != 1) {
        die(json_encode(['status' => 'error', 'msg' => __('This function is under maintenance')]));
    }
    if ($SMARTSTUDY->site('korapay_secretKey') == '') {
        die(json_encode(['status' => 'error', 'msg' => __('This function has not been configured')]));
    }
    if (empty($_POST['token'])) {
        die(json_encode(['status' => 'error', 'msg' => __('Please log in')]));
    ...
    if ($amount < $SMARTSTUDY->site('korapay_min')) {
        die(json_encode(['status' => 'error', 'msg' => __('Minimum deposit amount is ' . $SMARTSTUDY->site('korapay_min') . '')]));
    }
    if ($amount > $SMARTSTUDY->site('korapay_max')) {
        die(json_encode(['status' => 'error', 'msg' => __('Maximum deposit amount is ' . $SMARTSTUDY->site('korapay_max') . '')]));
    }
    $trans_id = random('QWERTYUIOPASDFGHJKLZXCVBNM', 3) . time();
    // Tính số tiền thực nhận: áp dụng khuyến mãi nạp tiền (nếu có) rồi nhân tỷ giá
    $received_amount = calculateCryptoReceivedAmount($amount, $SMARTSTUDY->site('korapay_promotions'));
    $price = $received_amount * $SMARTSTUDY->site('korapay_rate');
    ...
    $params = [
        ...
        "currency"    => $SMARTSTUDY->site('korapay_currency_code'),
        "reference"   => $trans_id,
    ...
    $secretKey = $SMARTSTUDY->site('korapay_secretKey'); // Thay bằng secret key thực tế
    ...
    if ($response && isset($response['status']) && $response['status'] === true) {
        ...
        $isInsert = $SMARTSTUDY->insert('payment_korapay', array(
            'user_id'           => $getUser['id'],
            'trans_id'          => $reference,
            'price'             => $price,
            'amount'            => $amount,
...
if ($_POST['action'] == 'RechargePocketfi') {
    ...
    if ($SMARTSTUDY->site('pocketfi_status') != 1) {
        die(json_encode(['status' => 'error', 'msg' => __('This function is under maintenance')]));
    }
    if ($SMARTSTUDY->site('pocketfi_api_token') == '' || $SMARTSTUDY->site('pocketfi_business_id') == '') {
        die(json_encode(['status' => 'error', 'msg' => __('This function has not been configured')]));
    }
    if (empty($_POST['token'])) {
        die(json_encode(['status' => 'error', 'msg' => __('Please log in')]));
    ...
    if ($amount < $SMARTSTUDY->site('pocketfi_min')) {
        die(json_encode(['status' => 'error', 'msg' => __('Minimum deposit amount is ' . $SMARTSTUDY->site('pocketfi_min') . '')]));
    }
    if ($amount > $SMARTSTUDY->site('pocketfi_max')) {
        die(json_encode(['status' => 'error', 'msg' => __('Maximum deposit amount is ' . $SMARTSTUDY->site('pocketfi_max') . '')]));
    }
    $trans_id = random('QWERTYUIOPASDFGHJKLZXCVBNM', 3) . time();
    $price = $amount * $SMARTSTUDY->site('pocketfi_rate');
    ...
    $params = [
        ...
        "last_name"     => "User",
        "phone"         => $phone,
        "business_id"   => strval($SMARTSTUDY->site('pocketfi_business_id')),
        "email"         => $email,
    ...
    $apiToken = $SMARTSTUDY->site('pocketfi_api_token');
    ...
    if ($response && isset($response['status']) && $response['status'] === 'success') {
        ...
        $isInsert = $SMARTSTUDY->insert('payment_pocketfi', array(
            'user_id'           => $getUser['id'],
            'trans_id'          => $trans_id,
...
if ($_POST['action'] == 'RechargeTmweasyapi') {
    ...
    if ($SMARTSTUDY->site('tmweasyapi_status') != 1) {
        die(json_encode(['status' => 'error', 'msg' => __('This function is under maintenance')]));
    }
    if ($SMARTSTUDY->site('tmweasyapi_username') == '' || $SMARTSTUDY->site('tmweasyapi_password') == '' || $SMARTSTUDY->site('tmweasyapi_con_id') == '') {
        die(json_encode(['status' => 'error', 'msg' => __('This function has not been configured')]));
    }
    if (empty($_POST['token'])) {
        die(json_encode(['status' => 'error', 'msg' => __('Please log in')]));
    ...
    if ($amount < $SMARTSTUDY->site('tmweasyapi_min')) {
        die(json_encode(['status' => 'error', 'msg' => __('Minimum deposit amount is ' . $SMARTSTUDY->site('tmweasyapi_min') . '')]));
    }
    if ($amount > $SMARTSTUDY->site('tmweasyapi_max')) {
        die(json_encode(['status' => 'error', 'msg' => __('Maximum deposit amount is ' . $SMARTSTUDY->site('tmweasyapi_max') . '')]));
    }
    $trans_id = random('QWERTYUIOPASDFGHJKLZXCVBNM', 3) . time();
    $price = $amount * $SMARTSTUDY->site('tmweasyapi_rate');

    require_once(__DIR__ . "/../../libs/tmweasyapi.php");


    $paramsCreate = [
        "username" => $SMARTSTUDY->site('tmweasyapi_username'),
        "password" => $SMARTSTUDY->site('tmweasyapi_password'),
        "con_id"   => $SMARTSTUDY->site('tmweasyapi_con_id'),
        "amount"   => $amount,
        "ref1"     => $trans_id,
        "method"   => "create_pay"
    ];
    ...
    if (isset($responseCreate['status']) && $responseCreate['status'] == 1) {
        ...
        if (isset($responseDetail['status']) && $responseDetail['status'] == 1) {
            ...
            $isInsert = $SMARTSTUDY->insert('payment_tmweasyapi', array(
                'user_id'           => $getUser['id'],
                'trans_id'          => $trans_id,
                'id_pay'            => $idPay,
                'price'             => $price,
...
if ($_POST['action'] == 'RechargeOpenPix') {
    ...
    if ($SMARTSTUDY->site('openpix_status') != 1) {
        die(json_encode(['status' => 'error', 'msg' => __('This function is under maintenance')]));
    }
    if ($SMARTSTUDY->site('openpix_api_key') == '') {
        die(json_encode(['status' => 'error', 'msg' => __('This function has not been configured')]));
    }
    if ($SMARTSTUDY->site('openpix_HMAC_key') == '') {
        die(json_encode(['status' => 'error', 'msg' => __('This function has not been configured')]));
    }
    if (empty($_POST['token'])) {
        die(json_encode(['status' => 'error', 'msg' => __('Please log in')]));
    ...
    if ($amount < $SMARTSTUDY->site('openpix_min')) {
        die(json_encode(['status' => 'error', 'msg' => __('Minimum deposit amount is ' . $SMARTSTUDY->site('openpix_min') . '')]));
    }
    if ($amount > $SMARTSTUDY->site('openpix_max')) {
        die(json_encode(['status' => 'error', 'msg' => __('Maximum deposit amount is ' . $SMARTSTUDY->site('openpix_max') . '')]));
    }
    $trans_id = random('QWERTYUIOPASDFGHJKLZXCVBNM', 3) . time();
    $price = $amount * $SMARTSTUDY->site('openpix_rate');
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_CUSTOMREQUEST => "POST",
        CURLOPT_HTTPHEADER => [
            "Authorization: " . $SMARTSTUDY->site('openpix_api_key'),
    ...
    if (($response && isset($response['charge']) && $response['charge']['status'] === 'ACTIVE') || ($response && isset($response['status']) && $response['status'] === 'ACTIVE')) {
        ...
        $isInsert = $SMARTSTUDY->insert('payment_openpix', array(
            'user_id'           => $getUser['id'],
            'trans_id'          => $trans_id,
            'price'             => $price,
            'amount'            => $amount,
...
if ($_POST['action'] == 'RechargeBakong') {
    ...
    if ($SMARTSTUDY->site('bakong_status') != 1) {
        die(json_encode(['status' => 'error', 'msg' => __('This function is under maintenance')]));
    }
    if (empty($_POST['token'])) {
        die(json_encode(['status' => 'error', 'msg' => __('Please log in')]));
    ...
    if ($amount < $SMARTSTUDY->site('bakong_min')) {
        die(json_encode(['status' => 'error', 'msg' => __('Minimum deposit amount is ' . $SMARTSTUDY->site('bakong_min') . '')]));
    }
    if ($amount > $SMARTSTUDY->site('bakong_max')) {
        die(json_encode(['status' => 'error', 'msg' => __('Maximum deposit amount is ' . $SMARTSTUDY->site('bakong_max') . '')]));
    }
    $trans_id = random('123456789', 4) . time();
    $price = $amount * $SMARTSTUDY->site('bakong_rate');
    ...
    if (isset($response)) {
        $isInsert = $SMARTSTUDY->insert('payment_bakong', array(
            'user_id'           => $getUser['id'],
            'trans_id'          => $trans_id,
            'price'             => $price,
            'amount'            => $amount,
...
if ($_POST['action'] == 'CreatePaymentpointAccount') {
    ...
    if ($SMARTSTUDY->site('paymentpoint_status') != 1) {
        die(json_encode(['status' => 'error', 'msg' => __('This function is under maintenance')]));
    }
    if ($SMARTSTUDY->site('paymentpoint_api_secret') == '' || $SMARTSTUDY->site('paymentpoint_api_key') == '') {
        die(json_encode(['status' => 'error', 'msg' => __('This function has not been configured')]));
    }
    if (empty($_POST['token'])) {
        die(json_encode(['status' => 'error', 'msg' => __('Please log in')]));
    ...
    $bankCodesStr = $SMARTSTUDY->site('paymentpoint_bank_codes');
    ...
    $params = [
        ...
        "phoneNumber" => $phone,
        "bankCode"    => $bankCodes,
        "businessId"  => $SMARTSTUDY->site('paymentpoint_business_id')
    ];

    // Gọi hàm tạo virtual account
    $apiSecret = $SMARTSTUDY->site('paymentpoint_api_secret');
    $apiKey = $SMARTSTUDY->site('paymentpoint_api_key');
    ...
    if ($response && isset($response['status']) && $response['status'] === 'success') {
        ...
        // Lưu vào database
        $isInsert = $SMARTSTUDY->insert('payment_paymentpoint', array(
            'user_id'               => $getUser['id'],
            'trans_id'              => $trans_id,
...
if ($_POST['action'] == 'CreateDsociopayAccount') {
    ...
    if ($SMARTSTUDY->site('dsociopay_status') != 1) {
        die(json_encode(['status' => 'error', 'msg' => __('This function is under maintenance')]));
    }
    if ($SMARTSTUDY->site('dsociopay_private_key') == '') {
        die(json_encode(['status' => 'error', 'msg' => __('This function has not been configured')]));
    }
    if (empty($_POST['token'])) {
        die(json_encode(['status' => 'error', 'msg' => __('Please log in')]));
    ...
    // Gọi hàm tạo virtual account
    $privateKey = $SMARTSTUDY->site('dsociopay_private_key');
    ...
    if ($response && isset($response['status']) && $response['status'] === 'success') {
        ...
        // Lưu vào database
        $isInsert = $SMARTSTUDY->insert('payment_dsociopay', array(
            'user_id'               => $getUser['id'],
            'trans_id'              => $trans_id,
            'price'                 => 0,
            'amount'                => 0,
...
if ($_POST['action'] == 'RechargeLempay') {
    ...
    if ($SMARTSTUDY->site('lempay_status') != 1) {
        die(json_encode(['status' => 'error', 'msg' => __('This function is under maintenance')]));
    }
    if ($SMARTSTUDY->site('lempay_pid') == '' || $SMARTSTUDY->site('lempay_key') == '') {
        die(json_encode(['status' => 'error', 'msg' => __('This function has not been configured')]));
    }
    if (empty($_POST['token'])) {
        die(json_encode(['status' => 'error', 'msg' => __('Please log in')]));
    ...
    // Check min/max
    if ($amount < $SMARTSTUDY->site('lempay_min')) {
        die(json_encode(['status' => 'error', 'msg' => __('Minimum deposit amount is ') . $SMARTSTUDY->site('lempay_min') . ' CNY']));
    }
    if ($amount > $SMARTSTUDY->site('lempay_max')) {
        die(json_encode(['status' => 'error', 'msg' => __('Maximum deposit amount is ') . $SMARTSTUDY->site('lempay_max') . ' CNY']));
    }

    // Get rate based on payment type
    $rateKey = 'lempay_rate_' . $paymentType;
    $rate = $SMARTSTUDY->site($rateKey);
    if (empty($rate) || $rate <= 0) {
        $rate = $SMARTSTUDY->site('lempay_rate'); // Fallback to default rate
    }
    ...
    // Cấu hình API
    $lempayConfig = [
        'pid'     => $SMARTSTUDY->site('lempay_pid'),
        'key'     => $SMARTSTUDY->site('lempay_key'),
        'api_url' => $SMARTSTUDY->site('lempay_api_url') ?: 'https://a119a.lempay.com'
    ];
    ...
    if ($response && isset($response['code']) && $response['code'] == 1) {
        ...
        $isInsert = $SMARTSTUDY->insert('payment_lempay', array(
            'user_id'    => $getUser['id'],
            'trans_id'   => $trans_id,
            'trade_no'   => $tradeNo,
            'type'       => $paymentType,
...
if ($_POST['action'] == 'RechargeZinipay') {
    ...
    if ($SMARTSTUDY->site('zinipay_status') != 1) {
        die(json_encode(['status' => 'error', 'msg' => __('This function is under maintenance')]));
    }
    if ($SMARTSTUDY->site('zinipay_api_key') == '') {
        die(json_encode(['status' => 'error', 'msg' => __('This function has not been configured')]));
    }
    if (empty($_POST['token'])) {
        die(json_encode(['status' => 'error', 'msg' => __('Please log in')]));
    ...
    // Check min/max
    if ($amount < $SMARTSTUDY->site('zinipay_min')) {
        die(json_encode(['status' => 'error', 'msg' => __('Minimum deposit amount is ') . $SMARTSTUDY->site('zinipay_min') . ' BDT']));
    }
    if ($amount > $SMARTSTUDY->site('zinipay_max')) {
        die(json_encode(['status' => 'error', 'msg' => __('Maximum deposit amount is ') . $SMARTSTUDY->site('zinipay_max') . ' BDT']));
    }

    $rate = $SMARTSTUDY->site('zinipay_rate');
    if (empty($rate) || $rate <= 0) {
        $rate = 1;
    }
    ...
    // Cấu hình API
    $zinipayConfig = [
        'api_key' => $SMARTSTUDY->site('zinipay_api_key'),
        'api_url' => $SMARTSTUDY->site('zinipay_api_url') ?: 'https://api.zinipay.com'
    ];
    ...
    $params = [
        ...
        'webhook_url'  => base_url('api/callback_zinipay.php?secret=' . $SMARTSTUDY->site('zinipay_callback_secret')),
    ...
    if ($response && isset($response['status']) && ($response['status'] === true || $response['status'] === 'true') && !empty($response['payment_url'])) {
        ...
        $isInsert = $SMARTSTUDY->insert('payment_zinipay', array(
            'user_id'    => $getUser['id'],
            'trans_id'   => $trans_id,
            'trade_no'   => $invoiceId,
            'price'      => $price,
...
if ($_POST['action'] == 'RechargeTripay') {
    ...
    if ($SMARTSTUDY->site('tripay_status') != 1) {
        die(json_encode(['status' => 'error', 'msg' => __('This function is under maintenance')]));
    }
    if ($SMARTSTUDY->site('tripay_api_key') == '' || $SMARTSTUDY->site('tripay_private_key') == '' || $SMARTSTUDY->site('tripay_merchant_code') == '') {
        die(json_encode(['status' => 'error', 'msg' => __('This function has not been configured')]));
    }
    if (empty($_POST['token'])) {
        die(json_encode(['status' => 'error', 'msg' => __('Please log in')]));
    ...
    // Check min/max
    if ($amount < $SMARTSTUDY->site('tripay_min')) {
        die(json_encode(['status' => 'error', 'msg' => __('Minimum deposit amount is ') . number_format($SMARTSTUDY->site('tripay_min')) . ' IDR']));
    }
    if ($amount > $SMARTSTUDY->site('tripay_max')) {
        die(json_encode(['status' => 'error', 'msg' => __('Maximum deposit amount is ') . number_format($SMARTSTUDY->site('tripay_max')) . ' IDR']));
    }
    ...
    $trans_id = random('QWERTYUIOPASDFGHJKLZXCVBNM', 3) . time();
    $price = $amount * $SMARTSTUDY->site('tripay_rate');

    require_once(__DIR__ . "/../../libs/tripay.php");

    // Cấu hình API
    $tripayConfig = [
        'api_key'       => $SMARTSTUDY->site('tripay_api_key'),
        'private_key'   => $SMARTSTUDY->site('tripay_private_key'),
        'merchant_code' => $SMARTSTUDY->site('tripay_merchant_code'),
        'sandbox'       => $SMARTSTUDY->site('tripay_sandbox') == 1
    ];
    ...
    if ($response && isset($response['success']) && $response['success'] === true) {
        ...
        $isInsert = $SMARTSTUDY->insert('payment_tripay', array(
            'user_id'      => $getUser['id'],
            'trans_id'     => $trans_id,
            'reference'    => $reference,
            'method'       => $paymentMethod,
