...
if (!$product = $SMARTSTUDY->get_row_safe(" SELECT * FROM `products` WHERE `id` = ? AND `status` = 1 ", [validate_int($_GET['id'])])) {
    die('<script type="text/javascript">if(!alert("' . __('Sản phảm không tồn tại') . '")){location.reload();}</script>');
}

// Khóa học không phụ thuộc kho mã; các sản phẩm còn lại giữ nguyên cơ chế tồn kho.
$isCourse = (($product['product_type'] ?? 'stock') === 'course');
$stock = $isCourse ? 1 : ($product['supplier_id'] != 0 ? $product['api_stock'] : getStock($product['code']));
?>
    ...
    <div class="row">
        <div class="col-md-6 col-lg-6">
            <div class="view-details">
                ...
                <div class="view-meta">
                    <p><label class="label-text feat"><?= $isCourse ? __('Hình thức:') : __('Kho hàng:'); ?>
                            <strong><?= $isCourse ? __('Học trực tuyến') : format_cash($stock); ?></strong></label>
                ...
                if (!isset($getUser) || $getUser['discount'] == 0): ?>
                    <?php if ($SMARTSTUDY->num_rows_safe(" SELECT * FROM product_discount WHERE product_id = ? ", [$product['id']]) > 0): ?>
                        <div class="mb-3 card-hot-deal">
                            <span><i class="fa-solid fa-fire-flame-simple" style="color:red;"></i> Hot Deal: </span><br>
                            <?php foreach ($SMARTSTUDY->get_list_safe(" SELECT * FROM product_discount WHERE product_id = ? ", [$product['id']]) as $product_discount): ?>
        ...
        <div class="col-md-6 col-lg-6">
            <div class="view-details">
                    ...
                    <tbody>
                        ...
                        <td>
                            <div class="product-action" style="display: flex;">
                                ...
                                <button class="action-minus1" title="Quantity Minus" <?= $isCourse ? 'disabled' : ''; ?>><i
                                        class="fa-solid fa-minus"></i></button>
                                <input class="action-input" onkeyup="totalPayment()" title="Quantity Number"
                                    type="number" id="amount" value="1" min="1" max="<?= $isCourse ? 1 : (int)$product['max']; ?>" <?= $isCourse ? 'readonly' : ''; ?>>
                                <button class="action-plus1" title="Quantity Plus" <?= $isCourse ? 'disabled' : ''; ?>><i
                        ...
                        <tr style="display: none;" id="into_vat_row">
                            <td><?= __('Thuế VAT'); ?> (<?= $SMARTSTUDY->site('tax_vat'); ?>%)</td>
                ...
                <div class="view-action-group">
                    ...
                    if (isset($getUser['id'])) {
                        $isButtonFavorite = $SMARTSTUDY->get_row_safe(" SELECT * FROM `favorites` WHERE `user_id` = ? AND `product_id` = ? ", [$getUser['id'], $product['id']]);
                    }
                    ?>
