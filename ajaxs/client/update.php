...
if($_POST['action'] == 'saveNoteOrder'){
    ...
    if (!$order = $SMARTSTUDY->get_row_safe("SELECT * FROM `product_order` WHERE `id` = ? AND `buyer` = ?", [$id, $getUser['id']])) {
        die(json_encode(['status' => 'error', 'msg' => __('Đơn hàng không tồn tại trong hệ thống')]));
    }
    ...
    $isUpdate = $SMARTSTUDY->update('product_order', [
        'note'  => $note
    ], " `id` = ?", [$order['id']]);
...
if($_POST['action'] == 'toggleFavorite'){
    ...
    if (!$product = $SMARTSTUDY->get_row_safe("SELECT * FROM `products` WHERE `id` = ? AND `status` = 1", [$id])) {
    ...
    if($isFavorite = $SMARTSTUDY->get_row_safe("SELECT * FROM `favorites` WHERE `user_id` = ? AND `product_id` = ?", [$getUser['id'], $product['id']])){
        $SMARTSTUDY->remove('favorites', " `id` = ?", [$isFavorite['id']]);
        die(json_encode([
            'status'    => 'success',
            'button'    => false,
...
if($_POST['action'] == 'confirmPaypal' && isset($_POST['order']) ){
    ...
    if ($SMARTSTUDY->site('paypal_status') != 1) {
    ...
    $clientId = $SMARTSTUDY->site('paypal_clientId');
    $clientSecret = $SMARTSTUDY->site('paypal_clientSecret');
    ...
    try {
        ...
        if ($SMARTSTUDY->get_row_safe("SELECT * FROM `payment_paypal` WHERE `trans_id` = ?", [$order->id])) {
            die(json_encode(['status' => 'error', 'msg' => __('Giao dịch này đã được xử lý')]));
        }
        $price = $SMARTSTUDY->site('paypal_rate') * $orderDetail->amount->value;
        $isInsert = $SMARTSTUDY->insert("payment_paypal", [
            'user_id'       => $getUser['id'],
            'trans_id'      => $order->id,
