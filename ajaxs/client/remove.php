...
if($_POST['action'] == 'removeOrder'){
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `product_order` WHERE `id` = ? AND `buyer` = ?", [$id, $getUser['id']])) {
        die(json_encode([
            'status'    => 'error',
    ...
    $isRemove = $SMARTSTUDY->update("product_order", [
        'trash' => 1
    ], " `id` = '".$row['id']."' ");
    if ($isRemove) {
...
if($_POST['action'] == 'removeFavorite'){
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `favorites` WHERE `id` = ? AND `user_id` = ?", [$id, $getUser['id']])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => __('Xóa dữ liệu thất bại')
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("favorites", " `id` = '".$row['id']."' ");
    if ($isRemove) {
        die(json_encode([
            'status'    => 'success',
