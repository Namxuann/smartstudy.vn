...
if ($_POST['action'] == 'Login') {
    ...
    $getUser = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `username` = ?", [$username]);
    if (!$getUser) {
        // Rate limit
    ...
    if ($SMARTSTUDY->site('type_password') == 'bcrypt') {
        if (!password_verify($password, $getUser['password'])) {
            // Rate limit
            checkBlockIP('LOGIN', 5);
            if ($getUser['login_attempts'] >= $SMARTSTUDY->site('limit_block_client_login')) {
                $User = new users();
            ...
            $SMARTSTUDY->cong('users', 'login_attempts', 1, " `id` = ? ", [$getUser['id']]);
            die(json_encode([
                'status'    => 'error',
                'msg'       => __('Thông tin đăng nhập không chính xác')
            ]));
    ...
    if ($getUser['status_otp_mail'] == 1) {
        ...
        if ($SMARTSTUDY->site('email_temp_subject_otp_mail') != '') {
            $content = $SMARTSTUDY->site('email_temp_content_otp_mail');
            $content = str_replace('{domain}', $_SERVER['SERVER_NAME'], $content);
            $content = str_replace('{title}', $SMARTSTUDY->site('title'), $content);
            ...
            ////////////////////////////////////////////////////////////////////
            $subject = $SMARTSTUDY->site('email_temp_subject_otp_mail');
            $subject = str_replace('{domain}', $_SERVER['SERVER_NAME'], $subject);
            $subject = str_replace('{title}', $SMARTSTUDY->site('title'), $subject);
    ...
    if ($getUser['status_noti_login_to_mail'] == 1 && $SMARTSTUDY->site('email_temp_subject_warning_login') != '') {
        try {
...
if ($_POST['action'] == 'Verify2FA') {
    ...
    $getUser = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `token_2fa` = ? AND `token_2fa` IS NOT NULL", [$token_2fa]);
    if (!$getUser) {
        // Rate limit
        checkBlockIP('2FA', 5);
    ...
    if ($google2fa->verifyKey($getUser['SecretKey_2fa'], $code, 2) != true) {
        ...
        $SMARTSTUDY->cong('users', 'limit_2fa', 1, " `id` = ? ", [$getUser['id']]);
        // Rate limit
        checkBlockIP('2FA', 5);
...
if ($_POST['action'] == 'VerifyOTP') {
    ...
    // Sử dụng prepared statement
    $getUser = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `token_otp_mail` = ? AND `token_otp_mail` IS NOT NULL", [$token_otp_mail]);
    if (!$getUser) {
        // Rate limit
        checkBlockIP('OTP', 15);
        die(json_encode(['status' => 'error', 'msg' => __('Thông tin đăng nhập không chính xác')]));
    ...
    if ($code != $getUser['otp_mail']) {
        ...
        $SMARTSTUDY->cong('users', 'limit_otp_mail', 1, " `id` = ? ", [$getUser['id']]);
        // Rate limit
        checkBlockIP('OTP', 15);
...
if ($_POST['action'] == 'Register') {
    ...
    if ($SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `email` = ?", [$email])) {
    ...
    if ($SMARTSTUDY->num_rows_safe("SELECT * FROM `users` WHERE `ip` = ?", [$user_ip]) >= $SMARTSTUDY->site('max_register_ip')) {
    ...
    // Thực hiện truy vấn
    $query = $SMARTSTUDY->query("SELECT COUNT(id) as total FROM `users`");
    ...
    $isCreate = $SMARTSTUDY->insert("users", [
        'admin'          => $role,
        'remember_token'    => $remember_token,
...
if ($_POST['action'] == 'ChangeProfile') {
    ...
    $getUser = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `token` = ?", [$token]);
    if (!$getUser) {
        die(json_encode(['status' => 'error', 'msg' => __('Vui lòng đăng nhập')]));
    }
    // Validate input data
...
if ($_POST['action'] == 'SavePrefixFullname') {
    ...
    $counter = 0;
    while ($SMARTSTUDY->get_row_safe("SELECT `id` FROM `users` WHERE `prefix_fullname` = ? AND `id` != ?", [$fullname, $getUser['id']])) {
        $counter++;
...
if ($_POST['action'] == 'ForgotPassword') {
    ...
    if (empty($SMARTSTUDY->site('smtp_password'))) {
    ...
    $getUser = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `email` = ?", [$email]);
    if (!$getUser) {
    ...
    if ($SMARTSTUDY->site('email_temp_subject_forgot_password') != '') {
        $content = $SMARTSTUDY->site('email_temp_content_forgot_password');
        $content = str_replace('{domain}', $_SERVER['SERVER_NAME'], $content);
        $content = str_replace('{title}', $SMARTSTUDY->site('title'), $content);
        $content = str_replace('{username}', $getUser['username'], $content);
        ...
        $content = str_replace('{time}', gettime(), $content);
        ////////////////////////////////////////////////////////////////////
        $subject = $SMARTSTUDY->site('email_temp_subject_forgot_password');
        $subject = str_replace('{domain}', $_SERVER['SERVER_NAME'], $subject);
        $subject = str_replace('{title}', $SMARTSTUDY->site('title'), $subject);
        $subject = str_replace('{username}', $getUser['username'], $subject);
...
if ($_POST['action'] == 'ChangePassword') {
    ...
    $getUser = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `token_forgot_password` = ? AND `token_forgot_password` IS NOT NULL", [$token]);
    if (!$getUser) {
        checkBlockIP('RESET_PASSWORD', 15);
        die(json_encode(['status' => 'error', 'msg' => __('Vui lòng đăng nhập')]));
    }
...
if ($_POST['action'] == 'changeAPIKey') {
    ...
    $getUser = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `token` = ? AND `banned` = 0", [$token]);
    if (!$getUser) {
        die(json_encode(['status' => 'error', 'msg' => __('Vui lòng đăng nhập để sử dụng tính năng này')]));
    }
...
if ($_POST['action'] == 'changeSecurity') {
    ...
    if ($SMARTSTUDY->site('smtp_status') != 1) {
