...
if ($SMARTSTUDY->site('status_demo') != 0) {
...
if ($_POST['action'] == 'removeAllAccountsSold') {
    ...
    $isRemove = $SMARTSTUDY->remove("product_sold", " `id` > 0 ");
    if ($isRemove) {
        $SMARTSTUDY->insert("logs", [
            'user_id'       => $getUser['id'],
            'ip'            => myip(),
...
if ($_POST['action'] == 'remove_payment_manual') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `payment_manual` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => __('Item does not exist in the system')
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("payment_manual", " `id` = ? ", [$id]);
    if ($isRemove) {
        // XÓA LOGO BANK
        unlink("../../" . $row['icon']);
        $SMARTSTUDY->insert("logs", [
...
if ($_POST['action'] == 'empty_all_list_die') {
    ...
    $isRemove = $SMARTSTUDY->remove("product_die", " `id` > 0 ");
    if ($isRemove) {
...
if ($_POST['action'] == 'removeTaskAutomation') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `automations` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => 'Dữ liệu không tồn tại trong hệ thống'
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("automations", " `id` = ? ", [$id]);
    if ($isRemove) {
        $SMARTSTUDY->insert("logs", [
            'user_id'       => $getUser['id'],
            'ip'            => myip(),
...
if ($_POST['action'] == 'removeAccountSold') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `product_sold` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => 'Tài khoản không tồn tại trong hệ thống'
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("product_sold", " `id` = ? ", [$id]);
    if ($isRemove) {
        $SMARTSTUDY->insert("logs", [
            'user_id'       => $getUser['id'],
            'ip'            => myip(),
...
if ($_POST['action'] == 'empty_list_account_stock') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `product_stock` WHERE `product_code` = ? ", [$product_code])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => __('Kho hàng đang trống')
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("product_stock", " `product_code` = ? ", [$product_code]);
    if ($isRemove) {
        $SMARTSTUDY->insert("logs", [
            'user_id'       => $getUser['id'],
            'ip'            => myip(),
...
if ($_POST['action'] == 'empty_list_die') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `product_die` WHERE `product_code` = ? ", [$product_code])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => 'Không có dữ liệu cần xóa'
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("product_die", " `product_code` = ? ", [$product_code]);
    if ($isRemove) {
        $SMARTSTUDY->insert("logs", [
            'user_id'       => $getUser['id'],
            'ip'            => myip(),
...
if ($_POST['action'] == 'removeProductDiscount') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `product_discount` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => 'Dữ liệu không tồn tại'
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("product_discount", " `id` = ? ", [$id]);
    if ($isRemove) {
        $SMARTSTUDY->insert("logs", [
            'user_id'       => $getUser['id'],
            'ip'            => myip(),
...
if ($_POST['action'] == 'removeMultipleBlockIP') {
    ...
    foreach ($ids as $id) {
        ...
        if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `block_ip` WHERE `id` = ?", [$id])) {
            $errorCount++;
            continue;
        }
        ...
        // Tiến hành xóa
        if ($SMARTSTUDY->remove("block_ip", " `id` = ?", [$id])) {
            $removeCount++;
        } else {
            $errorCount++;
        }
...
if ($_POST['action'] == 'removeBlockIP') {
    ...
    $isRemove = $SMARTSTUDY->remove("block_ip", " `id` = ? ", [$id]);
    if ($isRemove) {
        $SMARTSTUDY->insert("logs", [
            'user_id'       => $getUser['id'],
            'ip'            => myip(),
...
if ($_POST['action'] == 'removePromotion') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `promotions` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => 'Dữ liệu không tồn tại'
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("promotions", " `id` = ? ", [$id]);
    if ($isRemove) {
        $SMARTSTUDY->insert("logs", [
            'user_id'       => $getUser['id'],
            'ip'            => myip(),
...
if ($_POST['action'] == 'removeCategoriesOnly') {
    ...
    if (!$supplier = $SMARTSTUDY->get_row_safe("SELECT * FROM `suppliers` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
    ...
    $categories = $SMARTSTUDY->get_list_safe("SELECT * FROM `categories` WHERE `supplier_id` = ? ", [$id]);
    ...
    foreach ($categories as $category) {
        ...
        $SMARTSTUDY->remove("categories", " `id` = ? ", [$category['id']]);
    }
...
if ($_POST['action'] == 'removeProductsOnly') {
    ...
    $productsCountRow = $SMARTSTUDY->get_row_safe("SELECT COUNT(`id`) AS `count` FROM `products` WHERE `supplier_id` = ? ", [$id]);
    $productsCount = isset($productsCountRow['count']) ? intval($productsCountRow['count']) : 0;

    $SMARTSTUDY->remove("products", " `supplier_id` = ? ", [$id]);
...
if ($_POST['action'] == 'removeCategoriesAndProducts') {
    ...
    $categoriesCount = $SMARTSTUDY->get_row_safe("SELECT COUNT(id) as count FROM `categories` WHERE `supplier_id` = ? ", [$id])['count'];
    $productsCount = $SMARTSTUDY->get_row_safe("SELECT COUNT(id) as count FROM `products` WHERE `supplier_id` = ? ", [$id])['count'];

    // Xóa sản phẩm API
    $SMARTSTUDY->remove("products", " `supplier_id` = ? ", [$id]);

    // Xóa chuyên mục API và icon
    foreach ($SMARTSTUDY->get_list_safe(" SELECT * FROM `categories` WHERE `supplier_id` = ? ", [$id]) as $category) {
        $imagePath = "../../" . $category['icon'];
        if (file_exists($imagePath)) {
...
if ($_POST['action'] == 'removeSupplier') {
    ...
    $productsCount = $SMARTSTUDY->num_rows_safe("SELECT * FROM `products` WHERE `supplier_id` = ? ", [$id]);
    $categoriesCount = count($categories);

    // Bắt đầu transaction
    $SMARTSTUDY->query("START TRANSACTION");

    try {
        // Bước 1: Xóa sản phẩm trước
        $SMARTSTUDY->remove("products", " `supplier_id` = ? ", [$supplier['id']]);
        ...
        // Bước 3: Xóa supplier
        $isRemove = $SMARTSTUDY->remove("suppliers", " `id` = ? ", [$id]);
        ...
        $remainingProducts = $SMARTSTUDY->num_rows_safe("SELECT * FROM `products` WHERE `supplier_id` = ? ", [$id]);
        $remainingCategories = $SMARTSTUDY->num_rows_safe("SELECT * FROM `categories` WHERE `supplier_id` = ? ", [$id]);
        $remainingSupplier = $SMARTSTUDY->get_row_safe("SELECT * FROM `suppliers` WHERE `id` = ? ", [$id]);
...
if ($_POST['action'] == 'removeOrder') {
    ...
    $isRemove = $SMARTSTUDY->remove("product_order", " `id` = ? ", [$id]);
    if ($isRemove) {
        $SMARTSTUDY->remove('product_sold', " `trans_id` = ? ", [$product_order['trans_id']]);
        $SMARTSTUDY->insert("logs", [
            'user_id'       => $getUser['id'],
            'ip'            => myip(),
            'device'        => getUserAgent(),
...
if ($_POST['action'] == 'bulkRemoveOrders') {
    ...
    foreach ($ids as $id) {
        ...
        // Tiến hành xóa
        if ($SMARTSTUDY->remove("product_order", " `id` = ?", [$id])) {
            $SMARTSTUDY->remove('product_sold', " `trans_id` = ? ", [$product_order['trans_id']]);
            $removeCount++;
        } else {
            $errorCount++;
...
if ($_POST['action'] == 'removeMenu') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `menu` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => 'Dữ liệu không tồn tại'
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("menu", " `id` = ? ", [$id]);
    if ($isRemove) {
        $SMARTSTUDY->insert("logs", [
            'user_id'       => $getUser['id'],
            'ip'            => myip(),
...
if ($_POST['action'] == 'removeRole') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `admin_role` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => 'Dữ liệu không tồn tại'
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("admin_role", " `id` = ? ", [$id]);
    if ($isRemove) {
...
if ($_POST['action'] == 'removeCounpon') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `coupons` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => 'Dữ liệu không tồn tại'
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("coupons", " `id` = ? ", [$id]);
    if ($isRemove) {
...
if ($_POST['action'] == 'removeAccountStock') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `product_stock` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => 'Tài khoản không tồn tại trong hệ thống'
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("product_stock", " `id` = ? ", [$id]);
    if ($isRemove) {
...
if ($_POST['action'] == 'removeImageProduct') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `products` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
...
if ($_POST['action'] == 'removeProduct') {
    ...
    if (!$product = $SMARTSTUDY->get_row_safe("SELECT * FROM `products` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => 'ID sản phẩm không tồn tại trong hệ thống'
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("products", " `id` = ? ", [$id]);
    if ($isRemove) {
...
if ($_POST['action'] == 'removeCategory') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `categories` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => 'ID chuyên mục không tồn tại trong hệ thống'
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("categories", " `id` = ? ", [$id]);
    if ($isRemove) {
        unlink("../../" . $row['icon']);
...
if ($_POST['action'] == 'cleanupCategories') {
    ...
    if ($cleanup_all) {
        $all_list = $SMARTSTUDY->get_list("SELECT `id`, `name`, `icon` FROM `categories`");
        ...
        if ($count > 0) {
            foreach ($all_list as $row) {
                ...
                $SMARTSTUDY->remove("categories", " `id` = ? ", [$row['id']]);
            }
            $total_deleted += $count;
        ...
        if ($cleanup_orphan) {
            $orphan_list = $SMARTSTUDY->get_list("SELECT `id`, `name`, `icon` FROM `categories` WHERE `parent_id` > 0 AND `parent_id` NOT IN (SELECT `id` FROM `categories`)");
        ...
        if ($cleanup_empty) {
            ...
            $empty_list = $SMARTSTUDY->get_list("
...
if ($_POST['action'] == 'removeCategoryBlog') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `post_category` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => __('ID chuyên mục không tồn tại trong hệ thống')
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("post_category", " `id` = ? ", [$id]);
    if ($isRemove) {
        unlink("../../" . $row['icon']);
...
if ($_POST['action'] == 'removePost') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `posts` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => __('Bài viết không tồn tại trong hệ thống')
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("posts", " `id` = ? ", [$id]);
    if ($isRemove) {
        unlink("../../" . $row['image']);
...
if ($_POST['action'] == 'removeBank') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `banks` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => __('Item does not exist in the system')
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("banks", " `id` = ? ", [$id]);
    if ($isRemove) {
        // XÓA LOGO BANK
        unlink("../../" . $row['image']);
...
if ($_POST['action'] == 'removeLanguage') {
    ...
    $row = $SMARTSTUDY->get_row_safe("SELECT * FROM `languages` WHERE `id` = ? ", [$id]);
    if (!$row) {
        $data = json_encode([
            'status'    => 'error',
            'msg'       => __('The ID to delete does not exist')
    ...
    $SMARTSTUDY->remove("translate", " `lang_id` = ? ", [$row['id']]);
    $isRemove = $SMARTSTUDY->remove("languages", " `id` = ? ", [$id]);
    if ($isRemove) {
        unlink("../../" . $row['image']);
...
if ($_POST['action'] == 'removeCurrency') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `currencies` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => __('Item does not exist in the system')
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("currencies", " `id` = ? ", [$id]);
    if ($isRemove) {
...
if ($_POST['action'] == 'removeWithdraw') {
    ...
    $row = $SMARTSTUDY->get_row_safe("SELECT * FROM `aff_withdraw` WHERE `id` = ? ", [$id]);
    if (!$row) {
        $data = json_encode([
            'status'    => 'error',
    ...
    $isRemove = $SMARTSTUDY->remove("aff_withdraw", " `id` = ? ", [$id]);
    if ($isRemove) {
...
if ($_POST['action'] == 'removeCtvWithdraw') {
    ...
    $row = $SMARTSTUDY->get_row_safe("SELECT * FROM `ctv_withdraw` WHERE `id` = ? ", [$id]);
    if (!$row) {
        $data = json_encode([
            'status'    => 'error',
            'msg'       => __('ID item không tồn tại trong hệ thống')
    ...
    $isRemove = $SMARTSTUDY->remove("ctv_withdraw", " `id` = ? ", [$id]);
    if ($isRemove) {
...
if ($_POST['action'] == 'removeCtvProduct') {
    ...
    if (!$product = $SMARTSTUDY->get_row_safe("SELECT * FROM `products` WHERE `id` = ?", [$product_id])) {
        die(json_encode(['status' => 'error', 'msg' => __('Sản phẩm không tồn tại trong hệ thống')]));
    }

    // Kiểm tra xem sản phẩm có thuộc về CTV không
    $ctv = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `id` = ? AND `ctv` = 1", [$product['user_id']]);
    if (!$ctv) {
        die(json_encode(['status' => 'error', 'msg' => __('Sản phẩm không thuộc về CTV')]));
    }

    // Kiểm tra xem sản phẩm có đơn hàng nào không
    $orderCount = $SMARTSTUDY->num_rows_safe("SELECT * FROM `product_order` WHERE `product_id` = ?", [$product_id]);
    if ($orderCount > 0) {
        die(json_encode(['status' => 'error', 'msg' => __('Không thể xóa sản phẩm đã có đơn hàng. Vui lòng từ chối thay vì xóa.')]));
    }

    // Xóa sản phẩm khỏi database
    $isDelete = $SMARTSTUDY->remove("products", " `id` = ?", [$product_id]);

    if ($isDelete) {
        // Xóa các dữ liệu liên quan
        $SMARTSTUDY->remove("product_stock", " `product_code` = ? ", [$product['code']]);
        $SMARTSTUDY->remove("product_die", " `product_code` = ? ", [$product['code']]);
        // $SMARTSTUDY->remove("product_sold", " `product_code` = '".$product['code']."' "); // Xóa tài khoản đã bán
...
if ($_POST['action'] == 'removeUser') {
    ...
    $row = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `id` = ? ", [$id]);
    if (!$row) {
        $data = json_encode([
            'status'    => 'error',
    ...
    $isRemove = $SMARTSTUDY->remove("users", " `id` = ? ", [$id]);
    if ($isRemove) {
...
if ($_POST['action'] == 'removeTranslate') {
    ...
    $row = $SMARTSTUDY->get_row_safe("SELECT * FROM `translate` WHERE `id` = ? ", [$id]);
    if (!$row) {
        $data = json_encode([
            'status'    => 'error',
            'msg'       => __('The ID to delete does not exist')
    ...
    // $isRemove = $SMARTSTUDY->remove("translate", " `value` = '".$row['value']."' ");
    $isRemove = $SMARTSTUDY->remove("translate", " `id` = ? ", [$id]);
    if ($isRemove) {
...
if ($_POST['action'] == 'email_campaigns') {
    ...
    if (!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `email_campaigns` WHERE `id` = ? ", [$id])) {
        die(json_encode([
            'status'    => 'error',
            'msg'       => __('Item không tồn tại trong hệ thống')
        ]));
    }
    $isRemove = $SMARTSTUDY->remove("email_campaigns", " `id` = ? ", [$id]);
    if ($isRemove) {
        $SMARTSTUDY->remove('email_sending', " `camp_id` = ? ", [$row['id']]);
...
if ($_POST['action'] == 'bulk_remove_categories') {
    ...
    foreach ($category_ids as $id) {
        if ($id > 0) {
            ...
            if ($row['parent_id'] == 0) {
                if ($SMARTSTUDY->num_rows_safe("SELECT * FROM `categories` WHERE `parent_id` = ?", [$id]) != 0) {
                    $error_count++;
...
if ($_POST['action'] == 'cleanupTelegramLogs') {
    ...
    $count_before = $SMARTSTUDY->num_rows_safe("SELECT * FROM `telegram_logs` WHERE `time` < ? ", [$cutoff_date]);
    ...
    $isRemove = $SMARTSTUDY->remove("telegram_logs", " `time` < ? ", [$cutoff_date]);

    if ($isRemove) {
        // Đếm lại số lượng log sau khi xóa
        $count_after = $SMARTSTUDY->num_rows_safe("SELECT * FROM `telegram_logs` WHERE `time` < ? ", [$cutoff_date]);
...
if ($_POST['action'] == 'cleanupBankLogs') {
    ...
    // Đếm số lượng log sẽ bị xóa
    $count_before = $SMARTSTUDY->num_rows_safe("SELECT * FROM `log_bank_auto` WHERE `create_gettime` < ? ", [$cutoff_date]);
    ...
    // Xóa các log cũ hơn số ngày được chỉ định
    $isRemove = $SMARTSTUDY->remove("log_bank_auto", " `create_gettime` < ? ", [$cutoff_date]);

    if ($isRemove) {
        // Đếm lại số lượng log sau khi xóa
        $count_after = $SMARTSTUDY->num_rows_safe("SELECT * FROM `log_bank_auto` WHERE `create_gettime` < ? ", [$cutoff_date]);
        $deleted_count = $count_before - $count_after;
...
if ($_POST['action'] == 'cleanupRechargeBank') {
    ...
    $count_before = $SMARTSTUDY->num_rows_safe("SELECT * FROM `payment_bank` WHERE `create_gettime` < ? ", [$cutoff_date]);
    ...
    $isRemove = $SMARTSTUDY->remove("payment_bank", " `create_gettime` < ? ", [$cutoff_date]);

    if ($isRemove) {
        $count_after = $SMARTSTUDY->num_rows_safe("SELECT * FROM `payment_bank` WHERE `create_gettime` < ? ", [$cutoff_date]);
        $deleted_count = $count_before - $count_after;
...
if ($_POST['action'] == 'cleanupRechargeCard') {
    ...
    // Đếm số lượng sẽ bị xóa
    $count_before = $SMARTSTUDY->num_rows_safe("SELECT * FROM `cards` WHERE `create_date` < ? ", [$cutoff_date]);
    ...
    // Xóa các bản ghi cũ hơn số ngày được chỉ định
    $isRemove = $SMARTSTUDY->remove("cards", " `create_date` < ? ", [$cutoff_date]);

    if ($isRemove) {
        $count_after = $SMARTSTUDY->num_rows_safe("SELECT * FROM `cards` WHERE `create_date` < ? ", [$cutoff_date]);
        $deleted_count = $count_before - $count_after;
...
if ($_POST['action'] == 'cleanupRechargeCrypto') {
    ...
    // Đếm số lượng sẽ bị xóa
    $count_before = $SMARTSTUDY->num_rows_safe("SELECT * FROM `payment_crypto` WHERE `create_gettime` < ? ", [$cutoff_date]);
    ...
    // Xóa các bản ghi cũ hơn số ngày được chỉ định
    $isRemove = $SMARTSTUDY->remove("payment_crypto", " `create_gettime` < ? ", [$cutoff_date]);

    if ($isRemove) {
        $count_after = $SMARTSTUDY->num_rows_safe("SELECT * FROM `payment_crypto` WHERE `create_gettime` < ? ", [$cutoff_date]);
        $deleted_count = $count_before - $count_after;
...
if ($_POST['action'] == 'cleanupTransactions') {
    ...
    // Đếm số lượng log sẽ bị xóa
    $count_before = $SMARTSTUDY->num_rows_safe("SELECT * FROM `dongtien` WHERE `thoigian` < ? ", [$cutoff_date]);
    ...
    // Xóa các log cũ hơn số ngày được chỉ định
    $isRemove = $SMARTSTUDY->remove("dongtien", " `thoigian` < ? ", [$cutoff_date]);

    if ($isRemove) {
        // Đếm lại số lượng log sau khi xóa
        $count_after = $SMARTSTUDY->num_rows_safe("SELECT * FROM `dongtien` WHERE `thoigian` < ? ", [$cutoff_date]);
        $deleted_count = $count_before - $count_after;
...
if ($_POST['action'] == 'cleanupLogs') {
    ...
    // Đếm số lượng log sẽ bị xóa
    $count_before = $SMARTSTUDY->num_rows_safe("SELECT * FROM `logs` WHERE `createdate` < ? ", [$cutoff_date]);
    ...
    // Xóa các log cũ hơn số ngày được chỉ định
    $isRemove = $SMARTSTUDY->remove("logs", " `createdate` < ? ", [$cutoff_date]);

    if ($isRemove) {
        // Đếm lại số lượng log sau khi xóa
        $count_after = $SMARTSTUDY->num_rows_safe("SELECT * FROM `logs` WHERE `createdate` < ? ", [$cutoff_date]);
        $deleted_count = $count_before - $count_after;
...
if ($_POST['action'] == 'cleanupOrders') {
    ...
    switch ($cleanup_type) {
        case 'delete_order_revenue':
            ...
            $SMARTSTUDY->remove('product_order', " " . time() . " - UNIX_TIMESTAMP(create_gettime) >= " . $schedule . " ");
            $SMARTSTUDY->remove('product_sold', " " . time() . " - UNIX_TIMESTAMP(create_gettime) >= " . $schedule . " ");
        ...
        case 'delete_order_only':
            ...
            $orders_count = $SMARTSTUDY->num_rows(" SELECT * FROM `product_order` WHERE " . time() . " - UNIX_TIMESTAMP(create_gettime) >= " . $schedule . " AND `trash` = 0 ");

            $SMARTSTUDY->update('product_order', [
                'trash' => 1
        ...
        case 'delete_order_not_uid':
            ...
            $SMARTSTUDY->update('product_sold', [
...
if ($_POST['action'] == 'cleanupEmailQueue') {
    ...
    if ($days == 0) {
        ...
        $SMARTSTUDY->remove("email_queue", " 1 ");
    ...
    $SMARTSTUDY->remove("email_queue", " `created_at` < ? ", [$cutoff]);
...
if ($_POST['action'] == 'cleanupTelegramQueue') {
    ...
    if ($days == 0) {
        ...
        $SMARTSTUDY->remove("telegram_queue", " 1 ");
    ...
    $SMARTSTUDY->remove("telegram_queue", " `created_at` < ? ", [$cutoff]);
