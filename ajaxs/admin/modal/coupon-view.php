...
if (!$getUser = $SMARTSTUDY->get_row("SELECT * FROM `users` WHERE `token` = '".check_string($_GET['token'])."' AND `banned` = 0 AND `admin` != 0 ")) {
    die('<script type="text/javascript">if(!alert("'.__('Please log in').'")){location.href=`' . base_url_admin('affiliate-withdraw') . '`;}</script>');
}
...
if(!$row = $SMARTSTUDY->get_row(" SELECT * FROM `coupons` WHERE `id` = '".check_string($_GET['id'])."'  ")){
...
if (isset($_POST['btnSubmit'])) {
    ...
    $isUpdate = $SMARTSTUDY->update("aff_withdraw", [
...
<form action="<?=BASE_URL('ajaxs/admin/modal/coupon-view.php?id='.$row['id'].'&token='.$getUser['token']);?>"
    ...
    <div class="modal-body">
        <table id="datatable-basic" class="table text-nowrap table-striped table-hover table-bordered"
            ...
            <tbody>
                <?php $i=0; foreach ($SMARTSTUDY->get_list("SELECT * FROM `coupon_used` WHERE `coupon_id` = '$id' ORDER BY `id` DESC ") as $row) {?>
                <tr>
                    <td><?=$i++;?></td>
