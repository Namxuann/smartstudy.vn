...
if ($_POST['action'] == 'view_product_live') {
    ...
    $accounts = $SMARTSTUDY->get_list_safe("SELECT * FROM `product_stock` WHERE `product_code` = ? ORDER BY `id` DESC", [$code]);
...
if ($_POST['action'] == 'view_product_die') {
    ...
    $accounts = $SMARTSTUDY->get_list_safe("SELECT * FROM `product_die` WHERE `product_code` = ? ORDER BY `id` DESC", [$code]);
...
if ($_POST['action'] == 'view_order') {
    ...
    $order = $SMARTSTUDY->get_row_safe("SELECT * FROM `product_order` WHERE `trans_id` = ? AND `seller` = ?", [$trans_id, $getUser['id']]);
    if (!$order) {
        die(json_encode(['status' => 'error', 'msg' => __('Bạn không có quyền xem đơn hàng này!')]));
    }
    
    // Get sold accounts for this order
    $accounts = $SMARTSTUDY->get_list_safe("SELECT * FROM `product_sold` WHERE `trans_id` = ? ORDER BY `id` DESC", [$trans_id]);
...
if($_POST['action'] == 'tinh_tien_refund'){
    ...
    if(!$product_order = $SMARTSTUDY->get_row_safe("SELECT * FROM `product_order` WHERE `id` = ? AND `seller` = ?", [$id, $getUser['id']])){
...
if($_POST['action'] == 'calculateWithdrawFee'){
    ...
    $feePercent = $SMARTSTUDY->site('ctv_fee_withdraw');
