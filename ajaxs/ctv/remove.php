...
if($SMARTSTUDY->site('ctv_status') != 1){
...
if ($_POST['action'] == 'deleteProduct') {
    ...
    $product = $SMARTSTUDY->get_row_safe("SELECT * FROM `products` WHERE `id` = ? AND `user_id` = ?", [$product_id, $getUser['id']]);
    if (!$product) {
    ...
    $stock_count = $SMARTSTUDY->get_row_safe("SELECT COUNT(*) as total FROM `product_stock` WHERE `product_id` = ?", [$product_id])['total'];
    if ($stock_count > 0) {
...
if ($_POST['action'] == 'removeAccountStock') {
    ...
    $account = $SMARTSTUDY->get_row_safe("SELECT * FROM `product_stock` WHERE `id` = ? AND `seller` = ?", [$account_id, $getUser['id']]);
    if (!$account) {
        die(json_encode(['status' => 'error', 'msg' => __('Tài khoản không tồn tại hoặc bạn không có quyền xóa!')]));
    }
    
    // Remove account from stock
    $isRemove = $SMARTSTUDY->remove('product_stock', "`id` = ?", [$account_id]);
...
if ($_POST['action'] == 'empty_list_die') {
    ...
    $product = $SMARTSTUDY->get_row_safe("SELECT * FROM `products` WHERE `code` = ? AND `user_id` = ?", [$code, $getUser['id']]);
    if (!$product) {
        die(json_encode(['status' => 'error', 'msg' => __('Bạn không có quyền truy cập kho hàng này!')]));
    }
    
    // Count die accounts before deletion
    $die_count = $SMARTSTUDY->get_row_safe("SELECT COUNT(*) as total FROM `product_die` WHERE `product_code` = ?", [$code])['total'];
    
    // Remove all die accounts for this product
    $isRemove = $SMARTSTUDY->remove('product_die', "`product_code` = ?", [$code]);
...
if ($_POST['action'] == 'empty_list_account_stock') {
    ...
    $live_count = $SMARTSTUDY->get_row_safe("SELECT COUNT(*) as total FROM `product_stock` WHERE `product_code` = ?", [$code])['total'];
    
    // Remove all live accounts for this product
    $isRemove = $SMARTSTUDY->remove('product_stock', "`product_code` = ?", [$code]);
