...
if($_POST['action'] == 'CtvWithdraw'){
    ...
    $allowedBanks = explode(PHP_EOL, $SMARTSTUDY->site('ctv_banks_withdraw'));
    ...
    if($amount < $SMARTSTUDY->site('ctv_min_withdraw')){
        die(json_encode(['status' => 'error', 'msg' => __('Số tiền rút tối thiểu là').' '.format_currency($SMARTSTUDY->site('ctv_min_withdraw'))]));
    }
    ...
    // Tạo mã giao dịch
    $trans_id = $SMARTSTUDY->site('ctv_prefix_withdraw') . time() . rand(1000, 9999);
    
    // Kiểm tra mã giao dịch trùng lặp
    while($SMARTSTUDY->get_row_safe("SELECT * FROM `ctv_withdraw` WHERE `trans_id` = ?", [$trans_id])){
        $trans_id = $SMARTSTUDY->site('ctv_prefix_withdraw') . time() . rand(1000, 9999);
    }
    ...
    $insert = $SMARTSTUDY->insert("ctv_withdraw", [
        'trans_id'  => $trans_id,
        'user_id'   => $getUser['id'],
        'bank'      => $bank,
        'stk'       => $stk,
