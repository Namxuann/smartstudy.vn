...
$body = [
    'title' => __('Recharge PaymentPoint') . ' | ' . $SMARTSTUDY->site('title'),
...
$existingAccount = $SMARTSTUDY->get_row_safe(
        ...
        <div class="row">
            ...
            <div class="col-md-5">
                <div class="account-card">
                    ...
                    <?= $SMARTSTUDY->site('paymentpoint_notice'); ?>

                    <div class="mt-3">
                        <h6><?= __('Tỷ giá quy đổi:'); ?></h6>
                        <p><strong>1 <?= $SMARTSTUDY->site('paymentpoint_currency_code'); ?> = <?= format_currency($SMARTSTUDY->site('paymentpoint_rate')); ?></strong></p>
                    </div>
                </div>
            </div>
            <div class="col-md-12">
                            ...
                            <tbody>
                                <?php foreach ($listDatatable as $row) { ?>
                                    <tr>
                                        ...
                                        <td class="text-center"><b><?= format_cash($row['amount']); ?></b> <?= $SMARTSTUDY->site('paymentpoint_currency_code'); ?></td>
                                ...
                                <tr>
                                    <td colspan="6">
                                        <div class="float-right">
                                            <?= __('Paid:'); ?>
                                            <strong
                                                style="color:red;"><?= format_currency($SMARTSTUDY->get_row_safe(" SELECT SUM(`price`) FROM `payment_paymentpoint` WHERE $where_clause AND `status` = 1 ", $where_params)['SUM(`price`)']); ?></strong>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p class="mt-3 mb-0"><strong><?= __('Tỷ giá:'); ?></strong> 1 <?= $SMARTSTUDY->site('paymentpoint_currency_code'); ?> = <?= format_currency($SMARTSTUDY->site('paymentpoint_rate')); ?></p>
            </div>
