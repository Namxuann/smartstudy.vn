...
$body = [
    'title' => __('Nạp tiền bằng ví THESIEURE').' | '.$SMARTSTUDY->site('title'),
        ...
        <div class="row">
            ...
            <div class="col-lg-7">
                ...
                <div class="account-card p-3">
                    <?=$SMARTSTUDY->site('thesieure_notice');?>
                </div>
            </div>
            <div class="col-lg-5">
                <div class="account-card">
                    ...
                    <ul class="list-group">
                        <li class="list-group-item"><?=__('Số điện thoại:');?> <b id="copySTK"
                                style="color: green;"><?=$SMARTSTUDY->site('thesieure_number');?></b> <button
                                onclick="copy()" class="copy" data-clipboard-target="#copySTK"><i
                                    class="fas fa-copy"></i></button>
                        </li>
                        <li class="list-group-item"><?=__('Địa chỉ Eamil:');?> <b id="copyEmail"
                                style="color: green;"><?=$SMARTSTUDY->site('thesieure_email');?></b> <button onclick="copy()"
                        ...
                        <li class="list-group-item"><?=__('Chủ tài khoản:');?>
                            <b><?=$SMARTSTUDY->site('thesieure_name');?></b>
                        </li>
                    </ul>
                                ...
                                <tr>
                                    <td colspan="7">
                                        <div class="float-right">
                                            <?=__('Đã thanh toán:');?>
                                            <strong
                                                style="color:red;"><?=format_currency($SMARTSTUDY->get_row_safe("SELECT SUM(`amount`) FROM `payment_thesieure` WHERE $where_clause", $where_params)['SUM(`amount`)']);?></strong>
                                            |

                                            <?=__('Thực nhận:');?>
                                            <strong
                                                style="color:blue;"><?=format_currency($SMARTSTUDY->get_row_safe("SELECT SUM(`received`) FROM `payment_thesieure` WHERE $where_clause", $where_params)['SUM(`received`)']);?></strong>
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
