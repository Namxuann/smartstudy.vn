...
$body = [
    'title' => __('Xác minh OTP').' | '.$SMARTSTUDY->site('title'),
...
if (isset($_GET['token'])) {
    ...
    if($token === false || !$getUser = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `token_otp_mail` = ? AND `token_otp_mail` IS NOT NULL", [$token])) {
