...
$body = [
    'title' => __('Nạp tiền bằng ví MOMO').' | '.$SMARTSTUDY->site('title'),
...
<section class="py-5 inner-section profile-part">
    <div class="container">
        <div class="row">
            <?php if($SMARTSTUDY->num_rows(" SELECT * FROM `promotions` ") != 0):?>
            <div class="col-lg-12">
                ...
                <div class="account-card p-0">
                    <table class="table fs-sm mb-0">
                        ...
                        <tbody>
                            <?php $i=1;foreach($SMARTSTUDY->get_list_safe(" SELECT * FROM `promotions` ORDER BY `min` DESC ", []) as $promotion):?>
                            <tr>
            ...
            <div class="col-lg-5">
                <div class="account-card">
                    <center class="py-3">
                        ...
                        <?php if($SMARTSTUDY->site('momo_number') != ''):?>
                        <?=file_get_contents("https://api.web2m.com/api/qrmomo.php?amount=10000&phone=".$SMARTSTUDY->site('momo_number')."&noidung=".urlencode($SMARTSTUDY->site('prefix_autobank')).$getUser['id']."&size=300");?>
                        <?php endif?>
                    </center>
                    <ul class="list-group">
                        <li class="list-group-item"><?=__('Số điện thoại:');?> <b id="copySTK"
                                style="color: green;"><?=$SMARTSTUDY->site('momo_number');?></b> <button onclick="copy()"
                                class="copy" data-clipboard-target="#copySTK"><i
                                    class="fas fa-copy"></i></button>
                        </li>
                        <li class="list-group-item" style="font-size:17px;"><?=__('Nội dung chuyển khoản:');?> <b
                                id="copyNoiDung"
                                style="color: red;"><?=$SMARTSTUDY->site('prefix_autobank').$getUser['id'];?></b>
                                ...
                                <tr>
                                    <td colspan="7">
                                        <div class="float-right">
                                            <?=__('Đã thanh toán:');?>
                                            <strong
                                                style="color:red;"><?=format_currency($SMARTSTUDY->get_row_safe(" SELECT SUM(`amount`) FROM `payment_momo` WHERE $where_clause ", $where_params)['SUM(`amount`)']);?></strong>
                                            |

                                            <?=__('Thực nhận:');?>
                                            <strong
                                                style="color:blue;"><?=format_currency($SMARTSTUDY->get_row_safe(" SELECT SUM(`received`) FROM `payment_momo` WHERE $where_clause ", $where_params)['SUM(`received`)']);?></strong>
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
