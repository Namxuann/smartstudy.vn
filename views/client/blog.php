...
if(isset($_GET['slug'])){
    ...
    if(!$row = $SMARTSTUDY->get_row_safe("SELECT * FROM `posts` WHERE `slug` = ?", [$slug])){
        redirect(base_url('blogs'));
    }
    // ✅ Sử dụng prepared statement cho cập nhật view
    $SMARTSTUDY->update('posts', ['view' => $row['view'] + 1], "`id` = ?", [$row['id']]);
...
$body = [
    'title' => __($row['title']).' | '.$SMARTSTUDY->site('title'),
