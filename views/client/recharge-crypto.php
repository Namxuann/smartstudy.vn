...
$body = [
    'title' => __('Nạp tiền bằng Crypto') . ' | ' . $SMARTSTUDY->site('title'),
...
$totalResult = $SMARTSTUDY->get_row_safe($count_sql, $where_params);
...
<section class="py-5 inner-section profile-part">
    <div class="container">
        <div class="row">
            <?php
            $cryptoPromotions = parseCryptoPromotionsConfig($SMARTSTUDY->site('crypto_promotions'));
            ?>
            ...
            <div class="col-md-7">
                <div class="crypto-form-card mb-3">
                    <div class="crypto-form-inner">
                        ...
                        <input type="hidden" id="crypto_rate" value="<?= $SMARTSTUDY->site('crypto_rate'); ?>">
                            ...
                            <div class="crypto-rate-box">
                                ...
                                <div style="flex: 1; text-align: center;">
                                    ...
                                    <p class="crypto-rate-value">
                                        1 USDT = <?= format_currency($SMARTSTUDY->site('crypto_rate')); ?>
                                    </p>
                                </div>
                                <div>
                                ...
                                <tr>
                                    <td colspan="8">
                                        <div class="float-right">
                                            ...
                                            $completed_total = $SMARTSTUDY->get_row_safe("SELECT SUM(`received`) as total FROM `payment_crypto` WHERE $completed_where", $completed_params);

                                            // Tính tổng chưa thanh toán  
                                            $waiting_params = array_merge($where_params, ['waiting']);
                                            $waiting_where = $where_clause . ' AND `status` = ?';
                                            $waiting_total = $SMARTSTUDY->get_row_safe("SELECT SUM(`received`) as total FROM `payment_crypto` WHERE $waiting_where", $waiting_params);
                                            ?>
                                            <?= __('Đã thanh toán:'); ?>
...
<script type="text/javascript">
    ...
    $("#CreateInvoiceCrypto").on("click", function() {
        ...
        $.ajax({
            ...
            dataType: "JSON",
            data: {
                action: '<?= $SMARTSTUDY->site('crypto_type_api') == 'fpayment.net' ? 'RechargeCryptoNew' : 'RechargeCrypto'; ?>',
                token: $("#token").val(),
                amount: $("#amount").val()
            },
