...
$body = [
    'title' => __('Recharge Korapay').' | '.$SMARTSTUDY->site('title'),
...
<section class="py-5 inner-section profile-part">
    <div class="container">
        <div class="row">
            <?php
            // Lấy danh sách mốc khuyến mãi nạp tiền Korapay (nếu có)
            $korapayPromotions = parseCryptoPromotionsConfig($SMARTSTUDY->site('korapay_promotions'));
            ?>
            <?php if (!empty($korapayPromotions)): ?>
                <div class="col-lg-12">
                    <div class="account-card mb-3">
                        <h4 class="account-title"><i class="fa-solid fa-percent me-2"></i> <?=__('Khuyến mãi nạp tiền');?></h4>
                            ...
                            <table class="table fs-sm mb-0">
                                ...
                                <tbody>
                                    <?php foreach ($korapayPromotions as $promotion):
                                        // Format số tiền mốc nạp (bỏ số 0 thừa ở cuối)
                                    ...
                                    ?>
                                        <tr>
                                            <td><b style="color: blue;"><?=$minFormatted;?></b> <strong><?=$SMARTSTUDY->site('korapay_currency_code');?></strong></td>
                ...
                <div class="account-card">
                    ...
                    <div class="row mb-3">
                        <label class="col-sm-4 col-form-label"
                            for="example-hf-email"><?=__('Enter the deposit amount: ('.$SMARTSTUDY->site('korapay_currency_code').')');?></label>
                        <div class="col-sm-8">
            ...
            <div class="col-md-5">
                <div class="account-card">
                    ...
                    <?=$SMARTSTUDY->site('korapay_notice');?>
                </div>
            </div>
            <div class="col-md-12">
                <div class="account-card">
                                ...
                                <tr>
                                    <td colspan="7">
                                        <div class="float-right">
                                            <?=__('Paid:');?>
                                            <strong
                                                style="color:red;"><?=format_currency($SMARTSTUDY->get_row_safe(" SELECT SUM(`price`) FROM `payment_korapay` WHERE $where_clause AND `status` = 1 ", $where_params)['SUM(`price`)']);?></strong>
