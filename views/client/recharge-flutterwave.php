...
$body = [
    'title' => __('Recharge Flutterwave').' | '.$SMARTSTUDY->site('title'),
...
if($SMARTSTUDY->site('flutterwave_status') != 1){
    redirect(base_url());
}
...
if ($SMARTSTUDY->get_row_safe("SELECT * FROM `payment_flutterwave` WHERE `user_id` = ? AND `status` = 'pending'", [$getUser['id']])) {
    $tx_ref = $SMARTSTUDY->get_row_safe("SELECT * FROM `payment_flutterwave` WHERE `user_id` = ? AND `status` = 'pending'", [$getUser['id']])['tx_ref'];
} else {
    $tx_ref = md5(random('QWERTYUIOPASDFGHJKLZXCVBNM', 4).'_'.time());
    $SMARTSTUDY->insert("payment_flutterwave", [
        'user_id'       => $getUser['id'],
        'tx_ref'        => $tx_ref,
        'amount'        => 0,
        'currency'      => $SMARTSTUDY->site('flutterwave_currency_code'),
        ...
        <div class="row">
                ...
                <div class="account-card">
                    ...
                    <div class="row mb-3">
                        <label class="col-sm-4 col-form-label"
                            for="example-hf-email"><?=__('Enter the deposit amount: ('.$SMARTSTUDY->site('flutterwave_currency_code').')');?></label>
                        <div class="col-sm-8">
                            <input type="hidden" name="public_key" value="<?=$SMARTSTUDY->site('flutterwave_publicKey');?>" />
                            ...
                            <input type="hidden" name="currency" value="<?=$SMARTSTUDY->site('flutterwave_currency_code');?>" />
            ...
            <div class="col-md-5">
                <div class="account-card">
                    ...
                    <?=$SMARTSTUDY->site('flutterwave_notice');?>
                </div>
            </div>
            <div class="col-md-12">
                <div class="account-card">
...
function SquadPay() {
    ...
    const squadInstance = new squad({
        ...
        key: "<?=$SMARTSTUDY->site('squadco_Public_Key');?>",
