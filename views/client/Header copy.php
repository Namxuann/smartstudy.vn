...
<head>
    ...
    <title><?=isset($body['title']) ? $body['title'] : $SMARTSTUDY->site('title');?></title>
    <meta name="description" content="<?=isset($body['desc']) ? $body['desc'] : $SMARTSTUDY->site('description');?>" />
    <meta name="keywords" content="<?=isset($body['keyword']) ? $body['keyword'] : $SMARTSTUDY->site('keywords');?>">
    <meta name="copyright" content="<?=$SMARTSTUDY->site('author');?>" />
    <meta name="author" content="<?=$SMARTSTUDY->site('author');?>" />
    <meta property="og:url" content="<?=base_url('');?>">
    <meta property="og:site_name" content="<?=base_url();?>" />
    <meta property="og:title" content="<?=$body['title'];?>" />
    <meta property="og:type" content="website" />
    <meta property="og:image"
        content="<?=isset($body['image']) ? $body['image'] : BASE_URL($SMARTSTUDY->site('image'));?>" />
    ...
    <link rel="icon" type="image/png" href="<?=BASE_URL($SMARTSTUDY->site('favicon'));?>" />
    <style>
    :root {
        --primary: <?=$SMARTSTUDY->site('theme_color');
        ?>;
    }
    </style>
    <style>
    :root {
        --primary1: <?=$SMARTSTUDY->site('theme_color1');
        ?>;
    }
    </style>
    ...
    <?php if($SMARTSTUDY->site('google_analytics_status') == 1):?>
    <!-- Google tag (gtag.js) -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=<?=$SMARTSTUDY->site('google_analytics_id');?>"></script>
    <script>
    window.dataLayer = window.dataLayer || [];
    function gtag(){dataLayer.push(arguments);}
    gtag('js', new Date());

    gtag('config', '<?=$SMARTSTUDY->site('google_analytics_id');?>');
    </script>
    <?php endif?>
    <?php if($SMARTSTUDY->site('reCAPTCHA_status') == 1):?>
    <!-- reCaptcha -->
    <script src="https://www.google.com/recaptcha/api.js" async defer></script>
    <?php endif?>
    <?=$body['header'];?>
    <link rel="stylesheet" href="<?=BASE_URL('mod/css/main.css?v=19');?>">
    <script src="<?=base_url('mod/js/main.js');?>"></script> 
    <?=$SMARTSTUDY->site('javascript_header');?>
