...
$body = [
    'title' => __('Recharge Tmweasyapi Thailand').' | '.$SMARTSTUDY->site('title'),
        ...
        <div class="row">
            ...
            <div class="col-md-5">
                <div class="account-card">
                    ...
                    <?=$SMARTSTUDY->site('tmweasyapi_notice');?>
                </div>
            </div>
            <div class="col-md-12">
                <div class="account-card">
                                ...
                                <tr>
                                    <td colspan="7">
                                        <div class="float-right">
                                            <?=__('Paid:');?>
                                            <strong
                                                style="color:red;"><?=format_currency($SMARTSTUDY->get_row_safe("SELECT SUM(`price`) FROM `payment_tmweasyapi` WHERE $where_clause", $where_params)['SUM(`price`)']);?></strong>
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status == 'success') {
                ...
                var WM_TEXT = '<?=$SMARTSTUDY->site('tmweasyapi_watermark_text');?>';
                var WM_COLOR = '<?=($SMARTSTUDY->site('tmweasyapi_watermark_color') != '' ? $SMARTSTUDY->site('tmweasyapi_watermark_color') : '#ff0000');?>';
                var WM_OPACITY = parseFloat('<?=($SMARTSTUDY->site('tmweasyapi_watermark_opacity') != '' ? $SMARTSTUDY->site('tmweasyapi_watermark_opacity') : 0.28);?>');
                var WM_FONT_RATIO = parseFloat('<?=($SMARTSTUDY->site('tmweasyapi_watermark_font_size') != '' ? $SMARTSTUDY->site('tmweasyapi_watermark_font_size') : 0.08);?>');
