...
$body = [
    'title' => __('Sản phẩm yêu thích').' | '.$SMARTSTUDY->site('title'),
                                ...
                                <table class="table fs-sm text-nowrap mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        <?php
                                            ...
                                            $product = $SMARTSTUDY->get_row_safe("SELECT * FROM `products` WHERE `id` = ?", [$favorite['product_id']]);
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <label class="label-text feat">
                                                    <?php 
                                                    $stock = (($product['product_type'] ?? 'stock') === 'course') ? 1 : ($product['supplier_id'] != 0 ? $product['api_stock'] : getStock($product['code']));?>
                                                    <b><?=($product['product_type'] ?? 'stock') === 'course' ? __('Học trực tuyến') : format_cash($stock);?></b></label>
                                            </td>
                                            <td class="text-right">
                                                <b
...
<ul id="top-menu-<?=$SMARTSTUDY->site('menu_category_right') == 1 ? 'right' : 'left';?>">
    ...
    <?php foreach($SMARTSTUDY->get_list_safe(" SELECT * FROM `categories` WHERE `status` = ? AND `parent_id` != ? ", [1, 0]) as $category):?>
    <li>
