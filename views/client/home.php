...
if($SMARTSTUDY->site('isLoginRequiredToViewProduct') == 1) {
...
if($SMARTSTUDY->site('popup_vat') == 1 && isset($getUser)) {
    // Kiểm tra xem user đã nhập thông tin VAT chưa
    $has_vat_info = false;
    try {
        $vat_info_json = $SMARTSTUDY->get_row_safe("SELECT vat_info FROM `users` WHERE `id` = ?", [$getUser['id']]);
...
if (isset($_GET['slug'])) {
    ...
    if($slug !== false) {
        if (!$category = $SMARTSTUDY->get_row_safe("SELECT * FROM `categories` WHERE `slug` = ? AND `status` = ?", [$slug, 1])) {
            $category_id = '';
            $category_slug = '';
        }else{
            $category_id = $category['id'];
            $category_slug = $category['slug'];
            $body['title']  = $category['name'].' | '.$SMARTSTUDY->site('title');
...
<section class="section feature-part">
    <div class="container">
        ...
        <div class="row mb-5">
            <?php if($SMARTSTUDY->site('notice_home') != ''):?>
            <div class="col-md-12">
                <div class="account-card pt-3">
                    <?=$SMARTSTUDY->site('notice_home');?>
                </div>
            </div>
            <?php endif?>
            ...
            <div class="<?=$SMARTSTUDY->site('cot_so_du_ben_phai') == 1 ? 'col-xl-9' : 'col-xl-12';?>">
                <?php if($SMARTSTUDY->site('show_btn_category_home') == 1):?>
            ...
            <?php if($SMARTSTUDY->site('cot_so_du_ben_phai') == 1):?>
            <div class="col-xl-3">
        ...
        <?php if($SMARTSTUDY->site('status_giao_dich_gan_day') == 1):?>
        <div class="row">
            <div class="col-lg-6 mb-3">
                ...
                <div style="height:350px;overflow-x:hidden;overflow-y:auto;">
                    ...
                    $recent_orders = $SMARTSTUDY->get_list_safe("
                        SELECT o.*, u.username 
                        FROM `order_log` o 
                        ...
                        <div class="feature-content">
                            <div class="row">
                                <div class="col-10 col-md-10">
                                    <?php
                                    $content = $SMARTSTUDY->site('content_gd_mua_gan_day');
            ...
            <div class="col-lg-6 mb-3">
                ...
                <div style="height:350px;overflow-x:hidden;overflow-y:auto;">
                    <?php 
                    // ⚡ Tối ưu: Sử dụng JOIN thay vì query trong vòng lặp (tránh N+1 problem)
                    $recent_deposits = $SMARTSTUDY->get_list_safe("
                        SELECT d.*, u.username 
                        FROM `deposit_log` d 
                        ...
                        <div class="feature-content">
                            <div class="row">
                                <div class="col-9 col-md-10">
                                    <?php
                                    $content = $SMARTSTUDY->site('content_gd_nap_tien_gan_day');
...
function loadProductsByCategory(categoryId, categorySlug) {
    ...
    let newUrl = '<?=base_url();?>';
    let pageTitle = <?=json_encode($SMARTSTUDY->site('title'));?>;
    ...
    if(categorySlug) {
        ...
        pageTitle = categoryName.trim() + ' | ' + <?=json_encode($SMARTSTUDY->site('title'));?>;
    }
...
<?php if($SMARTSTUDY->site('menu_category_right') != 0):?>
<ul id="top-menu-<?=$SMARTSTUDY->site('menu_category_right') == 1 ? 'right' : 'left';?>">
    <li>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const topMenu = document.getElementById('top-menu-<?=$SMARTSTUDY->site('menu_category_right') == 1 ? 'right' : 'left';?>');
...
<?php if($SMARTSTUDY->site('popup_status') == 1):?>
