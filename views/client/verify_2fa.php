...
$body = [
    'title' => __('Xác minh 2FA').' | '.$SMARTSTUDY->site('title'),
...
if (isset($_GET['token'])) {
    ...
    if($token === false || !$getUser = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `token_2fa` = ? AND `token_2fa` IS NOT NULL", [$token])) {
