...
$body = [
    'title' => __('Nạp tiền bằng Perfect Money').' | '.$SMARTSTUDY->site('title'),
...
if($SMARTSTUDY->site('perfectmoney_status') != 1){
    redirect(base_url());
}
...
if ($SMARTSTUDY->get_row_safe("SELECT * FROM `payment_pm` WHERE `user_id` = ? AND `status` = 0", [$getUser['id']])) {
    $payment_id = $SMARTSTUDY->get_row_safe("SELECT * FROM `payment_pm` WHERE `user_id` = ? AND `status` = 0", [$getUser['id']])['payment_id'];
} else {
    $payment_id = random('QWERTYUIOPASDFGHJKLZXCVBNM', 4).'_'.time();
    $SMARTSTUDY->insert("payment_pm", [
        'user_id'       => $getUser['id'],
...
$params = [
    ...
    'PAYEE_ACCOUNT'         => $SMARTSTUDY->site('perfectmoney_account'), // mã tài khoản Perfect Money
    'PAYMENT_UNITS'         => $SMARTSTUDY->site('perfectmoney_units'), // đơn vị tiền tệ,
    ...
    'SUGGESTED_MEMO'        => 'Payment - '.$SMARTSTUDY->site('title')
];
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-5">
                ...
                <div class="account-card pt-3">
                    <?=$SMARTSTUDY->site('perfectmoney_notice');?>
                </div>
            </div>
            <div class="col-md-12">
                <div class="home-heading mb-3">
                                ...
                                <tr>
                                    <td colspan="7">
                                        <div class="float-right">
                                            <?=__('Đã thanh toán:');?>
                                            <strong
                                                style="color:red;"><?=format_currency($SMARTSTUDY->get_row_safe(" SELECT SUM(`price`) FROM `payment_pm` WHERE $where_clause ", $where_params)['SUM(`price`)']);?></strong>
