...
$body = [
    'title' => __('CTV Withdraw').' | '.$SMARTSTUDY->site('title'),
...
$listDatatable = $SMARTSTUDY->get_list_safe("SELECT * FROM `ctv_withdraw` WHERE $where ORDER BY `id` DESC LIMIT ?, ?", array_merge($where_params, [$from, $limit]));
$totalDatatable = $SMARTSTUDY->num_rows_safe("SELECT * FROM `ctv_withdraw` WHERE $where ORDER BY id DESC", $where_params);

// Tính toán số dư CTV (tổng doanh thu - tổng rút tiền)
$totalRevenue = $SMARTSTUDY->get_row_safe("SELECT SUM(pay) as total FROM `product_order` WHERE `seller` = ? AND `trash` = 0", [$getUser['id']])['total'] ?? 0;
$totalWithdrawn = $SMARTSTUDY->get_row_safe("SELECT SUM(amount) as total FROM `ctv_withdraw` WHERE `user_id` = ? AND `status` = 'completed'", [$getUser['id']])['total'] ?? 0;
$totalFeePaid = $SMARTSTUDY->get_row_safe("SELECT SUM(fee) as total FROM `ctv_withdraw` WHERE `user_id` = ? AND `status` = 'completed'", [$getUser['id']])['total'] ?? 0;
        ...
        <div class="row">
                        ...
                        <form id="withdrawForm">
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-8">
                                    ...
                                    <select class="form-control" id="bank" required>
                                        ...
                                        $listbank = explode(PHP_EOL, $SMARTSTUDY->site('ctv_banks_withdraw')); 
                                        ?>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-8">
                                    ...
                                    <small class="text-muted"><?=__('Số tiền tối thiểu:');?> <?=format_currency($SMARTSTUDY->site('ctv_min_withdraw'));?> - <?=__('Phí rút');?> <?=$SMARTSTUDY->site('ctv_fee_withdraw');?>%</small>
                                </div>
                            </div>
                            ...
                            <div id="feeCalculation" style="display: none;">
                                <div class="row mb-2">
                                    ...
                                    <div class="col-sm-8">
                                        ...
                                        <small class="text-muted d-block">(<?=$SMARTSTUDY->site('ctv_fee_withdraw');?>%)</small>
                                    </div>
                                </div>
                                <div class="row mb-3">
                                    <div class="col-sm-4">
            ...
            <div class="col-xl-6">
                <div class="card custom-card">
                    <div class="card-body">
                        <div class="mb-3">
                        <?=$SMARTSTUDY->site('ctv_notice_withdraw');?>
                        </div>
                        <div class="row">
                            ...
                            <?php if($SMARTSTUDY->site('ctv_fee_withdraw') > 0): ?>
                            <div class="col-lg-4 col-md-6 col-12">
                                <div class="d-flex align-items-center justify-content-between p-3 border rounded mb-2">
                                    <div>
                        ...
                        <div class="table-responsive mb-3">
                            <table class="table text-nowrap table-striped table-hover table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <?php else: ?>
                                        <tr>
                                            <td colspan="<?=$SMARTSTUDY->site('ctv_fee_withdraw') > 0 ? '8' : '6';?>" class="text-center py-4">
...
$("#btnWithdraw").on("click", function() {
    ...
    if (parseFloat(amount) < <?=$SMARTSTUDY->site('ctv_min_withdraw');?>) {
        Swal.fire({
            title: '<?=__('Lỗi');?>',
            text: '<?=__('Số tiền rút tối thiểu là').' '.format_currency($SMARTSTUDY->site('ctv_min_withdraw'));?>',
            icon: 'error'
        });
        return;
    }
    ...
    var feePercent = <?=$SMARTSTUDY->site('ctv_fee_withdraw');?>;
