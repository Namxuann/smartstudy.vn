...
$body = [
    'title' => __('Chỉnh sửa sản phẩm').' | CTV Panel | '.$SMARTSTUDY->site('title'),
...
if (isset($_POST['save'])) {
    ...
    if($product['name'] != $name){
        if ($SMARTSTUDY->get_row_safe("SELECT * FROM `products` WHERE `name` = ?", [$name])) {
    ...
    if($product['slug'] != $slug){
        if ($SMARTSTUDY->get_row_safe("SELECT * FROM `products` WHERE `slug` = ? AND `id` != ?", [$slug, $product['id']])) {
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <select class="form-control js-example-basic-single" name="category_id" required>
                                    ...
                                    <?php foreach($SMARTSTUDY->get_list_safe("SELECT * FROM `categories` WHERE `parent_id` = 0 ORDER BY `name`", []) as $parent): ?>
                                        <optgroup label="<?=$parent['name'];?>">
                                            <?php foreach($SMARTSTUDY->get_list_safe("SELECT * FROM `categories` WHERE `parent_id` = ? ORDER BY `name`", [$parent['id']]) as $child): ?>
