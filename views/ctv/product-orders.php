...
$body = [
    'title' => __('Đơn hàng').' | '.$SMARTSTUDY->site('title'),
...
if (!empty($_GET['account'])) {
    ...
    if ($account !== false) {
        $product_sold_rows = $SMARTSTUDY->get_list_safe('SELECT * FROM `product_sold` WHERE `account` LIKE ?', ["%$account%"]);
...
if (!empty($_GET['uid'])) {
    ...
    if ($uid !== false) {
        $product_sold_rows = $SMARTSTUDY->get_list_safe('SELECT * FROM `product_sold` WHERE `uid` = ?', [$uid]);
        if (!empty($product_sold_rows)) {
            $trans_ids = array_map(function($row) {
                return $row['trans_id'];
            }, $product_sold_rows);
...
if (!empty($_GET['username'])) {
    ...
    if ($username !== false) {
        if($idUser = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `email` LIKE ?", ["%$username%"])){
            $where .= ' AND `buyer` = ? ';
...
$listDatatable = $SMARTSTUDY->get_list_safe("SELECT * FROM `product_order` WHERE $where ORDER BY `id` DESC LIMIT ?, ?", array_merge($where_params, [$from, $limit]));
$totalDatatable = $SMARTSTUDY->num_rows_safe("SELECT * FROM `product_order` WHERE $where ORDER BY id DESC", $where_params);
                            ...
                            <div class="row g-2 mb-3">
                                ...
                                <div class="col-md-3 col-6">
                                    <select class="form-control js-example-basic-single" name="product_id">
                                        ...
                                        <?php foreach($SMARTSTUDY->get_list_safe("SELECT * FROM `products` WHERE `user_id` = ?", [$getUser['id']]) as $product):?>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <?php $user = $SMARTSTUDY->get_row_safe("SELECT * FROM users WHERE id = ?", [$order['buyer']]);?>
                                    ...
                                    <tr>
                                        <td colspan="7">
                                            <div class="text-right">
                                                <?=__('Tài khoản đã bán');?>:
                                                <strong><?=format_cash($SMARTSTUDY->get_row_safe("SELECT SUM(`amount`) as total FROM `product_order` WHERE `refund` = 0 AND $where", $where_params)['total']);?></strong>
                                                |
                                                <?=__('Đơn hàng');?>: <strong
                                                    style="color: green;"><?=format_cash($totalDatatable);?></strong> |
                                                <?=__('Doanh thu');?>: <strong
                                                    style="color:red;"><?=format_currency($SMARTSTUDY->get_row_safe("SELECT SUM(`pay`) as total FROM `product_order` WHERE `refund` = 0 AND $where", $where_params)['total']);?></strong>
                                                |
                                                <?=__('Lợi nhuận');?>: <strong
                                                    style="color:blue;"><?=format_currency($SMARTSTUDY->get_row_safe("SELECT SUM(`pay`) as total FROM `product_order` WHERE `refund` = 0 AND $where", $where_params)['total']-$SMARTSTUDY->get_row_safe("SELECT SUM(`cost`) as total FROM `product_order` WHERE `refund` = 0 AND $where", $where_params)['total']);?></strong>
                                            </div>
                                        </td>
                                    </tr>
                                </tfoot>
