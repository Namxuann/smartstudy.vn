...
$body = [
    'title' => __('Kho hàng sản phẩm').' | '.$SMARTSTUDY->site('title'),
...
$product = $SMARTSTUDY->get_row_safe("SELECT * FROM `products` WHERE `code` = ? AND `user_id` = ? AND `pending` = 0", [$code, $getUser['id']]);
if (!$product) {
...
$listDatatable = $SMARTSTUDY->get_list_safe("SELECT * FROM `product_stock` WHERE $where ORDER BY `id` DESC LIMIT ?, ?", array_merge($where_params, [$from, $limit]));
$totalDatatable = $SMARTSTUDY->num_rows_safe("SELECT * FROM `product_stock` WHERE $where ORDER BY id DESC", $where_params);
...
if(isset($_POST['RemoveAccounts'])){
    ...
    foreach ($list as $account){
        ...
        $check_uid = $SMARTSTUDY->get_row_safe("SELECT * FROM `product_stock` WHERE `uid` = ? AND `product_code` = ?", [$uid_remove, $code]);
        if(!$check_uid) {
            $value_not_found++;
            continue;
        }
        
        $isRemove = $SMARTSTUDY->remove("product_stock", " `uid` = ? AND `product_code` = ?", [$uid_remove, $code]);
        if ($isRemove) {
            $value_remove++;
...
if(isset($_POST['AddAccounts'])){
    ...
    foreach ($list as $account){
        ...
        if (isset($_POST['loc_trung_uid_sold']) && $_POST['loc_trung_uid_sold'] == 1){
            if($SMARTSTUDY->get_row_safe("SELECT * FROM `product_sold` WHERE `uid` = ?", [$uid])){
                $value_skip_sold++;
                continue;
            }
        }
        ...
        if (isset($_POST['loc_trung_uid']) && $_POST['loc_trung_uid'] == 1){
            if($SMARTSTUDY->get_row_safe("SELECT * FROM `product_stock` WHERE `uid` = ? AND `product_code` = ?", [$uid, $code])){
                ...
                $isAdd = $SMARTSTUDY->insert("product_stock", [
                    'product_code'  => $code,
                    'seller'        => $getUser['id'],
                    'uid'           => $uid,
                    'account'       => $account,
                    ...
                    <div class="card-header justify-content-between">
                        ...
                        <div class="btn-list">
                            ...
                            <button type="button" onclick="viewListDIE(`<?=$code;?>`)" id="btn_viewListDIE"
                                ...
                                <span class="badge ms-2 bg-dark text-white"><?=format_cash($SMARTSTUDY->get_row_safe("SELECT COUNT(id) as total FROM `product_die` WHERE `product_code` = ?", [$code])['total']);?></span>
                            </button>
                        </div>
                    </div>
                    <div class="card-body">
