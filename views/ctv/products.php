...
$body = [
    'title' => __('Quản lý sản phẩm').' | CTV Panel | '.$SMARTSTUDY->site('title'),
...
$listDatatable = $SMARTSTUDY->get_list_safe("SELECT * FROM `products` WHERE $where ORDER BY `id` DESC LIMIT ?, ?", array_merge($where_params, [$from, $limit]));
$totalDatatable = $SMARTSTUDY->num_rows_safe("SELECT * FROM `products` WHERE $where ORDER BY id DESC", $where_params);
                            ...
                            <div class="row g-2 mb-3">
                                ...
                                <div class="col-lg col-md-4 col-6">
                                    <select class="form-control js-example-basic-single" name="category_id">
                                        ...
                                        <?php foreach($SMARTSTUDY->get_list_safe("SELECT * FROM `categories` WHERE `parent_id` = 0", []) as $option):?>
                                        <option disabled value="<?=$option['id'];?>"><?=$option['name'];?></option>
                                        <?php foreach($SMARTSTUDY->get_list_safe("SELECT * FROM `categories` WHERE `parent_id` = ?", [$option['id']]) as $option1):?>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <b><?=format_cash($SMARTSTUDY->get_row_safe("SELECT COUNT(id) as total FROM product_die WHERE `product_code` = ?", [$product['code']])['total']);?></b><br>
