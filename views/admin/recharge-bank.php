...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `payment_bank` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `payment_bank` WHERE $where ORDER BY id DESC ");
...
$total_yesterday = intval($SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_bank WHERE  `create_gettime` LIKE '%" . $yesterday . "%' ")['SUM(amount)']);
$total_today = $SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_bank WHERE  `create_gettime` LIKE '%" . $currentDate . "%' ")['SUM(amount)'];
$total_all_time = $SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_bank ")['SUM(amount)'];
...
<div class="main-content app-content">
    <div class="container-fluid">
        ...
        <?php if (time() - $SMARTSTUDY->site('check_time_cron_bank') >= 120): ?>
            <div class="alert alert-danger alert-dismissible fade show custom-alert-icon shadow-sm" role="alert">
                ...
                Vui lòng thực hiện <b><a target="_blank" class="text-primary" href="https://smartstudy.vn">CRON JOB</a></b> liên kết: <a class="text-primary" href="<?= base_url('cron/bank.php?key=' . $SMARTSTUDY->site('key_cron_job')); ?>"
                    target="_blank"><?= base_url('cron/bank.php?key=' . $SMARTSTUDY->site('key_cron_job')); ?></a> 1 phút 1 lần hoặc nhanh hơn để hệ thống xử lý nạp tiền tự động, nếu
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?= format_currency($SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_bank WHERE  MONTH(create_gettime) = '$currentMonth' AND YEAR(create_gettime) = '$currentYear' ")['SUM(amount)']); ?>
                                        </p>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?= format_currency($SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_bank WHERE  YEAR(create_gettime) = $currentYear AND WEEK(create_gettime, 1) = $currentWeek ")['SUM(amount)']); ?>
                                        </p>
                                ...
                                const data = {
                                    ...
                                    datasets: [{
                                        ...
                                        data: [
                                            ...
                                            for ($day = 1; $day <= $numOfDays; $day++) {
                                                $date = "$year-$month-$day";
                                                $row = $SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_bank WHERE DATE(create_gettime) = '$date' ");
                                    ...
                                    <tr>
                                        <td colspan="7">
                                            <div class="float-right">
                                                <?= __('Đã thanh toán:'); ?>
                                                <strong
                                                    style="color:red;"><?= format_currency($SMARTSTUDY->get_row(" SELECT SUM(`amount`) FROM `payment_bank` WHERE $where ")['SUM(`amount`)']); ?></strong>
                                                |

                                                <?= __('Thực nhận:'); ?>
                                                <strong
                                                    style="color:blue;"><?= format_currency($SMARTSTUDY->get_row(" SELECT SUM(`received`) FROM `payment_bank` WHERE $where ")['SUM(`received`)']); ?></strong>
                                        </td>
                                    </tr>
                                </tfoot>
                            </table>
