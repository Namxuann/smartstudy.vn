...
$body = [
    'title' => __('Quản lý kết nối API') . ' | ' . $SMARTSTUDY->site('title'),
...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `suppliers` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `suppliers` WHERE $where ORDER BY id DESC ");
$urlDatatable = pagination(base_url_admin("product-api&limit=$limit&shortByDate=$shortByDate&"), $from, $totalDatatable, $limit);

?>
<?php
$doanh_thu = $SMARTSTUDY->get_row(" SELECT SUM(pay) FROM product_order WHERE `refund` = 0 AND supplier_id != 0 ")['SUM(pay)'];
$tien_von = $SMARTSTUDY->get_row(" SELECT SUM(cost) FROM product_order WHERE `refund` = 0 AND supplier_id != 0 ")['SUM(cost)'];
$loi_nhuan = $doanh_thu - $tien_von;
$don_hang = $SMARTSTUDY->get_row(" SELECT COUNT(id) FROM product_order WHERE `refund` = 0 AND supplier_id != 0 ")['COUNT(id)'];
    ...
    <div class="container-fluid">
        ...
        foreach ($listDatatable as $sup) {
            foreach ($cron_suppliers as $type => $key) {
                if ($sup['type'] == $type && !in_array($type, $cron_warned) && time() - $SMARTSTUDY->site("time_cron_suppliers_$key") >= 120) {
                    $cron_warned[] = $type;
        ?>
                                ...
                                <tbody>
                                    ...
                                    WHERE supplier_id = '" . $supplier['id'] . "'";
                                        ...
                                        <tr onchange="updateForm(`<?= $supplier['id']; ?>`)">
                                            ...
                                            <td>
                                                <?php
                                                $categoryCount = format_cash($SMARTSTUDY->get_row("SELECT COUNT(id) FROM `categories` WHERE `supplier_id` = '" . $supplier['id'] . "' ")['COUNT(id)']);
                                                $productCount = format_cash($SMARTSTUDY->get_row("SELECT COUNT(id) FROM `products` WHERE `supplier_id` = '" . $supplier['id'] . "' ")['COUNT(id)']);
                                                ?>
        ...
        <script>
            $(document).ready(function() {
                $('.brand-carousel').html('');
                $.ajax({
                    url: '<?= base_url('api/smartstudy_suppliers.php'); ?>',
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
                        ...
                        if (response && response.suppliers.length > 0) {
                            ...
                            $.each(response.suppliers, function(index, brand) {
                                ...
                                html += '<a href="' + brand.domain +
                                    '?utm_source=smartstudy" target="_blank" class="website-button btn btn-sm btn-primary">Xem</a>';
                                html += '</div>';
                            });
