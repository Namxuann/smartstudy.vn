...
if (isset($_POST['AddTask'])) {
    ...
    $isInsert = $SMARTSTUDY->insert("automations", [
...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `automations` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `automations` WHERE $where ORDER BY id DESC ");
...
<div class="main-content app-content">
    <div class="container-fluid">
        ...
        <?php if (time() - $SMARTSTUDY->site('check_time_cron_task') >= 300): ?>
            <div class="alert alert-danger alert-dismissible fade show custom-alert-icon shadow-sm" role="alert">
                ...
                Vui lòng thực hiện <b><a target="_blank" class="text-primary" href="https://smartstudy.vn">CRON JOB</a></b> liên kết: <a class="text-primary" href="<?= base_url('cron/task.php?key=' . $SMARTSTUDY->site('key_cron_job')); ?>"
                    target="_blank"><?= base_url('cron/task.php?key=' . $SMARTSTUDY->site('key_cron_job')); ?></a> 1 - 5 phút 1 lần để sử dụng được chức năng này.
                    ...
                    <div class="row mb-4" id="product_id_input" style="display: none;">
                        ...
                        <div class="col-sm-8">
                            ...
                            <div id="productCheckboxList" style="border: 1px solid #e0e0e0; border-radius: 6px; max-height: 300px; overflow-y: auto; background: #fff;">
                                ...
                                <?php foreach ($SMARTSTUDY->get_list(" SELECT * FROM `categories` ") as $category): ?>
                                    <?php $catProducts = $SMARTSTUDY->get_list(" SELECT * FROM `products` WHERE `category_id` = '" . $category['id'] . "' "); ?>
