...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `payment_thesieure` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `payment_thesieure` WHERE $where ORDER BY id DESC ");
...
$total_yesterday = intval($SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_thesieure WHERE  `create_gettime` LIKE '%".$yesterday."%' ")['SUM(amount)']);
$total_today = $SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_thesieure WHERE  `create_gettime` LIKE '%".$currentDate."%' ")['SUM(amount)'];
$total_all_time = $SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_thesieure ")['SUM(amount)'];
?>
...
<div class="main-content app-content">
    <div class="container-fluid">
        ...
        <?php if(time() - $SMARTSTUDY->site('check_time_cron_thesieure') >= 120):?>
        <div class="alert alert-danger alert-dismissible fade show custom-alert-icon shadow-sm" role="alert">
            ...
            Vui lòng thực hiện <b><a target="_blank" class="text-primary" href="https://smartstudy.vn">CRON JOB</a></b> liên kết: <a class="text-primary" href="<?=base_url('cron/thesieure.php?key='.$SMARTSTUDY->site('key_cron_job'));?>"
                target="_blank"><?=base_url('cron/thesieure.php?key='.$SMARTSTUDY->site('key_cron_job'));?></a> 1 phút 1 lần để hệ thống xử lý nạp tiền tự động.
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <select class="form-control" name="thesieure_status">
                                                <option <?=$SMARTSTUDY->site('thesieure_status') == 1 ? 'selected' : '';?>
                                                    value="1">ON
                                                </option>
                                                <option <?=$SMARTSTUDY->site('thesieure_status') == 0 ? 'selected' : '';?>
                                                    value="0">OFF
                                                </option>
                                            </select>
                                        </div>
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$SMARTSTUDY->site('thesieure_number');?>" name="thesieure_number">
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$SMARTSTUDY->site('thesieure_email');?>" name="thesieure_email">
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$SMARTSTUDY->site('thesieure_name');?>" name="thesieure_name">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-12 col-xl-6">
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-6">
                                            <input type="text" class="form-control"
                                                value="<?=$SMARTSTUDY->site('thesieure_token');?>" name="thesieure_token">
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-12">
                                            <textarea id="thesieure_notice"
                                                name="thesieure_notice"><?=$SMARTSTUDY->site('thesieure_notice');?></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_currency($SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_thesieure WHERE  MONTH(create_gettime) = '$currentMonth' AND YEAR(create_gettime) = '$currentYear' ")['SUM(amount)']);?>
                                        </p>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_currency($SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_thesieure WHERE  YEAR(create_gettime) = $currentYear AND WEEK(create_gettime, 1) = $currentWeek ")['SUM(amount)']);?>
                                        </p>
      ...
      echo '<span class="fs-12 text-danger ms-2"><i class="ti ti-trending-down me-1 d-inline-block"></i>' . round(abs($revenueGrowth), 2) . '% </span>';
            ...
            <div class="col-xl-7">
              ...
              ?>
                  ...
                  for ($day = 1; $day <= $numOfDays; $day++) {
                      $date = "$year-$month-$day";
                      $row = $SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_thesieure WHERE DATE(create_gettime) = '$date' ");
                                    ...
                                    <tr>
                                        <td colspan="7">
                                            <div class="float-right">
                                                <?=__('Đã thanh toán:');?>
                                                <strong
                                                    style="color:red;"><?=format_currency($SMARTSTUDY->get_row(" SELECT SUM(`amount`) FROM `payment_thesieure` WHERE $where ")['SUM(`amount`)']);?></strong>
                                                |

                                                <?=__('Thực nhận:');?>
                                                <strong
                                                    style="color:blue;"><?=format_currency($SMARTSTUDY->get_row(" SELECT SUM(`received`) FROM `payment_thesieure` WHERE $where ")['SUM(`received`)']);?></strong>
                                        </td>
                                    </tr>
                                </tfoot>
                            </table>
