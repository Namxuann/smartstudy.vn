...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `cards` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `cards` WHERE $where ORDER BY id DESC ");
...
$total_yesterday = intval($SMARTSTUDY->get_row("SELECT SUM(amount) FROM cards WHERE  `status` = 'completed' AND `create_date` LIKE '%" . $yesterday . "%' ")['SUM(amount)']);
$total_today = $SMARTSTUDY->get_row("SELECT SUM(amount) FROM cards WHERE  `status` = 'completed' AND `create_date` LIKE '%" . $currentDate . "%' ")['SUM(amount)'];
$total_all_time = $SMARTSTUDY->get_row("SELECT SUM(amount) FROM cards WHERE  `status` = 'completed' ")['SUM(amount)'];
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <select class="form-control" name="card_status">
                                                <option <?= $SMARTSTUDY->site('card_status') == 1 ? 'selected' : ''; ?>
                                                    value="1">ON
                                                </option>
                                                <option <?= $SMARTSTUDY->site('card_status') == 0 ? 'selected' : ''; ?>
                                                    value="0">OFF
                                                </option>
                                            </select>
                                        </div>
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?= $SMARTSTUDY->site('card_partner_id'); ?>" name="card_partner_id">
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?= $SMARTSTUDY->site('card_partner_key'); ?>" name="card_partner_key">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-12 col-xl-6">
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-6">
                                            <div class="input-group">
                                                <input type="text" class="form-control"
                                                    value="<?= $SMARTSTUDY->site('card_ck'); ?>" name="card_ck"
...
" rows="5" name="list_network_topup_card"><?= $SMARTSTUDY->site('list_network_topup_card'); ?></textarea>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-12">
                                            <textarea id="card_notice"
                                                name="card_notice"><?= $SMARTSTUDY->site('card_notice'); ?></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?= format_currency($SMARTSTUDY->get_row("SELECT SUM(amount) FROM cards WHERE  `status` = 'completed' AND MONTH(create_date) = '$currentMonth' AND YEAR(create_date) = '$currentYear' ")['SUM(amount)']); ?>
                                        </p>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?= format_currency($SMARTSTUDY->get_row("SELECT SUM(amount) FROM cards WHERE  `status` = 'completed' AND YEAR(create_date) = $currentYear AND WEEK(create_date, 1) = $currentWeek ")['SUM(amount)']); ?>
                                        </p>
                                ...
                                const data = {
                                    ...
                                    datasets: [{
                                        ...
                                        data: [
                                            ...
                                            for ($day = 1; $day <= $numOfDays; $day++) {
                                                $date = "$year-$month-$day";
                                                $row = $SMARTSTUDY->get_row("SELECT SUM(amount) FROM cards WHERE DATE(create_date) = '$date' AND  `status` = 'completed' ");
                                    ...
                                    <tr>
                                        <td colspan="7">
                                            <div class="float-right">
                                                <?= __('Tổng nạp:'); ?>
                                                <strong
                                                    style="color:red;"><?= format_currency($SMARTSTUDY->get_row(" SELECT SUM(`amount`) FROM `cards` WHERE $where AND `status` = 'completed' ")['SUM(`amount`)']); ?></strong>
                                                | <?= __('Thực nhận:'); ?>
                                                <strong
                                                    style="color:blue;"><?= format_currency($SMARTSTUDY->get_row(" SELECT SUM(`price`) FROM `cards` WHERE $where AND `status` = 'completed' ")['SUM(`price`)']); ?></strong>
                                            </div>
                                        </td>
                                    </tr>
                                </tfoot>
