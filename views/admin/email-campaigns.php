...
$body = [
    'title' => __('Email Campaigns').' | '.$SMARTSTUDY->site('title'),
...
if (isset($_POST['submit'])) {
    ...
    $isInsert = $SMARTSTUDY->insert('email_campaigns', [
    ...
    if (empty($_POST['listUser'])) {
        foreach ($SMARTSTUDY->get_list("SELECT * FROM `users` WHERE `banned` = 0 AND `email` IS NOT NULL ") as $user) {
            $SMARTSTUDY->insert('email_sending', [
                'camp_id'           => $SMARTSTUDY->get_row(" SELECT `id` FROM `email_campaigns` ORDER BY id DESC LIMIT 1 ")['id'],
                'user_id'           => $user['id'],
                'status'            => 0,
                'create_gettime'    => gettime(),
                'update_gettime'    => gettime()
        ...
        foreach ($_POST['listUser'] as $user) {
            $user = $SMARTSTUDY->get_row("SELECT * FROM `users` WHERE `id` = '$user' ");
            $SMARTSTUDY->insert('email_sending', [
                'camp_id'           => $SMARTSTUDY->get_row(" SELECT `id` FROM `email_campaigns` ORDER BY id DESC LIMIT 1 ")['id'],
                'user_id'           => $user['id'],
                'status'            => 0,
...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `email_campaigns` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `email_campaigns` WHERE $where ORDER BY id DESC ");
...
<div class="main-content app-content">
    <div class="container-fluid">
        ...
        <?php if(time() - $SMARTSTUDY->site('check_time_cron_sending_email') >= 120):?>
        <div class="alert alert-danger alert-dismissible fade show custom-alert-icon shadow-sm" role="alert">
            ...
            Vui lòng thực hiện <b><a target="_blank" class="text-primary" href="https://smartstudy.vn">CRON JOB</a></b> liên kết: <a class="text-primary"
                href="<?=base_url('cron/sending_email.php?key='.$SMARTSTUDY->site('key_cron_job'));?>"
                target="_blank"><?=base_url('cron/sending_email.php?key='.$SMARTSTUDY->site('key_cron_job'));?></a> 1 phút 1 lần để sử dụng chức năng Email
            Campaigns.
                            ...
                            <table class="table text-nowrap table-striped table-hover table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php
                                                $total_success = $SMARTSTUDY->get_row(" SELECT COUNT(id) FROM `email_sending` WHERE `camp_id` = '".$row['id']."' AND `status` = 1 ")['COUNT(id)'];
                                                $total = $SMARTSTUDY->get_row(" SELECT COUNT(id) FROM `email_sending` WHERE `camp_id` = '".$row['id']."' ")['COUNT(id)'];
                                                $phantram = 0;
                                                if($total != 0){
