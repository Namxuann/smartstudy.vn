...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `ctv_withdraw` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `ctv_withdraw` WHERE $where ORDER BY id DESC ");
...
<div class="main-content app-content">
    <div class="container-fluid">
        ...
        <?php if($SMARTSTUDY->site('cong_tien_nguoi_ban') != 1):?>
        <div class="row">
            <div class="col-12">
        ...
        <div class="row">
            <div class="col-12">
                <div class="alert alert-danger alert-dismissible fade show custom-alert-icon shadow-sm mb-3"
                    ...
                    <br><small class="text-muted">Vui lòng thêm cột <code>pending</code> vào bảng <code>products</code> theo cấu trúc dữ liệu Smartstudy.vn để sử dụng đầy đủ tính năng.</small>
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"><i
                            class="bi bi-x"></i></button>
                </div>
            </div>
        ...
        <?php endif?>
        <?php if($SMARTSTUDY->site('ctv_status') != 1):?>
        <div class="row">
            <div class="col-12">
                        ...
                        <div class="d-flex align-items-center">
                            <div class="flex-fill">
                                <p class="mb-1 fs-5 fw-semibold text-default">
                                    <?=format_currency($SMARTSTUDY->get_row(" SELECT SUM(amount) FROM `ctv_withdraw` WHERE `status` = 'completed' ")['SUM(amount)']);?>
                                </p>
                ...
                <div class="card custom-card">
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            <div class="flex-fill">
                                <p class="mb-1 fs-5 fw-semibold text-default">
                                    <?=format_currency($SMARTSTUDY->get_row("SELECT SUM(amount) FROM `ctv_withdraw` WHERE `status` = 'completed' AND MONTH(created_at) = '$currentMonth' AND YEAR(created_at) = '$currentYear' ")['SUM(amount)']);?>
                                </p>
                ...
                <div class="card custom-card">
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            <div class="flex-fill">
                                <p class="mb-1 fs-5 fw-semibold text-default">
                                    <?=format_currency($SMARTSTUDY->get_row("SELECT SUM(amount) FROM ctv_withdraw WHERE  `status` = 'completed' AND YEAR(created_at) = $currentYear AND WEEK(created_at, 1) = $currentWeek ")['SUM(amount)']);?>
                                </p>
                ...
                <div class="card custom-card">
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            <div class="flex-fill">
                                <p class="mb-1 fs-5 fw-semibold text-default">
                                    <?=format_currency($SMARTSTUDY->get_row("SELECT SUM(amount) FROM ctv_withdraw WHERE  `status` = 'completed' AND `created_at` LIKE '%".$currentDate."%' ")['SUM(amount)']);?>
                                </p>
