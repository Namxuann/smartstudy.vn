...
$body = [
    'title' => __('Categories').' | '.$SMARTSTUDY->site('title'),
...
if (isset($_POST['submit'])) {
    ...
    if ($SMARTSTUDY->get_row_safe("SELECT * FROM `categories` WHERE `slug` = ?", [$slug])) {
        ...
        <div class="row">
            ...
            <div class="col-xl-12">
                <div class="card custom-card">
                    ...
                    <div class="card-body">
                        <?php
                        $parentCategories = $SMARTSTUDY->get_list_safe("SELECT * FROM `categories` WHERE `parent_id` = 0 ORDER BY `stt` DESC");
                        
                        // Lấy danh sách chuyên mục con không có cha hợp lệ
                        $orphanCategories = $SMARTSTUDY->get_list_safe("SELECT c.* FROM `categories` c LEFT JOIN `categories` p ON c.parent_id = p.id WHERE c.parent_id != 0 AND p.id IS NULL ORDER BY c.stt DESC");
                                ...
                                <li class="sortable-parent-item" id="parent-item-<?= $category['id']; ?>"
                                            ...
                                            <div class="d-flex align-items-center flex-wrap category-header-right">
                                                <div class="category-badges">
                                                    <span class="badge bg-primary rounded-pill me-1 category-badge">
                                                        <i
                                                            class="fa-solid fa-folder me-1"></i><?= format_cash($SMARTSTUDY->num_rows_safe("SELECT * FROM `categories` WHERE `parent_id` = ?", [$category['id']])); ?>
                                                    </span>
                                    ...
                                    <div id="category-<?= $category['id']; ?>" class="collapse">
                                        <div class="card-body">
                                            ...
                                            <?php $childCategories = $SMARTSTUDY->get_list_safe("SELECT * FROM `categories` WHERE `parent_id` = ? ORDER BY `stt` DESC", [$category['id']]); ?>
                                                        ...
                                                        <tr id="child-item-<?= $child['id']; ?>"
                                                            ...
                                                            <td>
                                                                <span class="badge bg-primary rounded-pill">
                                                                    <?= format_cash($SMARTSTUDY->num_rows_safe("SELECT * FROM `products` WHERE `category_id` = ?", [$child['id']])); ?>
                                                                </span>
                                                            </td>
                                                            <td>
                        ...
                        <div class="alert alert-info mb-2">
                            ...
                            <div class="tab-pane fade" id="tab-orphan" role="tabpanel" aria-labelledby="orphan-tab">
                                <?php
                                $allParentCategories = $SMARTSTUDY->get_list_safe("SELECT * FROM `categories` WHERE `parent_id` = 0 ORDER BY `stt` DESC");
                                ?>
                                <?php if ($orphanCount > 0): ?>
                                            ...
                                            <tbody>
                                                <?php foreach ($orphanCategories as $orphan): ?>
                                                    <tr id="orphan-row-<?= $orphan['id']; ?>">
                                                        ...
                                                        <td>
                                                            <span class="badge bg-primary rounded-pill">
                                                                <?= format_cash($SMARTSTUDY->num_rows_safe("SELECT * FROM `products` WHERE `category_id` = ?", [$orphan['id']])); ?>
                                                            </span>
                                                        </td>
                                                        <td class="text-center">
            ...
            <div class="modal-body">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="newParentSelect" required>
                        ...
                        $allParentCategories = $SMARTSTUDY->get_list_safe("SELECT * FROM `categories` WHERE `parent_id` = 0 ORDER BY `name` ASC");
