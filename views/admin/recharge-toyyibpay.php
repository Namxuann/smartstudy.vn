...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `payment_toyyibpay` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `payment_toyyibpay` WHERE $where ORDER BY id DESC ");
...
$total_yesterday = intval($SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_toyyibpay WHERE  `status` = 1 AND `create_gettime` LIKE '%".$yesterday."%' ")['SUM(amount)']);
$total_today = $SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_toyyibpay WHERE  `status` = 1 AND `create_gettime` LIKE '%".$currentDate."%' ")['SUM(amount)'];
$total_all_time = $SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_toyyibpay WHERE  `status` = 1 ")['SUM(amount)'];
                            ...
                            <div class="row">
                                <div class="col-lg-12 col-xl-6">
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <select class="form-control" name="toyyibpay_status">
                                                <option <?=$SMARTSTUDY->site('toyyibpay_status') == 1 ? 'selected' : '';?>
                                                    value="1">
                                                    ON
                                                </option>
                                                <option <?=$SMARTSTUDY->site('toyyibpay_status') == 0 ? 'selected' : '';?>
                                                    value="0">
                                                    OFF
                                                </option>
                                            </select>
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$SMARTSTUDY->site('toyyibpay_userSecretKey');?>"
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$SMARTSTUDY->site('toyyibpay_categoryCode');?>"
                                ...
                                <div class="col-lg-12 col-xl-6">
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-5">
                                            <div class="input-group">
                                                <input type="text" class="form-control"
                                                    value="<?=$SMARTSTUDY->site('toyyibpay_rate');?>" name="toyyibpay_rate"
                                                    placeholder="5258">
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-5">
                                            <div class="input-group">
                                                <input type="text" class="form-control"
                                                    value="<?=$SMARTSTUDY->site('toyyibpay_min');?>" name="toyyibpay_min"
                                                    placeholder="1 = RM1">
                                                <span class="input-group-text">
                                                    RM
                                                </span>
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-6">
                                            <select class="form-control" name="toyyibpay_billChargeToCustomer">
                                                <option
                                                    <?=$SMARTSTUDY->site('toyyibpay_billChargeToCustomer') == '0' ? 'selected' : '';?>
                                                    value="0">Your customers pay a fee
                                                </option>
                                                <option
                                                    <?=$SMARTSTUDY->site('toyyibpay_billChargeToCustomer') == '' ? 'selected' : '';?>
                                                    value="">You pay fees
                                                </option>
                                            </select>
                                        </div>
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-12">
                                            <textarea id="toyyibpay_notice"
                                                name="toyyibpay_notice"><?=$SMARTSTUDY->site('toyyibpay_notice');?></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_cash($SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_toyyibpay WHERE  `status` = 1 AND MONTH(create_gettime) = '$currentMonth' AND YEAR(create_gettime) = '$currentYear' ")['SUM(amount)']);?>
                                            RM
                                        </p>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_cash($SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_toyyibpay WHERE  `status` = 1 AND YEAR(create_gettime) = $currentYear AND WEEK(create_gettime, 1) = $currentWeek ")['SUM(amount)']);?>
                                            RM
                                        </p>
      ...
      echo '<span class="fs-12 text-danger ms-2"><i class="ti ti-trending-down me-1 d-inline-block"></i>' . round(abs($revenueGrowth), 2) . '% </span>';
            ...
            <div class="col-xl-7">
              ...
              ?>
                  ...
                  for ($day = 1; $day <= $numOfDays; $day++) {
                      $date = "$year-$month-$day";
                      $row = $SMARTSTUDY->get_row("SELECT SUM(amount) FROM payment_toyyibpay WHERE DATE(create_gettime) = '$date' AND  `status` = 1 ");
                                    ...
                                    <tr>
                                        <td colspan="7">
                                            <div class="float-right">
                                                <?=__('Paid:');?>
                                                <strong
                                                    style="color:red;"><?=format_cash($SMARTSTUDY->get_row(" SELECT SUM(`amount`) FROM `payment_toyyibpay` WHERE $where AND `status` = 1 ")['SUM(`amount`)']);?>
                                                    RM</strong>
