...
if (isset($_GET['id'])) {
    ...
    $row = $SMARTSTUDY->get_row("SELECT * FROM `email_campaigns` WHERE `id` = '$id' ");
    if (!$row) {
...
if(!empty($_GET['username'])){
    ...
    $user_id = $SMARTSTUDY->get_row(" SELECT `id` FROM `users` WHERE `username` = '$username' ")['id'];
    $where .= ' AND `user_id` = "'.$user_id.'" ';
}
if(!empty($_GET['email'])){
    $email = check_string($_GET['email']);
    $user_id = $SMARTSTUDY->get_row(" SELECT `id` FROM `users` WHERE `email` = '$email' ")['id'];
    $where .= ' AND `user_id` = "'.$user_id.'" ';
}
...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `email_sending` WHERE $where ORDER BY `id` ASC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `email_sending` WHERE $where ORDER BY id DESC ");
