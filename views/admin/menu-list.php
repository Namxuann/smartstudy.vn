...
if (isset($_POST['AddMenu'])) {
    ...
    if ($SMARTSTUDY->get_row("SELECT * FROM `menu` WHERE `name` = '".check_string($_POST['name'])."' ")) {
    ...
    $isCreate = $SMARTSTUDY->insert("menu", [
...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `menu` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `menu` WHERE $where ORDER BY id DESC ");
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-sm-8">
                            <select class="form-control mb-2" name="parent_id" required>
                                ...
                                <?php foreach($SMARTSTUDY->get_list("SELECT * FROM `menu` WHERE `parent_id` = 0 ") as $option):?>
                                <option value="<?=$option['id'];?>" <?=$id == $option['id'] ? 'selected' : '';?>>
                                    <?=$option['name'];?></option>
                                <?php foreach($SMARTSTUDY->get_list("SELECT * FROM `menu` WHERE `parent_id` = '".$option['id']."' ") as $option1):?>
