...
$body = [
    'title' => __('Chuyên mục bài viết').' | '.$SMARTSTUDY->site('title'),
...
if (isset($_POST['submit'])) {
    ...
    if ($SMARTSTUDY->get_row("SELECT * FROM `post_category` WHERE `name` = '".check_string($_POST['name'])."' ")) {
    ...
    $isInsert = $SMARTSTUDY->insert("post_category", [
        'icon'          => $url_icon,
...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `post_category` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `post_category` WHERE $where ORDER BY id DESC ");
