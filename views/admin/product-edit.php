...
if (isset($_GET['id'])) {
    ...
    if (!$product = $SMARTSTUDY->get_row("SELECT * FROM `products` WHERE `id` = '$id' ")) {
...
if (isset($_POST['save'])) {
    ...
    if ($product['name'] != check_string($_POST['name'])) {
        if ($SMARTSTUDY->get_row("SELECT * FROM `products` WHERE `name` = '" . check_string($_POST['name']) . "' ")) {
    ...
    if ($product['slug'] != check_string($_POST['slug'])) {
        if ($SMARTSTUDY->get_row("SELECT * FROM `products` WHERE `slug` = '" . check_string($_POST['slug']) . "' ")) {
    ...
    $isInsert = $SMARTSTUDY->update("products", [
        ...
        'min'               => $productType === 'course' ? 1 : (!empty($_POST['min']) ? check_string($_POST['min']) : 1),
        'max'               => $productType === 'course' ? 1 : (!empty($_POST['max']) ? check_string($_POST['max']) : 100000),
...
if (isset($_POST['AddDiscount'])) {
    ...
    $isInsert = $SMARTSTUDY->insert("product_discount", [
...
<div class="main-content app-content">
    <div class="container-fluid">
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            <?php if ($product['check_live'] == 'Clone' && time() - $SMARTSTUDY->site('time_cron_checklive_clone') >= 120): ?>
                <div class="alert alert-danger alert-dismissible fade show custom-alert-icon shadow-sm" role="alert">
                    ...
                    Vui lòng thực hiện <b><a target="_blank" class="text-primary"
                            href="https://smartstudy.vn">CRON
                            JOB</a></b> liên kết: <a class="text-primary" href="<?= base_url('cron/checklive/clone.php?key=' . $SMARTSTUDY->site('key_cron_job')); ?>"
                        target="_blank"><?= base_url('cron/checklive/clone.php?key=' . $SMARTSTUDY->site('key_cron_job')); ?></a> 1 phút 1 lần để hệ thống
                    check live tài khoản.
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"><i
                            class="bi bi-x"></i></button>
                </div>
            <?php endif ?>
            <?php if ($product['check_live'] == 'Hotmail' && time() - $SMARTSTUDY->site('time_cron_checklive_hotmail') >= 120): ?>
                <div class="alert alert-danger alert-dismissible fade show custom-alert-icon shadow-sm" role="alert">
                    <svg class="svg-danger" xmlns="http://www.w3.org/2000/svg" height="1.5rem" viewBox="0 0 24 24"
                        width="1.5rem" fill="#000000">
                        <path d="M0 0h24v24H0z" fill="none" />
                    ...
                    Vui lòng thực hiện <b><a target="_blank" class="text-primary"
                        ...
                        href="<?= base_url('cron/checklive/hotmail.php?key=' . $SMARTSTUDY->site('key_cron_job')); ?>"
                        target="_blank"><?= base_url('cron/checklive/hotmail.php?key=' . $SMARTSTUDY->site('key_cron_job')); ?></a> 1 phút 1 lần để hệ
                    thống
                    check live tài khoản.
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"><i
                            class="bi bi-x"></i></button>
            ...
            <?php endif ?>
            <?php if ($product['check_live'] == 'Gmail' && time() - $SMARTSTUDY->site('time_cron_checklive_gmail') >= 120): ?>
                <div class="alert alert-danger alert-dismissible fade show custom-alert-icon shadow-sm" role="alert">
                    <svg class="svg-danger" xmlns="http://www.w3.org/2000/svg" height="1.5rem" viewBox="0 0 24 24"
                        width="1.5rem" fill="#000000">
                        <path d="M0 0h24v24H0z" fill="none" />
                    ...
                    Vui lòng thực hiện <b><a target="_blank" class="text-primary"
                            href="https://smartstudy.vn">CRON
                            JOB</a></b> liên kết: <a class="text-primary" href="<?= base_url('cron/checklive/gmail.php?key=' . $SMARTSTUDY->site('key_cron_job')); ?>"
                        target="_blank"><?= base_url('cron/checklive/gmail.php?key=' . $SMARTSTUDY->site('key_cron_job')); ?></a> 1 phút 1 lần để hệ
                    thống
                    check live tài khoản.
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"><i
                            class="bi bi-x"></i></button>
            ...
            <?php endif ?>
            <?php if ($product['check_live'] == 'Instagram' && time() - $SMARTSTUDY->site('time_cron_checklive_instagram') >= 120): ?>
                <div class="alert alert-danger alert-dismissible fade show custom-alert-icon shadow-sm" role="alert">
                    <svg class="svg-danger" xmlns="http://www.w3.org/2000/svg" height="1.5rem" viewBox="0 0 24 24"
                        width="1.5rem" fill="#000000">
                        <path d="M0 0h24v24H0z" fill="none" />
                    ...
                    Vui lòng thực hiện <b><a target="_blank" class="text-primary"
                            href="https://smartstudy.vn">CRON
                            JOB</a></b> liên kết: <a class="text-primary"
                        href="<?= base_url('cron/checklive/instagram.php?key=' . $SMARTSTUDY->site('key_cron_job')); ?>"
                        target="_blank"><?= base_url('cron/checklive/instagram.php?key=' . $SMARTSTUDY->site('key_cron_job')); ?></a> 1 phút 1 lần để hệ
                    thống
                    check live tài khoản.
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"><i
                            class="bi bi-x"></i></button>
            ...
            <?php endif ?>
            <?php if ($product['check_live'] == 'Tiktok' && time() - $SMARTSTUDY->site('time_cron_checklive_tiktok') >= 120): ?>
                <div class="alert alert-danger alert-dismissible fade show custom-alert-icon shadow-sm" role="alert">
                    <svg class="svg-danger" xmlns="http://www.w3.org/2000/svg" height="1.5rem" viewBox="0 0 24 24"
                        width="1.5rem" fill="#000000">
                        <path d="M0 0h24v24H0z" fill="none" />
                    ...
                    Vui lòng thực hiện <b><a target="_blank" class="text-primary"
                            href="https://smartstudy.vn">CRON
                            JOB</a></b> liên kết: <a class="text-primary"
                        href="<?= base_url('cron/checklive/tiktok.php?key=' . $SMARTSTUDY->site('key_cron_job')); ?>"
                        target="_blank"><?= base_url('cron/checklive/tiktok.php?key=' . $SMARTSTUDY->site('key_cron_job')); ?></a> 1 phút 1 lần để hệ
                    thống
                    check live tài khoản.
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"><i
                            class="bi bi-x"></i></button>
                ...
                <div class="col-xl-4">
                    <div class="card custom-card">
                        <div class="card-body">
                            <div class="row mb-3">
                                <div class="col-sm-12 mb-2">
                                    <label class="form-label"><?= __('Loại sản phẩm:'); ?></label>
                                ...
                                <div class="col-sm-12 mb-2">
                                    ...
                                    <select class="form-control js-example-basic-single" name="category_id" required>
                                        ...
                                        <?php foreach ($SMARTSTUDY->get_list("SELECT * FROM `categories` WHERE `parent_id` = 0 ") as $option): ?>
                                            <option disabled value="<?= $option['id']; ?>"><?= $option['name']; ?></option>
                                            <?php foreach ($SMARTSTUDY->get_list("SELECT * FROM `categories` WHERE `parent_id` = '" . $option['id'] . "' ") as $option1): ?>
                ...
                <div class="modal-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-sm text-nowrap table-striped table-hover table-bordered">
                            ...
                            <tbody>
                                <?php foreach ($SMARTSTUDY->get_list(" SELECT * FROM product_discount WHERE product_id = '" . $product['id'] . "' ORDER BY min ASC ") as $product_discount): ?>
                                    <tr>
