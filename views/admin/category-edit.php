...
$body = [
    'title' => __('Chỉnh sửa chuyên mục') . ' | ' . $SMARTSTUDY->site('title')
];
$body['header'] = '
...
if (isset($_POST['SaveCategory'])) {
    ...
    if ($row['parent_id'] == 0 && $parent_id != 0) {
        $childCount = $SMARTSTUDY->num_rows_safe("SELECT * FROM `categories` WHERE `parent_id` = ?", [$row['id']]);
        if ($childCount > 0) {
    ...
    if ($SMARTSTUDY->get_row_safe("SELECT * FROM `categories` WHERE `name` = ? AND `id` != ?", [$name, $row['id']])) {
    ...
    if ($SMARTSTUDY->get_row_safe("SELECT * FROM `categories` WHERE `slug` = ? AND `id` != ?", [$slug, $row['id']])) {
    ...
    $isInsert = $SMARTSTUDY->update("categories", [
        'stt'               => $stt,
        'parent_id'         => $parent_id,
        'name'              => $name,
        'slug'              => $slug,
                        ...
                        <form action="" method="POST" enctype="multipart/form-data">
                            <div class="row mb-4">
                                <div class="col-sm-12">
                                    ...
                                    <?php
                                    $hasChildren = $SMARTSTUDY->num_rows_safe("SELECT * FROM `categories` WHERE `parent_id` = ?", [$row['id']]);
