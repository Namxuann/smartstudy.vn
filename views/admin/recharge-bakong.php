...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `payment_bakong` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `payment_bakong` WHERE $where ORDER BY id DESC ");
...
$total_yesterday = intval($SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_bakong WHERE `status` = 1 AND  `created_at` LIKE '%".$yesterday."%' ")['SUM(price)']);
$total_today = $SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_bakong WHERE `status` = 1 AND `created_at` LIKE '%".$currentDate."%' ")['SUM(price)'];
$total_all_time = $SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_bakong WHERE `status` = 1 ")['SUM(price)'];
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-5">
                                            <select class="form-control" name="bakong_status">
                                                <option <?=$SMARTSTUDY->site('bakong_status') == 1 ? 'selected' : '';?>
                                                    value="1">ON
                                                </option>
                                                <option <?=$SMARTSTUDY->site('bakong_status') == 0 ? 'selected' : '';?>
                                                    value="0">
                                                    OFF
                                                </option>
                                            </select>
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control" id="bakong_profile_id"
                                                value="<?=$SMARTSTUDY->site('bakong_profile_id');?>"
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control" id="bakong_profile_key"
                                                value="<?=$SMARTSTUDY->site('bakong_profile_key');?>"
                                        ...
                                        <div class="col-sm-5">
                                            <div class="input-group">
                                                <input type="text" class="form-control"
                                                    value="<?=$SMARTSTUDY->site('bakong_rate');?>" name="bakong_rate"
                                                    placeholder="">
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="number" class="form-control"
                                                value="<?=$SMARTSTUDY->site('bakong_min');?>" name="bakong_min">
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="number" class="form-control"
                                                value="<?=$SMARTSTUDY->site('bakong_max');?>" name="bakong_max">
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$SMARTSTUDY->site('bakong_proxy');?>" name="bakong_proxy">
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-12">
                                            <textarea id="bakong_notice"
                                                name="bakong_notice"><?=$SMARTSTUDY->site('bakong_notice');?></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_currency($SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_bakong WHERE `status` = 1 AND MONTH(created_at) = '$currentMonth' AND YEAR(created_at) = '$currentYear' ")['SUM(price)']);?>
                                        </p>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_currency($SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_bakong WHERE `status` = 1 AND YEAR(created_at) = $currentYear AND WEEK(created_at, 1) = $currentWeek ")['SUM(price)']);?>
                                        </p>
      ...
      echo '<span class="fs-12 text-danger ms-2"><i class="ti ti-trending-down me-1 d-inline-block"></i>' . round(abs($revenueGrowth), 2) . '% </span>';
            ...
            <div class="col-xl-7">
              ...
              ?>
                  ...
                  for ($day = 1; $day <= $numOfDays; $day++) {
                      $date = "$year-$month-$day";
                      $row = $SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_bakong WHERE `status` = 1 AND DATE(created_at) = '$date' ");
                                    ...
                                    <tr>
                                        <td colspan="7">
                                            <div class="float-right">
                                                <?=__('Paid:');?>
                                                <strong
                                                    style="color:red;"><?=format_currency($SMARTSTUDY->get_row(" SELECT SUM(`price`) FROM `payment_bakong` WHERE $where AND `status` = 1 ")['SUM(`price`)']);?></strong>
