...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `payment_flutterwave` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `payment_flutterwave` WHERE $where ORDER BY id DESC ");
...
$total_yesterday = intval($SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_flutterwave WHERE `status` = 'success' AND  `create_gettime` LIKE '%".$yesterday."%' ")['SUM(price)']);
$total_today = $SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_flutterwave WHERE `status` = 'success' AND `create_gettime` LIKE '%".$currentDate."%' ")['SUM(price)'];
$total_all_time = $SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_flutterwave WHERE `status` = 'success' ")['SUM(price)'];
                            ...
                            <div class="row">
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-5">
                                            <select class="form-control" name="flutterwave_status">
                                                <option <?=$SMARTSTUDY->site('flutterwave_status') == 1 ? 'selected' : '';?>
                                                    value="1">ON
                                                </option>
                                                <option <?=$SMARTSTUDY->site('flutterwave_status') == 0 ? 'selected' : '';?>
                                                    value="0">
                                                    OFF
                                                </option>
                                            </select>
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-5">
                                            <input type="text" class="form-control"
                                                value="<?=$SMARTSTUDY->site('flutterwave_currency_code');?>"
                                                name="flutterwave_currency_code">
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        <label class="col-sm-7 col-form-label"
                                            for="example-hf-email"><?=__('1 '.$SMARTSTUDY->site('flutterwave_currency_code').' =');?></label>
                                        <div class="col-sm-5">
                                            <div class="input-group">
                                                <input type="text" class="form-control"
                                                    value="<?=$SMARTSTUDY->site('flutterwave_rate');?>" name="flutterwave_rate"
                                                    placeholder="">
                                ...
                                <div class="col-lg-12 col-xl-6">
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$SMARTSTUDY->site('flutterwave_secretKey');?>"
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$SMARTSTUDY->site('flutterwave_publicKey');?>"
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-12">
                                            <textarea id="flutterwave_notice"
                                                name="flutterwave_notice"><?=$SMARTSTUDY->site('flutterwave_notice');?></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_currency($SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_flutterwave WHERE `status` = 'success' AND MONTH(create_gettime) = '$currentMonth' AND YEAR(create_gettime) = '$currentYear' ")['SUM(price)']);?>
                                        </p>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_currency($SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_flutterwave WHERE `status` = 'success' AND YEAR(create_gettime) = $currentYear AND WEEK(create_gettime, 1) = $currentWeek ")['SUM(price)']);?>
                                        </p>
      ...
      echo '<span class="fs-12 text-danger ms-2"><i class="ti ti-trending-down me-1 d-inline-block"></i>' . round(abs($revenueGrowth), 2) . '% </span>';
            ...
            <div class="col-xl-7">
              ...
              ?>
                  ...
                  for ($day = 1; $day <= $numOfDays; $day++) {
                      $date = "$year-$month-$day";
                      $row = $SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_flutterwave WHERE `status` = 'success' AND DATE(create_gettime) = '$date' ");
                                    ...
                                    <tr>
                                        <td colspan="7">
                                            <div class="float-right">
                                                <?=__('Paid:');?>
                                                <strong
                                                    style="color:red;"><?=format_currency($SMARTSTUDY->get_row(" SELECT SUM(`price`) FROM `payment_flutterwave` WHERE $where ")['SUM(`price`)']);?></strong>
