...
$body = [
    'title' => 'Toàn bộ tài khoản đã bán | '.$SMARTSTUDY->site('title'),
...
if(isset($_POST['RemoveAccounts'])){
    ...
    foreach ($list as $account){
        ...
        $isRemove = $SMARTSTUDY->remove("product_sold", " `uid` = '$uid_remove' ");
        if ($isRemove) {
            $value_remove++;
...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `product_sold` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `product_sold` WHERE $where ORDER BY id DESC ");
