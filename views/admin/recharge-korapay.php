...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `payment_korapay` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `payment_korapay` WHERE $where ORDER BY id DESC ");
...
$total_yesterday = intval($SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_korapay WHERE `status` = 1 AND  `created_at` LIKE '%".$yesterday."%' ")['SUM(price)']);
$total_today = $SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_korapay WHERE `status` = 1 AND `created_at` LIKE '%".$currentDate."%' ")['SUM(price)'];
$total_all_time = $SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_korapay WHERE `status` = 1 ")['SUM(price)'];
                            ...
                            <div class="row">
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-5">
                                            <select class="form-control" name="korapay_status">
                                                <option <?=$SMARTSTUDY->site('korapay_status') == 1 ? 'selected' : '';?>
                                                    value="1">ON
                                                </option>
                                                <option <?=$SMARTSTUDY->site('korapay_status') == 0 ? 'selected' : '';?>
                                                    value="0">
                                                    OFF
                                                </option>
                                            </select>
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-5">
                                            <input type="text" class="form-control"
                                                value="<?=$SMARTSTUDY->site('korapay_currency_code');?>"
                                                name="korapay_currency_code">
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        <label class="col-sm-7 col-form-label"
                                            for="example-hf-email"><?=__('1 '.$SMARTSTUDY->site('korapay_currency_code').' =');?></label>
                                        <div class="col-sm-5">
                                            <div class="input-group">
                                                <input type="text" class="form-control"
                                                    value="<?=$SMARTSTUDY->site('korapay_rate');?>" name="korapay_rate"
                                                    placeholder="">
                                ...
                                <div class="col-lg-12 col-xl-6">
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$SMARTSTUDY->site('korapay_secretKey');?>"
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$SMARTSTUDY->site('korapay_proxy');?>" name="korapay_proxy">
                                ...
                                <div class="col-lg-12 col-xl-6">
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$SMARTSTUDY->site('korapay_min');?>" name="korapay_min">
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                value="<?=$SMARTSTUDY->site('korapay_max');?>" name="korapay_max">
                                        </div>
                                    </div>
                                </div>
                                <!-- Khuyến mãi nạp tiền Korapay - cho phép admin cấu hình mốc nạp và % khuyến mãi -->
                                <div class="col-lg-12 col-xl-12">
                                    <div class="row mb-4">
                                        <label class="col-sm-6 col-form-label"
                                            for="example-hf-email"><?=__('Khuyến mãi nạp tiền nếu có');?></label>
                                        <div class="col-sm-6">
                                            <textarea class="form-control" rows="4" placeholder="100|5 (<?=sprintf(__('tức nạp %s %s khuyến mãi %s%%'), '100', $SMARTSTUDY->site('korapay_currency_code'), '5');?>)
200|10 (<?=sprintf(__('tức nạp %s %s khuyến mãi %s%%'), '200', $SMARTSTUDY->site('korapay_currency_code'), '10');?>)
" name="korapay_promotions"><?=$SMARTSTUDY->site('korapay_promotions');?></textarea>
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-12">
                                            <textarea id="korapay_notice"
                                                name="korapay_notice"><?=$SMARTSTUDY->site('korapay_notice');?></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_currency($SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_korapay WHERE `status` = 1 AND MONTH(created_at) = '$currentMonth' AND YEAR(created_at) = '$currentYear' ")['SUM(price)']);?>
                                        </p>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?=format_currency($SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_korapay WHERE `status` = 1 AND YEAR(created_at) = $currentYear AND WEEK(created_at, 1) = $currentWeek ")['SUM(price)']);?>
                                        </p>
      ...
      echo '<span class="fs-12 text-danger ms-2"><i class="ti ti-trending-down me-1 d-inline-block"></i>' . round(abs($revenueGrowth), 2) . '% </span>';
            ...
            <div class="col-xl-7">
              ...
              ?>
                  ...
                  for ($day = 1; $day <= $numOfDays; $day++) {
                      $date = "$year-$month-$day";
                      $row = $SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_korapay WHERE `status` = 1 AND DATE(created_at) = '$date' ");
                            ...
                            <table class="table text-nowrap table-striped table-hover table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-right"><b><?=format_cash($row['amount']);?></b> <?=$SMARTSTUDY->site('korapay_currency_code');?></td>
                                    ...
                                    <tr>
                                        <td colspan="7">
                                            <div class="float-right">
                                                <?=__('Paid:');?>
                                                <strong
                                                    style="color:red;"><?=format_currency($SMARTSTUDY->get_row(" SELECT SUM(`price`) FROM `payment_korapay` WHERE $where AND `status` = 1 ")['SUM(`price`)']);?></strong>
