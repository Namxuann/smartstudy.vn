...
if (isset($_GET['id'])) {
    ...
    $user = $SMARTSTUDY->get_row_safe("SELECT * FROM `users` WHERE `id` = ?", [$id]);
    if (!$user) {
    ...
    if (isset($_POST['btnSaveUser'])) {
        ...
        if ($_POST['username'] != $user['username']) {
            ...
            if ($SMARTSTUDY->get_row_safe(" SELECT * FROM `users` WHERE `username` = ? AND `id` != ? ", [check_string($_POST['username']), $user['id']])) {
        ...
        if ($_POST['email'] != $user['email']) {
            ...
            if ($SMARTSTUDY->get_row_safe(" SELECT * FROM `users` WHERE `email` = ? AND `id` != ? ", [check_string($_POST['email']), $user['id']])) {
        ...
        if ($SMARTSTUDY->site('bank_recharge_type') == 'fullname_transfer' && isset($_POST['prefix_fullname'])) {
            ...
            if (!empty($prefix_fullname) && $prefix_fullname !== ($user['prefix_fullname'] ?? '')) {
                $existingUser = $SMARTSTUDY->get_row_safe("SELECT `id`, `username` FROM `users` WHERE `prefix_fullname` = ? AND `id` != ?", [$prefix_fullname, $user['id']]);
                if ($existingUser) {
        ...
        $SMARTSTUDY->update('users', $updateData, "`id` = ?", [$user['id']]);
    ...
    if (isset($_POST['cong_tien'])) {
        ...
        if ($_POST['wallet'] == 3) {
            ...
            $isDebit = $SMARTSTUDY->cong('users', 'debit', $amount, " `id` = ? ", [$id]);
            if ($isDebit) {
                $DBUser->AddCredits($id, $amount, $reason, 'admin_add_' . uniqid());
            }
        }
    ...
    if (isset($_POST['tru_tien'])) {
        ...
        if ($_POST['wallet'] == 2) {
            $SMARTSTUDY->tru('users', 'debit', $amount, " `id` = ? ", [$id]);
                            ...
                            <div class="tab-content">
                                ...
                                <div class="tab-pane fade show active" id="basic-info" role="tabpanel">
                                    <div class="row">
                                        ...
                                        <?php if ($SMARTSTUDY->site('bank_recharge_type') == 'fullname_transfer'): ?>
                                            <div class="col-md-6">
                                                <div class="mb-4">
                                        ...
                                        <div class="col-md-4">
                                            <div class="mb-4">
                                                ...
                                                <select class="form-select" name="admin">
                                                    ...
                                                    <?php foreach ($SMARTSTUDY->get_list(" SELECT * FROM `admin_role` ") as $role): ?>
                    ...
                    <div class="card-body">
                        <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                            <table class="table table-sm table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $recentLogs = $SMARTSTUDY->get_list_safe("SELECT * FROM `logs` WHERE `user_id` = ? ORDER BY `id` DESC LIMIT 20", [$user['id']]);
                                    if (empty($recentLogs)): ?>
                                        <tr>
                    ...
                    <div class="card-body">
                        <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                            <table class="table table-sm table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $recentTransactions = $SMARTSTUDY->get_list_safe("SELECT * FROM `dongtien` WHERE `user_id` = ? ORDER BY `id` DESC LIMIT 20", [$user['id']]);
