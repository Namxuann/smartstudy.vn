...
if(!empty($_GET['username'])){
    ...
    if($u = $SMARTSTUDY->get_row("SELECT id FROM `users` WHERE `username` = '$username' ")){
...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `product_order` WHERE $where AND `seller` > 0 ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `product_order` WHERE $where AND `seller` > 0 ");
...
$kpi_total_revenue = $SMARTSTUDY->get_row("SELECT SUM(pay) FROM `product_order` WHERE `trash` = 0 AND `seller` > 0 ")["SUM(pay)"];
$kpi_total_orders = $SMARTSTUDY->get_row("SELECT COUNT(*) FROM `product_order` WHERE `trash` = 0 AND `seller` > 0 ")["COUNT(*)"];
$kpi_month_revenue = $SMARTSTUDY->get_row("SELECT SUM(pay) FROM `product_order` WHERE `trash` = 0 AND `seller` > 0 AND MONTH(create_gettime) = '$currentMonth' AND YEAR(create_gettime) = '$currentYear' ")["SUM(pay)"];
$kpi_today_revenue = $SMARTSTUDY->get_row("SELECT SUM(pay) FROM `product_order` WHERE `trash` = 0 AND `seller` > 0 AND `create_gettime` LIKE '%$currentDate%' ")["SUM(pay)"];
