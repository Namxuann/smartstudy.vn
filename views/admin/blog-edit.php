...
if (isset($_GET['id'])) {
    ...
    if (!$row = $SMARTSTUDY->get_row("SELECT * FROM `posts` WHERE `id` = '$id' ")) {
...
if (isset($_POST['Save'])) {
    ...
    if ($SMARTSTUDY->get_row("SELECT * FROM `posts` WHERE `title` = '".check_string($_POST['title'])."' AND `id` != ".$row['id']." ")) {
    ...
    if (check_img('image') == true) {
        ...
        if ($addlogo) {
            $SMARTSTUDY->update("posts", [
                'image' => $uploads_dir
            ], " `id` = '".$row['id']."' ");
        }
    }
    $isInsert = $SMARTSTUDY->update("posts", [
