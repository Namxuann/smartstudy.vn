...
$body = [
    'title' => __('Block IP').' | '.$SMARTSTUDY->site('title'),
...
if (isset($_POST['AddIPBlock'])) {
    ...
    $isInsert = $SMARTSTUDY->insert("block_ip", [
        'ip'                => $ip,
        'attempts'          => 0,
        'banned'            => 1,
...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `block_ip` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `block_ip` WHERE $where ORDER BY id DESC ");
