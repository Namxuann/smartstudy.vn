...
if (isset($_POST['RegenerateSecret'])) {
    ...
    if ($SMARTSTUDY->get_row_safe("SELECT * FROM `settings` WHERE `name` = ?", ['zinipay_callback_secret'])) {
        $SMARTSTUDY->update('settings', ['value' => $newSecret], " `name` = 'zinipay_callback_secret' ");
    } else {
        $SMARTSTUDY->insert('settings', ['name' => 'zinipay_callback_secret', 'value' => $newSecret]);
    }
...
if (isset($_POST['CheckTransaction'])) {
    ...
    $row = $SMARTSTUDY->get_row_safe("SELECT * FROM `payment_zinipay` WHERE `id` = ? AND `status` = 0", [$checkId]);
    if (!$row) {
    ...
    if ($verifyStatus === 'COMPLETED') {
        ...
        $claimed = $SMARTSTUDY->update('payment_zinipay', [
            'status'     => 1,
        ...
        if ($isCong) {
            ...
            $SMARTSTUDY->update('payment_zinipay', [
                'status'     => 0,
                'updated_at' => gettime()
...
$listDatatable = $SMARTSTUDY->get_list(" SELECT * FROM `payment_zinipay` WHERE $where ORDER BY `id` DESC LIMIT $from,$limit ");
$totalDatatable = $SMARTSTUDY->num_rows(" SELECT * FROM `payment_zinipay` WHERE $where ORDER BY id DESC ");
...
$total_yesterday = intval($SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_zinipay WHERE `status` = 1 AND  `created_at` LIKE '%" . $yesterday . "%' ")['SUM(price)']);
$total_today = $SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_zinipay WHERE `status` = 1 AND `created_at` LIKE '%" . $currentDate . "%' ")['SUM(price)'];
$total_all_time = $SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_zinipay WHERE `status` = 1 ")['SUM(price)'];

// Tự sinh mã bí mật cho URL callback nếu chưa có (áp dụng cả site đã cài đặt sẵn)
$zinipayCallbackSecret = $SMARTSTUDY->site('zinipay_callback_secret');
if (empty($zinipayCallbackSecret)) {
    $zinipayCallbackSecret = bin2hex(random_bytes(16));
    if ($SMARTSTUDY->get_row_safe("SELECT * FROM `settings` WHERE `name` = ?", ['zinipay_callback_secret'])) {
        $SMARTSTUDY->update('settings', ['value' => $zinipayCallbackSecret], " `name` = 'zinipay_callback_secret' ");
    } else {
        $SMARTSTUDY->insert('settings', ['name' => 'zinipay_callback_secret', 'value' => $zinipayCallbackSecret]);
    }
}
                            ...
                            <div class="row">
                                <div class="col-lg-12 col-xl-6">
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-7">
                                            <select class="form-control" name="zinipay_status">
                                                <option <?= $SMARTSTUDY->site('zinipay_status') == 1 ? 'selected' : ''; ?> value="1">ON</option>
                                                <option <?= $SMARTSTUDY->site('zinipay_status') == 0 ? 'selected' : ''; ?> value="0">OFF</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-7">
                                            <input type="text" class="form-control" value="<?= $SMARTSTUDY->site('zinipay_name') ?: 'ZiniPay (bKash, Nagad)'; ?>" name="zinipay_name" placeholder="ZiniPay (bKash, Nagad)">
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-7">
                                            <div class="d-flex align-items-center mb-2">
                                                <?php if ($SMARTSTUDY->site('zinipay_icon')): ?>
                                                    <img src="<?= base_url($SMARTSTUDY->site('zinipay_icon')); ?>" width="40" height="40" class="me-2 rounded" style="object-fit: contain;">
                                                <?php endif; ?>
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-7">
                                            <input type="text" class="form-control" value="<?= $SMARTSTUDY->site('zinipay_api_key'); ?>" name="zinipay_api_key" placeholder="zini-api-key">
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-7">
                                            <input type="text" class="form-control" value="<?= $SMARTSTUDY->site('zinipay_api_url') ?: 'https://api.zinipay.com'; ?>" name="zinipay_api_url">
                                ...
                                <div class="col-lg-12 col-xl-6">
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-7">
                                            <input type="text" class="form-control" value="<?= $SMARTSTUDY->site('zinipay_min'); ?>" name="zinipay_min">
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        <label class="col-sm-5 col-form-label">Max (BDT)</label>
                                        <div class="col-sm-7">
                                            <input type="text" class="form-control" value="<?= $SMARTSTUDY->site('zinipay_max'); ?>" name="zinipay_max">
                                        </div>
                                    </div>
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-7">
                                            <div class="input-group">
                                                <input type="text" class="form-control" value="<?= $SMARTSTUDY->site('zinipay_rate'); ?>" name="zinipay_rate">
                                ...
                                <div class="col-lg-12">
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-10">
                                            <textarea id="zinipay_notice" name="zinipay_notice"><?= $SMARTSTUDY->site('zinipay_notice'); ?></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?= format_currency($SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_zinipay WHERE `status` = 1 AND MONTH(created_at) = '$currentMonth' AND YEAR(created_at) = '$currentYear' ")['SUM(price)']); ?>
                                        </p>
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-fill">
                                        <p class="mb-1 fs-5 fw-semibold text-default">
                                            <?= format_currency($SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_zinipay WHERE `status` = 1 AND YEAR(created_at) = $currentYear AND WEEK(created_at, 1) = $currentWeek ")['SUM(price)']); ?>
                                        </p>
                                ...
                                const data = {
                                    ...
                                    datasets: [{
                                        ...
                                        data: [
                                            ...
                                            for ($day = 1; $day <= $numOfDays; $day++) {
                                                $date = "$year-$month-$day";
                                                $row = $SMARTSTUDY->get_row("SELECT SUM(price) FROM payment_zinipay WHERE `status` = 1 AND DATE(created_at) = '$date' ");
                                    ...
                                    <tr>
                                        <td colspan="8">
                                            <div class="float-right">
                                                <?= __('Paid:'); ?> <strong style="color:red;"><?= format_currency($SMARTSTUDY->get_row(" SELECT SUM(`price`) FROM `payment_zinipay` WHERE $where AND `status` = 1 ")['SUM(`price`)']); ?></strong>
                                            </div>
                                        </td>
                                    </tr>
                                </tfoot>
